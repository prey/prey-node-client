...
var ActionsManager = function(){
  ...
  this.start_action = function(action, options){
    ...
    action.start(options || {}, function(err, emitter){
      self.action_returned(action, err, emitter);
    });

  };

  this.action_returned = function(action, err, emitter){

    if (err) {
      this.emit('action_failed', action.name, err);
      this.log('Could not start action: ' + err.message);
    } else {
      this.emit('action_started', action.name);
      this.log('Action run with great success: ' + action.name);
    }
    ...
    if (!err && (action.events || (emitter && emitter.emit)))
      this.wait_until_finished(action, emitter);
    else
      this.remove_from_running(action); // no need to emit 'action_finished'
