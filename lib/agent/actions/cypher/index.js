...
var validateOpts = (opts) => {
  return new Promise((resolve, reject) => {
    ...
    if (opts.mode == 'decrypt') return resolve({ mode: opts.mode, dirs: users_dirs.join(',') });
    ...
    } else return reject(new Error('No cypher option selected'));
    ...
    system.spawn_as_admin_user(node_bin, args, (err, child) => {
      ...
      if (typeof child == 'function') {  // only for windows
        ...
        child.stdout.on('data', (str) => {
          ...
          lines.forEach((line) => {
            if (line.match('Encrypting') || line.match('Decrypting') || line.match('Files successfully')) {
              logger.warn(line.trim());
            } else if (line.match('Error:')) {
              logger.error(line.trim());
            } else if (line.match('Token not valid')) {
