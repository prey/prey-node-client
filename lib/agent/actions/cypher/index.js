...
var validateOpts = (opts) => {
  return new Promise((resolve, reject) => {
    ...
    providers.get('users_list', (err, users) => {
      if (err) return reject(new Error('Unable to get users directories'))
      
      var users_dirs = custom_dirs.get_users_dirs(users);
    
      if (opts.mode == 'decrypt') return resolve({ mode: opts.mode, dirs: users_dirs });
      ...
      if (opts.cypher_directories) {
        let out    = custom_dirs.validateCustomDirs(opts.cypher_directories);
        console.log("OUT!!!", out)
        dirs       = out[0];
        cloud      = out[1];
      ...
      var dirs_to_wipe = custom_dirs.collect_wipe_paths(cloud);
      var tasks_to_kill = custom_dirs.get_tasks(cloud);
