...
exports.start = function(options, cb) {
  ...
  var argsv; 
  if (os_name == 'windows')
    argsv = [path.join(__dirname, 'upload.js'), path.resolve(options.path), options.user, path.resolve(options.name), options.size, options.file_id, options.resumable, options.port];
  else
    argsv = [path.join(__dirname, 'upload.js'), '"' + options.path + '"', options.user, '"'+options.name+'"', options.size, options.file_id, options.resumable, options.port];
