...
const retrieve_file_as_user = function (options, cb) {
  ...
  run_as_user(opts, (err, out) => {
    ...
    if (out.includes('EPIPE') || out.includes('EACCES')) {
      ...
      storage.do('update', {
        type: 'files', id: options.file_id, columns: 'resumable', values: resumable_value,
      }, (err) => { // resumable
        ...
        if (files && files.length == 0) {
          ...
          setTimeout(() => {
            ...
            if (options.resumable == 1) {
              storage.do('update', {
                type: 'files', id: options.file_id, columns: ['attempts', 'resumable'], values: [file_attempts + 1, 0],
              }, (err) => { // resumable
                if (err) logger.error('Database update error');
