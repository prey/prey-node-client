...
function sync(geofences) {
  ...
  if (len === 0) {
    ...
    storage.get_geofences(function(err, stored_geofences) {
      start_watcher(geofences, stored_geofences, done);
    });
  }

  function start_watcher(geofences, stored_geofences, cb) {
    watcher.start(geofences, stored_geofences, function(err, fence) {
