...
exports.sync = function(geofences) {
  ...
  storage2.do('all', { type: 'geofences' }, (err, stored_geofences) => {
    let array = [];
    ...
    stored_geofences.forEach((stored_fence) => {
      array.push((callback) => {
        ...
        if (occurrences.length > 0) {
          ...
          storage2.do('del', {type: 'geofences', id: stored_fence.id}, callback);
        }
      })
    })
    ...
    async.series(array, (err) => {
      ...
      fences.forEach(fence => {
        array2.push((callback) => {
          ...
          storage2.do('set', {type: 'geofences', id: fence.id, data: {name: fence.name, state: 'NULL'}}, callback)
        }) 
      })
