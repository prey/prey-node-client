...
const setTouchPadState = (state, id = null, options = null, cb = null) => {
  let data = {
  ...
  system.spawn_as_admin_user(node_bin, data, function(err, touchpad) {
    if(err) {
      logger.info('Error Enabling Touchpad:' + JSON.stringify(err));
      if(state !== 'disable' && (!id || !options || !cb)) return;
      start(id, options, cb);
      return;
    }
    if (typeof touchpad == 'function') {
      touchpad(action, data);
      if(state !== 'disable' && (!id || !options || !cb)) return;
      start(id, options, cb);
    }
  });
...
exports.start = function(id, options, cb){
  ...
  if(is_win)
    setTouchPadState("Disable", id, options, cb);
  else
    start(id, options, cb);
};
