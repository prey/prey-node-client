...
var util        = require('util'),
    ...
    os_path     = join(__dirname, osName), // used for cwd when spawning child
...
function lock_binary_path() {
  ...
  if (is_win) {
    ...
    binary_name += "3";
  } else if (is_mac && common.osRelease >= '11.0') {
...
function after(cb) {
  ...
  exec(lock_binary + ' --unblock', function() {
    if (is_win) run_as_user(join(__dirname, osName, 'tb-enable'), [], cb);
    else return cb();
  });
}
...
function start(id, opts, cb) {
  ...
  if (osName == 'windows' && path.basename(lock_binary) == 'prey-lock') // old prey-lock binary for windows
    password = password;
  else
...
function open(id, password, message, cb) {
  ...
  if (is_mac && common.osRelease >= '11.0')
    args.unshift('-lock')
