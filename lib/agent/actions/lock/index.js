...
function start(opts, cb) {
  ...
  before(function() {
    open(password, message, cb)
  })
}

function open(password, message, cb) {
  if (!message) message = "";

  timer = null;
  system.spawn_as_logged_user(lock_binary, [password, message], { cwd: os_path }, function(err, lock) {
    if (err || stopped) {

      // if no logged user is found, retry in a sec or two.
      if (err && err.code == 'NO_LOGGED_USER') {
        timer = setTimeout(function() { open(password, message, cb) }, 5000);
        return;
      } else {
