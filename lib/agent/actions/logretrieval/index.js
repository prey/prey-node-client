...
let COMMANDS_PATH = join(CONFIG_PATH, 'commands.db');
let commandsJsonPath = join(exports.tmpdir, 'commands.json');
  ...
  new Promise((resolve) => {
    utilStorage.getDataDbKey('hardware_changed', (_errorDataHardware, txtHardware) => {
      ...
      if (txtHardware !== '') {
        ...
        else jsonHardwareInformation = JSON.stringify(txtHardware, null, 2);
      }
  ...
  new Promise((resolve) => {
    utilStorage.getDataDbKey('wifiDataStored', (_errorDataWifi, txt) => {
      ...
      if (txt !== '') {
        ...
        else jsonInformation = JSON.stringify(txt, null, 2);
      }
  ...
  new Promise((resolve) => {
    (async () => {
      try {
        ...
        if (!isObjectEmpty(jsonDb)) fs.writeFileSync(commandsJsonPath, JSON.stringify(jsonDb, null, 2), { flag: 'w+' });
        resolve();
      } catch (error) {
        resolve(error);
      }
...
exports.start = (id, options, cb) => {
  ...
  Promise.all(promises).then((results) => {
    const [hardwareData, wifiData] = results;
