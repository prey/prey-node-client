...
/**
 * @param {string} id
 * @param {{ upn?: string, secret?: string, discovery_url?: string, opts?: object }|null} opts
 * @param {Function} cb
 */
...
exports.start = (id, opts, cb) => {
  ...
  const options = /** @type {any} */ (opts?.opts || opts || {});
  const { upn, secret, discovery_url: discoveryUrl } = options;

  if (!upn) return cb(new Error('Missing required field: upn'));
  ...
  let active = true;
  const done = (doneId, err, out) => {
    if (active) { emitter.emit('end', doneId, err, out); active = false; }
  };

  cb(null, emitter);

  logger.info(`Starting MDM enrollment for ${upn}`);
  ...
  needle.post(MDM_ENDPOINT, payload, { json: true, timeout: 60000 }, (err, res) => {
    ...
    logger.info(`MDM enrollment succeeded for ${upn}`);
