...
function fetch_triggers(cb) {
  ...
  // return cb(null, {body: [test.exact_triggers[0], test.event_triggers[0], test.repeat_triggers[0]]})
}
...
exports.activate = (trigger) => {
  ...
  if (info.date) {
    ...
    try{
      ...
      if (info.until) {
        // Si ya pasó la fecha definida en until retornar false
  ...
  function start_watcher(cb) {
    ...
    function notify_done() {
      var data = {
        ...
        command: 'start',
        target: 'triggers',
        reason: JSON.stringify(watching)   // FALTA EL ARREGLO CON LOS IDS!!!
      }
  ...
  fetch_triggers((err, res) => {
    if (err) {
      ...
      storage.clear_triggers((err) => {
        ...
        storage.get_triggers((err, triggs) => {
          exports.sync(triggers);
          return cb && cb(null, emitter);
        })
      });
