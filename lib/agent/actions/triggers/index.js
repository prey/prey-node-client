...
function run_trigger_actions(trigger) {
  ...
  storage2.do('update', {type: 'triggers', id: trigger.id, columns: 'last_exec', values: new Date().getTime() }, () => {
      // send_response('stopped', [trigger.id]);  ---> revisar!!!
    ...
    hooks.on(event.split('-')[0], (info) => {
      ...
      event_triggers[event].forEach(action => {
        ...
        if (action.after) {
          ...
          storage2.do('update', {type: 'triggers', id: current.id, columns: 'synced_at', values: exec_time}, (err) => {
...
exports.sync = (err, triggers, stored, cb) => {
  ...
  triggers.forEach((trigger, index) => {
    ...
    if (trigger.options && trigger.options.persist == true) {  // ta bien
      trigger.persist = 1;
    }
    ...
    if (state != 1 && state != 6) {
      ...
      if (!trigger.persist || trigger.persist == false || trigger.persist == 0)
        data.persist = 0;
      if (trigger.persist && (trigger.persist == true || trigger.persist == 1))
        data.persist = 1;
