  ...
  timeoutStartTrigger = setTimeout(() => {
    ...
    fetchTriggers((err, res) => {
      if (err) {
        ...
        let fetchedTriggers;
        try {
          const filteredUnseen = res.body.filter((element) => {
            // eslint-disable-next-line no-param-reassign
            if (typeof element.automation_events === 'string') element.automation_events = JSON.parse(element.automation_events);
            return !!(element.automation_events
              && element.automation_events.filter(
                (event) => event.type.localeCompare('device_unseen') === 0
          && typeof event.type === 'string',
              ).length > 0);
          });
          const unseenModified = filteredUnseen.map((obj) => ({
            ...obj,
            automation_events: obj.automation_events.map((event) => {
              if (event.type === 'after_time' && event.info && event.info.seconds) {
                return {
                  ...event,
                  info: { ...event.info, seconds: event.info.seconds / 144 },
                };
              }
              return event;
            }),
          ...
          const lookup = {};
          unseenModified.forEach((element) => {
            lookup[element.id] = element;
          });
          fetchedTriggers = res.body.map((element) => {
