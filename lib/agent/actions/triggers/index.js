...
function run_action(trigger, action) {
  ...
  setTimeout(() => {
    ...
    logger.info('action to perform')
    logger.info(JSON.stringify(action));
    //{"action":{"command":"start","target":"alert","options":{"alert_message":"Mensajito","trigger_id":10222}},"delay":0}
...
exports.sync = (id, err, triggers, stored, cb) => {
  ...
  logger.info('stored triggers: ' + JSON.stringify(stored));
  logger.info('triggersToWatch: ' + JSON.stringify(triggersToWatch));
  exports.cancel_all();
  event_triggers = {};

  // iterate over only active and filtered triggers
  triggersToWatch.forEach((trigger, index) => {
    logger.info('Verifying ID: ' + trigger.id);
    logger.info(`value de: ${stored.filter((x)=> x.id == trigger.id).length}`);
    ...
    if (state != 1 && state != 6) {
      logger.info('cancel trigger ID: ' + trigger.id);
      logger.warn(
...
function delete_obsolete_triggers(stored, triggers) {
  logger.info('deleting outdated triggers');
  logger.info(
    'iterates over: ' +
      triggers.map(function (item) {
        return item['id'];
