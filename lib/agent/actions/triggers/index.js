...
function run_trigger_actions(trigger) {
  // Update last_exec
  storage2.do('update', {type: 'triggers', id: trigger.id, columns: 'last_exec', values: new Date().getTime() }, () => {
  // storage.update(trigger.id, trigger.name, trigger.persist, trigger.automation_events, trigger.automation_actions, trigger.synced_at, trigger.last_exec, new Date().getTime(), () => {
    send_response('stopped', [trigger.id]);
    // storage.del(trigger.id);
    storage2.do('del', {type: 'triggers', id: trigger.id});
    return;
  })
...
function run_action(trigger, action, last) {
  var timeout = 0;
    ...
    hooks.on(event.split('-')[0], (info) => {
      ...
      event_triggers[event].forEach(action => {
        ...
        if (action.after) {
          ...
          storage2.do('update', {type: 'triggers', id: current.id, columns: 'synced_at', values: exec_time}, () => {
          // storage.update(current.id, current.name, current.persist, current.automation_events, current.automation_actions, current.synced_at, null, exec_time, (err) => {
...
exports.sync = (err, triggers, stored) => {
  ...
  triggers.forEach((trigger, index) => {
    ...
    if (state != 1) {
      ...
      storage2.do('set', {type: 'triggers', id: trigger.id, data: data}, (err) => {
      // storage2.do('set', opts, (err) => {
...
function refresh_triggers(opts, cb) {
  ...
  fetch_triggers((err, res) => {
    if (err) {
      ...
      storage2.do('all', {type: 'triggers'}, (err, stored_triggers) => {
        if (err) return done(err);
        exports.clear_triggers((err) => {
        // storage.clear_triggers((err) => {
          cb && cb(null, emitter);
          exports.sync(err, [], stored_triggers);
        });
      })
      ...
      // storage.get_triggers((err, stored_triggers) => {
        ...
        // storage.clear_triggers((err) => {
          cb && cb(null, emitter);
          exports.sync(null, triggers, stored_triggers);
        });
      })
    }
  });
...
exports.clear_triggers = (cb) => {
  storage2.do('clear', {type: 'triggers'}, (err) => {
