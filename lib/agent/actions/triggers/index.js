    ...
    hooks.on(event.split('-')[0], (info) => {
      ...
      eventTriggers[event].forEach((actionElement) => {
        ...
        if (action.after) {
          ...
          if (currentTriggers[triggerIndex].last_exec) return;
...
const deleteObsoleteTriggers = (stored, triggers) => {
  logger.debug('deleting outdated triggers');
  const triggersMapDebug = triggers.map((item) => item.id);
  logger.debug(`iterates over: ${triggersMapDebug}`);
  const storedMapDebug = stored.map((item) => item.id);
  logger.debug(`compare against stored: ${storedMapDebug}`);

  // eslint-disable-next-line max-len
  const intersectedTriggers = stored.filter((oldTrigger) => !triggers.some((newTrigger) => oldTrigger.id.toString() === newTrigger.id.toString()));
  const intersectedMapDebug = intersectedTriggers.map((item) => item.id);
  logger.debug(`result: ${intersectedMapDebug}`);

  intersectedTriggers.forEach((o) => storage.do('del', { type: 'triggers', id: o.id }));

  return intersectedTriggers;
};

const deleteAllTriggers = (id, cb) => {
...
const assignActiveTriggers = (stored, triggers) => {
  logger.debug('assigning active triggers');
  const triggersMap = triggers.map((item) => item.id);
  logger.debug(`iterates over: ${triggersMap}`);
  const storedMap = stored.map((item) => item.id);
  logger.debug(`compare against stored: ${storedMap}`);

  // eslint-disable-next-line max-len
  const intersectedTriggers = triggers.filter((newTrigger) => !stored.some((oldTrigger) => oldTrigger.id.toString() === newTrigger.id.toString()));

  logger.debug(`result: ${intersectedTriggers.map((item) => item.id)}`);

  return intersectedTriggers;
};

exports.sync = (id, err, triggersSync, storedSync, cb) => {
  let triggers = triggersSync;
  let stored = storedSync;
  const watching = [];
  ...
  if (triggers.length === 0) {
    deleteAllTriggers(id, cb); // delete all triggers in local db
  }
  const triggersToWatch = [...triggers];
  const triggersStoredToWatch = [...stored];
  currentTriggers = assignActiveTriggers(stored, triggers); // only active triggers
  // eslint-disable-next-line max-len
  stored = deleteObsoleteTriggers(stored, triggers); // will remove stored triggers that were deleted in control panel

  logger.debug(`current triggers: ${JSON.stringify(currentTriggers)}`);
  logger.debug(`stored triggers: ${JSON.stringify(stored)}`);
  ...
  triggersToWatch.forEach((triggerToWatch, index) => {
    ...
    try {
      if (
        trigger.automation_events.some((obj) => obj.type === 'exact_time' || obj.type === 'repeat_time')
      ) {
    ...
    if (state !== 1 && state !== 6) {
      logger.warn(`Unable to set up trigger "${trigger.name}": ${errorStatusList[state]}`);
...
const handleTriggersSuccesfully = (success, id, cb, triggers = null) => {
  ...
  storage.do('all', { type: 'triggers' }, (error, storedTriggers) => {
    ...
    if (success) {
      exports.sync(id, null, triggers, storedTriggers, () => {
        if (cb && typeof (cb) === 'function') cb(null, emitter);
      });
    } else {
      exports.clear_triggers((err) => {
        exports.sync(id, err, [], storedTriggers, () => {
          if (cb && typeof (cb) === 'function') cb(null, emitter);
        });
      });
    }
