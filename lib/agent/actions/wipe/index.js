      ...
      if (typeof child === 'function') { // only for windows
        ...
        wipe.fetch_dirs(items, directories, cloud, ['-token', token], (_err, dirs) => {
          compareWinsvcVersion((newer) => {
            ...
            child('wipe', optsData, (errChild, output) => {
              logger.info('=======================================');
              if (output && output !== '') {
                try {
                  logger.info('xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx');
                  const arrayOfLines = output.split('\n');
                  const removedFiles = [];
                  const notoverwrriten = [];
                  arrayOfLines.forEach((line) => {
                    if (line.startsWith('Removing file ')) { removedFiles.push(line.replace('Removing file ', '')); } else { notoverwrriten.push(line.replace("couldn't open file", '')); }
                  });
                  const jsonWiped = {
                    removedFiles,
                    notoverwrriten,
                  ...
                  logger.info(jsonWiped);
                  logger.info(JSON.stringify(jsonWiped));
                  logger.info('ooooooooooooooooooooooooooooooooooooooo');
                  logger.info('xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx');
                } catch (errrexx) {
                  logger.info(errrexx);
