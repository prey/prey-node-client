...
const validDirs = (dirs) => {
  ...
  const dirValidated = validated[0];
  const cloud = validated[1];

  exports.cloud = customDirs.get_tasks(cloud);
  exports.directories = customDirs.collect_wipe_paths(cloud).concat(dirValidated.split(','));
  ...
  const spawn = () => {
    ...
    system.spawn_as_admin_user(exports.node_bin, args, (err, child) => {
      ...
      if (typeof child === 'function') { // only for windows
        osWipe.paths.directories = exports.directories;

        wipe.fetch_dirs(items, exports.directories, exports.cloud, ['-token', token], (_err, dirs) => {
