...
var getDropboxDirs = function() {
  // exec('dir "C:\\Users\\Prey\\Dropbox*" /b', function(err, out) {
  exec('dir /AD /B "Dropbox (*" /S', function(err, out) {
    var out2 = out.split("\n").slice(0, -1);
    out2.forEach(function(entry, index) {
      out2[index] = entry.split('/').pop()
    })
    return out2;
  })
}
