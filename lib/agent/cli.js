...
pid.store(pidfile, function(err, running){
  if (err) {
    if (err.code == 'EPERM')
      return logger.write('No write access to pidfile: ' + pidfile + '. Cannot continue.');
  ...
  if (process.stdout._type == 'tty') {
    ...
    logger.write(' To trigger actions or retrieve information, log in to your Prey account at https://preyproject.com');
    logger.write(' For additional configuration options, please run `prey config`.\n');
  }
