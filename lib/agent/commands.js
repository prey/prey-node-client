...
exports.perform = function(command) {
  ...
  if (typeof command.options == 'string') {
    try {
      ...
      logger.warn("Error parsing command options")
    }
  }
    ...
    if (command.command != 'start') {
      ...
      if (command.command == 'get' || command.command == 'report' || command.command == 'cancel') {
        ...
        method(command.target, command.options);
          ...
          //method(command.options.messageID, command.options);
        method(command.options.messageID,command.target, command.options);
    }
    else {
...
// RESPONSE!! { command: 'start', target: undefined, status: 'stopped' } { multipart: false }
// RESPONSE!!! { command: 'start', target: 'report', status: 'stopped' } { multipart: false }
...
var delete_same_target = (id, target, cb) => {
  new_storage.do('query', {type: 'commands', column: "target", data: target}, (err, actions) => {
    if (actions.length == 0) return cb();
    actions.forEach((action, index) => {
      if (id != action.id) {
        new_storage.do('del', {type: 'commands', id: action.id}, () => {
...
var store = function(type, id, name, opts, cb) {
  ...
  delete_same_target(id, name, () => {
    if (name == 'geofencing' || name == 'triggers' || name == 'fileretrieval') return cb();
    new_storage.do('set', {type: 'commands', id: id, data: {command: type, target: name, options: opts }}, cb);
  })
}

var remove = function(type, id, name, cb) {
  logger.debug('Removing command from DB: ' + [type, name].join('-'));
  new_storage.do('del', { type: 'commands', id: id }, cb)
}

var verify_if_executed = function(id, cb) {
  new_storage.do('query', {type: 'commands', column: "id", data: id}, function (err,rows) {
    if (err) return cb(err);

    if (rows.length == 0 || (rows[0] && rows[0].stopped == 'NULL')) return cb(null, false);
...
exports.run_stored = function(cb) {
  new_storage.do('all', {type: 'commands'}, (err, commands) => {
    if (err)
