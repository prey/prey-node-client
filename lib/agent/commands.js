...
const actionsNotAllowed = ['user_activated', 'geofencing'];
/// /////////////////////////////////////////////////////////////////
// helpers
...
const handleError = function (err) {
  const errorObj = { ...err, level: 'warn' };
  hooks.trigger('error', errorObj);
};
/// /////////////////////////////////////////////////////////////////
...
exports.perform = function (command) {
  ...
  const encryptionTargets = ['encryption_status', 'encryption_keys'];
  if (encryptionTargets.includes(command.target) && config.getData('control-panel.send_encryption_keys') === false) {
    logger.info(`Encryption disabled. Blocking: ${command.command} ${command.target}`);
    return;
  }
  ...
  if (method && !actionsNotAllowed.find((x) => x == command.target)) {
    ...
    const error = new Error(`Unknown command: ${command.command}${command.target ? ` ${command.target}` : ''}`);
    handleError(error);
  }
};
...
const store = (type, idAction, name, opts, cb) => {
  logger.debug(`Storing command in DB: ${[type, name].join('-')}`);

  // eslint-disable-next-line consistent-return
  delete_same_target(idAction, name, () => {
...
exports.run_stored = function (cb) {
  storage.do('all', { type: 'commands' }, (err, commands) => {
    ...
    for (const id in commands) {
      // Delete any previous stored geofencing action
      if (commands[id].target == 'geofencing') {
        logger.info(`Deleting stored geofencing action with id: ${commands[id].id}`);
        storage.do('del', { type: 'commands', id: commands[id].id });
        continue;
      }
