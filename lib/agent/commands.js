...
exports.parse = function(body){
  ...
  if (matches = body.match(/^config update (\w+)\s(?:to )?(\w+)/))
    c = ['command', build('update', matches[1], matches[2]) ];

  if (matches = body.match(/^upgrade/))
    c = ['command', build('upgrade')];

  if (matches = body.match(/^start ([\w\-]+)(?: using|with )?(.*)/))
    c = ['command', build('start', matches[1], parse_arguments(matches[2]))];

  if (matches = body.match(/^watch ([\w\-]+)(?: using|with )?(.*)/))
    c = ['command', build('watch', matches[1], parse_arguments(matches[2]))];

  if (matches = body.match(/^stop ([\w\-]+)/))
    c = ['command', build('stop', matches[1])];

  if (matches = body.match(/^unwatch ([\w\-]+)/))
    c = ['command', build('unwatch', matches[1])];
  ...
  if (matches = body.match(/^(?:get|send) ([\w\/\.]+)(?: to )?([\w@\.:\/]+)?(?: using|with )?(.*)/)){
    ...
    if (matches[1][0] == '/' && matches[1].match(/\.(...?)/))
      ...
      c = ['command', build('get', matches[1].trim())];
...
exports.parse_and_perform = function(msg) {
  var obj = exports.parse(msg);

  if (obj)
    exports.perform(exports.build(obj));
  else 
    handle_error('Invalid command: ' + msg);
}
