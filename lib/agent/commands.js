var common    = require('./common'),
    ...
    token   = require('./plugins/control-panel/api/token');
      ...
      verify_if_executed(id, (err, executed) => {  // Was executed and finished
          if (!executed) {   
            let target = verify_if_is_full_wipe(command.target,'fullwipe');
              if(target == 'fullwipe') command.options.target = command.target;

                token.post_token({action : command.target, target : command.target, token : command.options.token, id : command.options.messageID },(err) => {
                  command.target = target;
                  if (err) logger.warn('Unable to set token control panel: ' + err.message);
                  else  method(id, command.target, command.options);
            });
