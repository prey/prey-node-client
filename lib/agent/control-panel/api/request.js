...
exports.send = (attempt, method, path, data, options, cb) => {
  ...
  const opts = options || {};
  opts.timeout = opts.timeout || exports.defaults.timeout;
  opts.user_agent = opts.user_agent || exports.defaults.user_agent;
  ...
  const base = `${exports.defaults.protocol}://${exports.defaults.host}`;
  ...
  if (cb) {
    exports.defaults.client.request(method, url, data, opts, (err, resp, body) => {
      ...
      if (is_temporary_error(err, resp)) {
        if (attempt < max_attempts) {
          ...
          }, exports.defaults.retry_timeout);
        }
        if (opts.proxy) {
          return retry_without_proxy();
        }
    ...
    return exports.defaults.client.request(method, url, data, opts);
  }
};
...
const set_proxy_and_send = (method, path, data, opts, cb) => {
  if (opts && typeof (opts) === 'object' && !opts.hasOwnProperty('proxy') && exports.defaults.try_proxy) {
    opts.proxy = exports.defaults.try_proxy;
    logger.debug(`Setting proxy to ${opts.proxy}`);
  }
  if (opts && typeof (opts) === 'object' && Object.prototype.hasOwnProperty.call(opts, 'headers')
    && Object.prototype.hasOwnProperty.call(opts.headers, 'X-Prey-Status')) {
    try {
      const xPreyStatus = JSON.parse(opts.headers['X-Prey-Status']);
      verifyContent(xPreyStatus);
      opts.headers['X-Prey-Status'] = JSON.stringify(xPreyStatus);
    } catch (e) {
      logger.debug(`Error verifying X-Prey-Status: ${e}`);
    }
  }
