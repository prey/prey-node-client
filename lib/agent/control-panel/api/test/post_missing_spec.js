var sinon   = require('sinon'),
    should  = require('should'),
    api     = require('..'),
    api_req = require('../request');

var post_missing  = api.devices.post_missing, // shortcut
    stub;

var stub_post = (body, code) => {
  return sinon.stub(api_req, 'post').callsFake((path, data, cb) => {
    var resp = { statusCode: (code || 200), body: body };
    cb(null, resp, body);
  })
}
describe('post missing', () => {
  
  describe('and api key or device key is not set', () => {
    
    before(() => {
      api.keys.unset('api');
      api.keys.unset('device');
    })

    after(() => {
      api.keys.set({ api: 'xxxxxxxxx', device: 'foobar' })
    })

    it('callsback an error', () => {
      post_missing(null, (err) => {
        should.exist(err);
        err.message.should.equal('Both API and Device keys are needed.');
      })
    })

  })

  describe('and the status code is 200', () => {

    before(() => {
      var body = null;
      stub = stub_post(body, 200);
    })

    after(() => {
      stub.restore();
    })

    it('doesnt fail', () => {
      post_missing(true, (err, resp) => {
        should.not.exist(err);
      })
    })
  })

  describe('and the status code is 201', () => {
    
    before(() => {
      var body = null;
      stub = stub_post(body, 201);
    })

    after(() => {
      stub.restore();
    })

    it('callbacks true', () => {
      post_missing(true, (err, resp) => {
        should.exist(err);
        err.message.should.equal('Missing state already set.');
      })
    })
  })

  describe('and the status code is 401', () => {

    before(() => {
      var body = null;
      stub = stub_post(body, 401);
    })

    after(() => {
      stub.restore();
    })

    it('callbacks true', () => {
      post_missing(false, (err, resp) => {
        err.message.should.equal('Invalid credentials.');
      })
    })
  })

  describe('and option is null', () => {

    it('callbacks true', () => {
      post_missing(null, (err, resp) => {
        err.message.should.equal('Invalid option for missing action');
      })
    })
  })
});