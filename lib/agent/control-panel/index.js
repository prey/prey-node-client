...
const init_api = (opts, cb) => {
  ...
  logger.info(`data init_api: ${JSON.stringify(data)}`);
  api.use({
    host: opts['control-panel.host'],
    protocol: opts['control-panel.protocol'],
    try_proxy: opts.try_proxy,
...
const wait_for_config = (cb) => {
  ...
  const timer = setInterval(() => {
    ...
    if (config.getData('control-panel.api_key') && config.getData('control-panel.device_key')) {
      ...
      logger.info(`data wait_for_config to disabled: ${JSON.stringify(data)}`);
...
exports.enabled = (cb) => {
  const data = config.all();
  logger.info(`data enabled to disabled: ${JSON.stringify(data)}`);
  init_api(data);
...
exports.disabled = (cb) => {
  const data = config.all();
  logger.info(`data load to disabled: ${JSON.stringify(data)}`);
  init_api(data, (err) => {
    if (err) return cb();
...
exports.load = (cb) => {
  ...
  const data = config.all();
  logger.info(`data load to init_api: ${JSON.stringify(data)}`);
  const initOpts = data;
...
exports.load_api = (opts, cb) => {
  logger.info(`data load_api to disabled: ${opts}`);
  init_api(opts, cb);
  return api;
};
