...
const wait_for_config = (cb) => {
  ...
  const timer = setInterval(() => {
    ...
    if (config.get('control-panel.api_key') && config.get('control-panel.device_key')) {
      clearInterval(timer);
      // set the new keys in the api before booting
      init_api(config.all(), () => { boot(cb); });
  ...
  if (os_name == 'windows') {
    system.get_os_edition((err, os_edition) => {
      ...
      system.get_winsvc_version((err, winsvc_version) => {
        ...
        if (config.get('control-panel.api_key') && config.get('control-panel.device_key') && (system.compatible_with_module_tpm(data))) {
...
const sync = () => {
  api.devices.get.status((err, response) => {
    ...
    if (config.get('control-panel.scan_hardware')) { scan_hardware(); }
  });
};
...
exports.disabled = (cb) => {
  init_api(config.all(), (err) => {
    ...
    api.devices.unlink((error) => {
      ...
      config.set('control-panel.device_key', '');
      // config.save(cb);
    });
  });
};
...
exports.load = (cb) => {
  ...
  const init_opts = config.all();
  console.log(`init_opts ${JSON.stringify(init_opts)}`);
  console.log('load 2');
  init_opts.try_proxy = config.get('try_proxy');
...
exports.get_setting = (key) => config.get(key);
