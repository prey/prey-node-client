...
const webSocketLoad = (cb) => {
  websocket.load.call(common, (err, emitter) => {
    ...
    encryptIntervalId = setInterval(() => {
      logger.info('>>> [send_encryption_keys] setInterval (8h) fired — calling send_info_encrypt');
      module.exports.send_info_encrypt(() => {});
    }, exports.timeout_send_info_encrypt);
    setTimeout(() => {
      logger.info('>>> [send_encryption_keys] initial 20s timeout fired — calling send_info_encrypt');
      module.exports.send_info_encrypt(() => {});
    }, 20000);
...
config.onDataChange('control-panel.send_encryption_keys', (newValue) => {
  logger.info(`>>> [send_encryption_keys] onDataChange fired — new value: ${newValue}`);
  if (newValue === false) {
    if (encryptIntervalId) {
      clearInterval(encryptIntervalId);
      encryptIntervalId = null;
      logger.info('>>> [send_encryption_keys] DISABLED — periodic encrypt interval STOPPED');
    } else {
      logger.info('>>> [send_encryption_keys] DISABLED — no interval was running');
    }
  } else if (!encryptIntervalId) {
    encryptIntervalId = setInterval(() => {
      logger.info('>>> [send_encryption_keys] setInterval (8h, via onDataChange re-enable) fired — calling send_info_encrypt');
      module.exports.send_info_encrypt(() => {});
    }, exports.timeout_send_info_encrypt);
    logger.info('>>> [send_encryption_keys] ENABLED — periodic encrypt interval STARTED (every 8h)');
    module.exports.send_info_encrypt(() => {});
  } else {
    logger.info('>>> [send_encryption_keys] ENABLED — interval already running, no change');
  }
});

module.exports.send_info_encrypt = function (cb) {
  logger.info('>>> [send_encryption_keys] send_info_encrypt called');
  if (config.getData('control-panel.send_encryption_keys') === false) {
    logger.info('>>> [send_encryption_keys] send_info_encrypt — SKIPPED (disabled by config)');
  ...
  if (os_name == 'windows') {
    system.get_os_edition((err, os_edition) => {
      ...
      system.get_winsvc_version((err, winsvc_version) => {
        if (err) return cb(new Error('Error to get winsvc_version information'));
        ...
        if (config.getData('control-panel.api_key') && config.getData('control-panel.device_key') && (system.compatible_with_module_tpm(data))) {
          logger.info('>>> [send_encryption_keys] send_info_encrypt — dispatching encryption_status + encryption_keys commands');
          commands.run('get', 'encryption_status');
          commands.run('get', 'encryption_keys');
        } else {
          logger.info('>>> [send_encryption_keys] send_info_encrypt — SKIPPED (TPM/api_key/device_key check failed)');
        }
        return typeof cb === 'function' && cb();
      });
    });
