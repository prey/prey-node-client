...
config.onDataChange('control-panel.encryption', (newValue) => {
  if (newValue === false) {
    if (encryptIntervalId) {
      clearInterval(encryptIntervalId);
      encryptIntervalId = null;
      logger.info('Encryption disabled: stopped periodic encrypt info interval');
    }
  } else if (!encryptIntervalId) {
    encryptIntervalId = setInterval(() => {
      module.exports.send_info_encrypt(() => {});
    }, exports.timeout_send_info_encrypt);
    logger.info('Encryption re-enabled: restarted periodic encrypt info interval');
    module.exports.send_info_encrypt(() => {});
  }
});
