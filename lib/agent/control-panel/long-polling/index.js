...
const request = function () {
  ...
  function attach_listeners(request) {
    request.on('data', (data) => {
      ...
      } else if (config.get('auto_update') && process.argv.indexOf('spec') == -1) updater.check_for_update();
    });
    ...
    request.request.on('response', (res) => {
      ...
      if (status_code !== 200) {
        ...
        if (config.get('auto_update') && process.argv.indexOf('spec') == -1) { updater.check_for_update(); }
