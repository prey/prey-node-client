...
var notify_linked = function (hardware) {
  if (!common.helpers.runningOnBackground()) return;
  var data = {
    message: {
      status: 'ok',
...
exports.open_config = function (device_key, cb) {
  setTimeout(function () {
    ...
    exports.generate_keys(function (err) {
      ...
      else if (osName == 'mac') return run_as_user('open', [link], cb);
      ...
      system.get_logged_user((error, logged_user) => {
        ...
        exec(
          `sudo -u ${logged_user} ${common.rootPath}/lib/agent/utils/openwebbrowser.sh ${link} &`,
          {
            timeout: 200,
          },
          (err, stderr) => {
