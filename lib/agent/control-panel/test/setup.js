...
describe('setup()', function() {
  ...
  config.global = {
    get: function(key) { return config.getData('global_' + key) },
    set: function(key, val) { return config.setData('global_' + key, val) }
  }

  // insert default values to config
  config.setData('host', 'destination.com');
  config.setData('protocol', 'https');
  ...
  describe('with empty keys', function() {
    ...
    describe('and config.global exists', function() {
      ...
      describe('and no keys either', function() {
        ...
        before(function() {
          config.setData('global_api_key', '');
          config.setData('global_device_key', '');

          config.getData('global_api_key').should.eql('');
        })
      ...
      describe('and we DO find something in there', function() {
        ...
        before(function() {
          config.setData('global_api_key', 'globalmegakey');
          config.setData('global_device_key', '97979797');
          config.getData('global_api_key').should.eql('globalmegakey');

          // stop the execution at the next stage (api.keys.set)
          // as we're only interested in this part right now
          stop = sinon.stub(api.keys, 'set').callsFake((keys, cb) => {
  ...
  describe('with API but no Device key', function() {
    ...
    before(function() {
      config.setData('api_key', 'foobar123456');
      config.setData('device_key', '');
    })
    ...
    describe('if request fails', function() {
      ...
      before(function() {
        api_link_stub = sinon.stub(api.devices, 'link').callsFake((opts, cb) => {
  ...
  describe('with both keys', function() {
    ...
    describe('and both keys are valid', function() {
      ...
      before(function() {
        config.setData('api_key', 'valid_api_key');
        config.setData('device_key', 'valid_device_key');
      })
