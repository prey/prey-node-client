...
const completationMaxWaitMs = 60 * 1000;
...
const agruparPorEstructuraAnidada = (arrayDeObjetos) => {
  const grupos = {};

  const obtenerFirmaEstructura = (obj) => {
  ...
  arrayDeObjetos.forEach((objeto) => {
    const firma = obtenerFirmaEstructura(objeto);
    if (!grupos[firma]) {
      grupos[firma] = [];
    }
    grupos[firma].push(objeto);
  });
...
/**
 * Función async para verificar si la tarea anterior ha completado su procesamiento.
 * Esta función debe ser implementada para incluir lógica específica de negocio.
 *
 * @param {Object} currentItem - El elemento actual siendo procesado
 * @param {Object} previousItem - El elemento anterior procesado (null si es el primero)
 * @returns {Promise<boolean>} - true si la tarea anterior completó y se puede procesar la siguiente
 */
// NOTE: completion is signaled via hooks.once('command_processed_or_stored', id)

/**
 * Marca un item como siendo procesado
 * @param {Object} item - El item siendo procesado
 */
const markItemAsProcessing = (item) => {
  ...
  currentlyProcessingStructure = Array.isArray(item) ? '__array__' : agruparPorEstructuraAnidada(item);
};

/**
 * Marca un item como completado
 * @param {Object} item - El item que completó
 */
  ...
  try {
    ...
    if (previous && previous.id && sameStructure) {
      ...
      if (!pendingCompletionListeners.has(prevId)) {
        ...
        timer = setTimeout(() => {
          try {
            logger.info(`Timeout waiting ${completationMaxWaitMs}ms for completion of ${prevId}; continuing.`);
  ...
  statusTrigger.get_status((_err, status) => {
    ...
    ws.on('open', () => {
      ...
      if (messageQueueStructureOrder.length > 0) {
        workNotifier.emit('work', {
 reason: 'startup', ts: Date.now(), forceImmediate: true, resetBackoff: true,
});
      }
    ...
    ws.on('message', (data) => {
      ...
      try {
        if (Array.isArray(parsedData)) {
          ...
          const grouped = agruparPorEstructuraAnidada(parsedData);
          ...
          const groupedSingle = agruparPorEstructuraAnidada([parsedData]);
