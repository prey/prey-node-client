...
const updateStoredConnection = (newStoredTime) => {
  storage.do('update', {
    ...
    logger.info('UpdatedStored referential first connection time');
  });
};
...
const setLastConnection = () => {
  storage.do('query', { type: 'keys', column: 'id', data: 'last_connection' }, (err, stored) => {
    ...
    if (stored && stored.length > 0) {
      ...
      logger.info(`last stored ${JSON.stringify(lastStored)}`);
    ...
    ws.on('open', () => {
      pingInterval = setInterval(() => {
        ...
        ws.ping(null, (errPing) => {
          ...
          setTimeout(() => {
            logger.info('[][][]CHECKING PONG');
            if (!pongReceived) {
              logger.info('[][][]PONG not received - RESTARTING WEBSOCKET');
              restartWebsocketCall();
            }
          }, 5000);
        });
...
exports.startWebsocket = () => {
  clearAndResetIntervals();
  logger.info('im right here dude');
  webSocketSettings();
};
...
exports.load = (cb) => {
  ...
  loadServer();
  loadHooks();
  logger.info('soy el terrible device websocket whatever');
  exports.startWebsocket();
