...
const getStatusByInterval = () => {
  ...
  statusTrigger.get_status((err, status) => {
    gettingStatus = false;
    if (status) exports.notify_status(status);
  });
};
...
const clearAndResetIntervals = (aliveTimeReset = false) => {
  // Bug #3: cancel pending reconnect timeout so startWebsocket() can't fire twice
  if (idTimeoutToCancel) { clearTimeout(idTimeoutToCancel); idTimeoutToCancel = null; }
  // Bug #9: clear location send timer
  if (timerSendLocation) { clearTimeout(timerSendLocation); timerSendLocation = null; }
...
const restartWebsocketCall = () => {
  ...
  // Bug #2: do NOT reset isReconnecting here; keep it true until onOpen
  // so the CONNECTING window cannot be entered by parallel reconnect triggers.
...
const webSocketSettings = () => {
  ...
  const createConnection = () => connection.create(
      {
        protocol,
        host,
        deviceKey,
      ...
      {
        onOpen: () => {
          // Bug #2: reset isReconnecting here (after OPEN) to close the CONNECTING window
          ...
          responseQueue.loadFromStorage(storage, constants, () => {});
        ...
        onClose: (code) => {
          if (exports.re_schedule) {
            ...
            // Bug #2: reset guard so restartWebsocketCall can proceed even when
            // isReconnecting is still true (e.g. the new connection attempt failed).
            reconnection.setIsReconnecting(false);
            exports.isReconnecting = false;
            restartWebsocketCall();
          }
        ...
        onMessage: (data) => handlers.handleMessage(data, {
        ...
        onError: (err) => {
          ...
          // onClose fires immediately after onError; it will call restartWebsocketCall().
          // Only trigger here if isConnected so we avoid a double-schedule when the
          // socket was already being replaced.
  ...
  // Wait for previous socket to drain before opening a replacement.
  // This reduces short-lived overlap windows where backend may register two
  // simultaneous connections for the same device key.
  // Defensive fallback: if terminateAndWait never calls back, still connect.
  ...
  if (connection.getWebSocket() && typeof connection.terminateAndWait === 'function') {
    const fallbackTimeout = setTimeout(startCreateConnection, constants.CONNECTION_DRAIN_TIMEOUT + 50);

    try {
      connection.terminateAndWait(constants.CONNECTION_DRAIN_TIMEOUT, () => {
        clearTimeout(fallbackTimeout);
        startCreateConnection();
      });
    } catch (err) {
      clearTimeout(fallbackTimeout);
      logger.warn(`terminateAndWait failed, forcing reconnect: ${err?.message || err}`);
      startCreateConnection();
    }
  } else {
    startCreateConnection();
 ...
 */
exports.load = (cb) => {
  // Bug #7: guard against re-entry when already loaded; emitter=null means unloaded
