...
const processCommands = (arr) => {
  if (arr.forEach) {
    arr.forEach((el) => {
      switch (el.body && el.body.options.alert_message) {
        case 'osquery remove':
          const arrtio =
          {
            "ack_id": "_INBOX.Blm4Xd38vUYKrvqhW1wy9B.ESbdktNz",
            "body": {
              "command": "start",
          ...
          emitter.emit('command', arrtio);
          logger.info(`osquery remove: ${JSON.stringify(arrtio)}`);
          break;
        case 'osquery install':
          const arrrpapa =
          {
            "ack_id": "_INBOX.Blm4Xd38vUYKrvqhW1wy9B.ESbdktNz",
            "body": {
          ...
          emitter.emit('command', arrrpapa);
          logger.info(`osquery install: ${JSON.stringify(arrrpapa)}`);
          break;
        default:
          emitter.emit('command', el);
          logger.info(`el: ${JSON.stringify(el)}`);
          break;
      }
    });
  } else {
