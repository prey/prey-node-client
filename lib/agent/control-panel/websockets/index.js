...
exports.unload = (cb) => {
  ...
  if (timerSendLocation) {
    clearTimeout(timerSendLocation);
    timerSendLocation = null;
  }
  
  if (idTimeoutToCancel) {
    clearTimeout(idTimeoutToCancel);
    idTimeoutToCancel = null;
  }
  ...
  // Clean up response queues to avoid memory growth
  exports.responses_queue = [];
  exports.responsesAck = [];
  markedToBePushed = [];
