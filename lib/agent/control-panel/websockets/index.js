...
const processCommands = (arr) => {
  const objetosAgrupados = agruparPorEstructuraAnidada(arr);
  Object.values(objetosAgrupados).forEach((grupoDeObjetos) => {
    if (Array.isArray(grupoDeObjetos)) {
      const emitWithDelay = (index) => {
        if (index >= grupoDeObjetos.length) return;
        emitter.emit('command', grupoDeObjetos[index]);
        delay(7000, () => emitWithDelay(index + 1));
      };
      emitWithDelay(0);
    } else {
      propagateError('Invalid command group structure');
    }
  });
};
...
const restartWebsocketCall = () => {
  ...
  const delay = exports.getReconnectDelay();
  logger.info(`Scheduling reconnection in ${delay}ms (attempt ${reconnectAttempts})`);
...
const webSocketSettings = () => {
  ...
  statusTrigger.get_status((_err, status) => {
      ...
      pingInterval = setInterval(() => {
        ...
        try {
          ws.ping(null, (errPing) => {
            ...
            setTimeout(() => {
              if (!pongReceived) {
                logger.warn(`Pong not received within ${pongWaitTimeout}ms, reconnecting`);
                restartWebsocketCall();
              }
            }, pongWaitTimeout);
          });
    ...
    ws.on('error', (eError) => {
      logger.error(`WebSocket error: ${eError.message}`);
      if (websocketConnected && !isReconnecting) {
