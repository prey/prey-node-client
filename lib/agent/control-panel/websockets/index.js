...
const processOneMessage = async () => {
  if (processingQueue) {
    logger.debug('processOneMessage: already processing, skipping');
    return false;
  }
  ...
  if (availableStructures.length === 0) {
    logger.debug('processOneMessage: no available structures to process');
    return false;
  }
  ...
  logger.debug(`processOneMessage: selected structure ${targetStructure}`);
  ...
  try {
    ...
    if (previous && previous.id && sameStructure) {
      ...
      logger.debug(`processOneMessage: previous item ${prevId} has same structure (${targetStructure}), waiting for completion`);
      ...
      if (!pendingCompletionListeners.has(prevId)) {
        ...
        const onCompleted = (idItem) => {
          try {
            ...
            logger.debug(`completion event received for ${prevId}`);
        ...
        try {
          ...
          logger.debug(`registered hooks.once for completion of ${prevId}`);
        } catch (e) {
        ...
        timer = setTimeout(() => {
          try {
            logger.debug(`Timeout waiting ${completionMaxWaitMs}ms for completion of ${prevId}; continuing.`);
            ...
            pendingCompletionListeners.delete(prevId);
            logger.denig(`completion timeout for ${prevId}`);
      ...
      logger.debug(`processOneMessage: requeued item for structure ${targetStructure} because previous ${prevId} is processing`);
      processingQueue = false;
      return false;
    }

    // Mark this item as currently processing
    logger.debug(`processOneMessage: marking item as processing id=${item && item.id ? item.id : '[array]'} structure=${targetStructure}`);
    markItemAsProcessing(item);

    // Log if we're processing in parallel (different structure from previous)
    if (previous && previous.id && !sameStructure) {
      logger.debug(`processOneMessage: previous item ${previous.id} has different structure, processing in parallel`);
    }
...
async function runOnce() {
  ...
  logger.debug('runOnce: waiting for work');
  // Wait until there's work (returns immediately if already queued)
  const meta = await waitForWork();
  logger.debug(`runOnce: woke with meta ${meta ? JSON.stringify(meta) : 'null'}`);
  ...
  try {
    ...
    if (!workerRunning) return;
    if (processed) {
      logger.debug('runOnce: processed one item, continuing immediately');
      // If processed, schedule next run immediately to keep consuming
      backoffAttempts = 0;
      setImmediate(runOnce);
    } else {
      logger.debug('runOnce: no progress, backing off');
      ...
      logger.debug(`runOnce: scheduling retry in ${delay}ms (attempt ${backoffAttempts})`);
...
const startWorker = () => {
  if (workerRunning) return;
  workerRunning = true;
  logger.debug('startWorker: worker started');
  setImmediate(runOnce);
};

const stopWorker = () => {
  workerRunning = false;
  logger.debug('stopWorker: worker stopped');
...
const scheduleReconnect = (reason, immediate = false) => {
  ...
  // Stop worker to prevent processing messages while disconnected
  stopWorker();
...
const webSocketSettings = () => {
  ...
  statusTrigger.get_status((_err, status) => {
    ...
    ws.on('open', () => {
      ...
      // Start worker to process incoming messages
      startWorker();
    ...
    ws.on('message', (data) => {
      ...
      try {
        ...
        if (Object.keys(parsedData).length) logger.debug(`message received from backend: ${JSON.stringify(parsedData)}`);
      } catch (e) {
        return propagateError('Invalid command object');
      }
      logger.debug('ws.on.message: enqueuing parsedData');
      ...
      try {
        ...
        logger.debug('ws.on.message: notifying worker about new work');
        workNotifier.emit('work');
        logger.debug('ws.on.message: worker notified');
      } catch (e) {
