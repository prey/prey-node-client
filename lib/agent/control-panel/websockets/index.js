...
const webSocketSettings = () => {
  ...
  statusTrigger.get_status((_err, status) => {
    ...
    ws.on('open', () => {
      ...
      // Start the message processor: dequeue and process one message every 7 seconds
      processorInterval = setInterval(processOneMessage, 7000);
    ...
    ws.on('message', (data) => {
      ...
      // Enqueue the message (array or single object)
      // The independent processor (processorInterval) will handle it
      try {
