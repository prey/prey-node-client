      ...
      if (stored && stored.length > 0) {
        ...
        storedFiltered.forEach((current) => {
          ...
          storage.do(
            ...
            (errUpdate) => {
              if (errUpdate) logger.error('Unable to update the execution time');
            },
          );
        });
      }
  ...
  statusTrigger.get_status((err, status) => {
    ...
    ws.on('pong', () => {
      ...
      network.get_connection_status((statusConnection) => {
        const diferenciaEnMinutos = (newLastConnection.getTime()
        - lastLastConnection.getTime()) / 1000 / 60;
        if (typeof statusConnection === 'string' && diferenciaEnMinutos >= 30) hooks.trigger('disconnected');
      });
