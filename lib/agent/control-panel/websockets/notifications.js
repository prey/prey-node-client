...
exports.notifyStatus = (ws, status, logger) => {
  ...
  logger.debug("Sending device's status information");
  try {
...
exports.notifyAction = (context, params) => {
  ...
  if (!queuedResponse) {
    ...
    // Always retry — server deduplicates by reply_id
    shouldSend = true;
    toSend.sent = false;
  }

  // Send if connection is ready and not sent yet
  if (!shouldSend || !isConnectionReady(ws)) return;
  try {
