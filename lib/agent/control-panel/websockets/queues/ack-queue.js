...
// State
let responsesAck = [];
// Track sent ACK IDs to prevent duplicates (Bug #14: capped to avoid unbounded growth)
const sentAckIds = new Set();
const MAX_SENT_ACK_IDS = 1000;
...
exports.sendAckToServer = (ws, sendToWs, logger) => {
  try {
    ...
    // Check if this ACK was already sent (prevent duplicates)
    if (sentAckIds.has(sendToWs.ack_id)) {
    ...
    // Track that this ACK was sent; clear when cap is reached (Bug #14)
    if (sentAckIds.size >= MAX_SENT_ACK_IDS) sentAckIds.clear();
    sentAckIds.add(sendToWs.ack_id);
