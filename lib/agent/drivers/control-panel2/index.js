...
var ControlPanelDriver = function(options) {
  ...
  this.set_urls = function(){
    var base_url = this.protocol + this.host + '/api/v2/devices/' + this.device_key,
        request_format = '.json';
    ...
    this.endpoints = {
      ...
      data:     base_url + '/data'     + request_format,
  ...
  this.fetch = function(){
    ...
    opts.proxy  = common.config.get('proxy_url');
    ...
    common.system.get_os_info(function(err, info){
      ...
      if (config.get('send_status_info')) {
        hb.before(function(req_opts, next){
          logger.debug('Gathering status info for request...')
          self.append_status_report(req_opts, next);
        })
      }

      hb.on('message', function(msg){
        logger.debug('Response body: ' + msg.toString())
        if (msg == '' || msg == '[]') return;
  ...
  this.process = function(response_body){
    ...
    try {
      ...
      return logger.error('Invalid response: ' + response_body);
    }

    logger.info('Got new commands: ' + commands.toString())
  ...
  this.send_data = function(key, data, callback){
    ...
    var remote = this.endpoints[key] || this.endpoints.data || this.endpoints.reports;
  ...
  this.send_event = function(name, data){
    var data = { event: { name: name, info: data } };
    this.send_data('events', data);
  };
