...
var Agent  = self = {
  ...
  run: function(){
    ...
    var start_wait_time = (!process.env.TERM || process.env.TERM == 'dumb')
        ? helpers.random_between(1, 59) : 0;

    setTimeout(function(){

      system.logged_user(function(err, user_name){
        if (!err && user_name)
          process.env.LOGGED_USER = user_name;
    ...
    if (start_wait_time > 0)
      logger.info('[agent] Sleeping for ' + start_wait_time + ' seconds...');
  },
  ...
  check_for_updates: function(cb){
    this.log('Checking for updates...')

    updater.get_latest_version(function(err, latest_version){
      if (err) {
        self.log_error(err);
        return cb();
      }

		  if (!helpers.is_greater_than(latest_version, common.version))
			  return cb();

      hooks.trigger('event', 'new_version', latest_version);
      helpers.run_detached(system.paths.bin_path, ['config', 'upgrade'])
      self.shutdown();
    });
  },
  ...
  initialize: function(callback){
    ...
    this.check_connection(1, function(connected){
      ...
      self.check_for_updates(callback);
    });
  ...
  check_connection: function(attempt, callback){
    ...
    var no_connection = function(e){
      self.log_error(_error('No connection found after ' + attempt + ' attempts.'));
  ...
  load_driver: function(driver_name, driver_options){
    ...
    if (self.drivers[driver_name])
      return self.log_error(_error(driver_name + " driver already loaded"));
  ...
  check_delay: function(requested_delay){
    ...
    if (this.interactive)
      return self.log_error('Running interactively -- will not update delay.');
  ...
  get_report: function(report_name, options){
    ...
    if (reports.is_active(report_name))
      return self.log_error(_error('Report ' + report_name + ' already requested!'));
    ...
    reports[when](report_name, function(data){
      ...
      if (!data || Object.keys(data).length == 0)
        return self.log_error(_error(report_name  + 'returned no data.'));
