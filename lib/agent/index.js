...
let timerForDataPreyConf;
let timerForDataPreyConfThirty;
...
const watchList = ['connection', 'hostname', 'location', 'network', 'power', 'status'];
let running = false;
let startedAt = null;
let runningAs = null;
...
const setTimeCorrectData = () => {
  if (timerForDataPreyConfThirty) clearInterval(timerForDataPreyConfThirty);
  setTimeout(correctDataTimedOut, 1000 * 60 * 5);
  timerForDataPreyConfThirty = setInterval(correctDataTimedOut, 1000 * 60 * 30);
};
    ...
    if (shouldPreyCFile && shouldPreyCFile.localeCompare('true') === 0) {
      ...
      getDataDb('preyconf', (_errorGetData, dataFromDb) => {
        if (dataFromDb && dataFromDb.length > 0) {
          ...
          timerForDataPreyConf = setInterval(() => {
            getDataFromShouldPreyCFile((errGetShould, dataShould) => {
              ...
              if (shouldPreyCFile && shouldPreyCFile.localeCompare('true') === 0) {
                setTimeCorrectData();
                clearInterval(timerForDataPreyConf);
                return;
              }
    ...
    try {
      storage.do('query', { type: 'keys', column: 'id', data: 'settingDeviceKey' }, (err, stored) => {
        ...
        if (stored && stored.length > 0) {
          ...
          if (!isBoolean(settingDeviceKeyValue.fromCli)
            || (Date.now() - settingDeviceKeyValue.dateTime) > 90000) {
