...
var Agent  = self = {
  ...
  run: function(){
    ...
    setTimeout(function(){
      ...
      system.get_logged_user(function(err, user_name){
        if (!err && user_name)
  ...
  write_header: function(){
    ...
    var info = "  Running over Node " +  process.version + " with PID " + process.pid;
    info += " on a " + common.os_name + " system as " + this.running_as + "\n";
    info += "  Current logged user: " + process.env.LOGGED_USER + "\n";
  ...
  get_report: function(report_name, options){
    ...
    if (reports.is_active(report_name))
      return self.log_error(new Error('Report ' + report_name + ' already requested!'));
  ...
  load_action_hooks: function(){
    ...
    actions.on('action_started', function(action_name, err){
      var success = err ? false : true;
      hooks.trigger('event', action_name + '_started', success);
      if (err) self.log_error(err);
    });
...
//    });
    ...
    actions.on('action_finished', function(action_name, err, data){
      hooks.trigger('event', action_name + '_finished', err || data || true);
      if (err) self.log_error(err);
