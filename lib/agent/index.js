...
var handle_error = function(err, source) {
  ...
  if (err.code == 'EADDRINFO') // no connection
    handle_no_connection();
  else if (config.get('send_crash_reports'))
    exceptions.send(err);
}

var handle_no_connection = function() {
  if (!config.get('auto_connect'))
    return;

  logger.notice('Not connected. Trying to connect...');
  system.auto_connect(function(success){
    logger.notice('Network reconnection ' + (success ? 'successful.' : 'unsuccessful.'));
    if (success) hooks.trigger('woken');
  })
}
