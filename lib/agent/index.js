...
var run = function() {
  ...
  manage_keys(function(err, encoded_public_key) {
    if (!keys.get().api) {
      lp2.load(function() {
        ...
        console.log("ENCODED:", encoded_public_key)
        opn('https://panel.preyhq.com/auth/start_configuration/' + encoded_public_key, {app: 'google chrome'})
  ...
  notifier.notify({
    title: 'Prey',
    subtitle: 'Device succesfully configured!',
    message: 'Click to go to the control panel!',
    icon: path.join(__dirname, '48px.png'), // Absolute path (doesn't work on balloons) 
  ...
  notifier.on('click', function (notifierObject, options) {
    console.log("CLICKED!!!")
    opn('https://google.com/', {app: 'google chrome'});
