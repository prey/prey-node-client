...
var messages = {
  ...
  unknown_response: 'Unknown response. Could not get device key.'
}
...
var link_device = function(api_key, cb){
  ...
  get_device_data(function(err, data){
    ...
    send_request(api_key, data, function(err, device_key){
      ...
      logger.notice(register_success + device_key);
...
var send_request = function(api_key, data, cb) {
  ...
  needle.post(attach_url, data, opts, function(err, resp, body){
    ...
    if (body.device && body.device.key) {
      ...
      cb(new Error(messages.unknown_response));
    }
  });
