...
var verify = function(cb) {
  ...
  if (!api_key || api_key == '') {
    var err = new Error(messages.no_api_key);
    err.code = 'NOAPIKEY';
    return cb(err);
  }
  ...
  linking = true;
  config.writable(function(yes, x){
    if (!yes) throw(new Error('Config not writable. Cannot setup device.'))
...
var link_device = function(api_key, cb){
  ...
  get_device_data(function(err, data){
    ...
    data.name = os.hostname().replace(/\.local$/, ''); // remove tailing '.local'
...
var send_request = function(api_key, data, cb) {
  ...
  var host       = config.get('host'),
      ...
      attach_url = protocol + '://' + host + '/api/v2/devices.json';

  needle.post(attach_url, data, opts, function(err, resp, body){
    if (err) return cb(err);

    if (body && body.key) {
      cb(null, body.key)
    } else if (resp.statusCode == 401) {
      cb(new Error(messages.invalid_credentials))
