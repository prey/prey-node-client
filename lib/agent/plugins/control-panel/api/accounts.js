...
exports.authorize = function(opts, cb) {
  const logger = common.logger.prefix('API_ACCOUNT');
  logger.info("logger authorize in api accounts.js");
  if (!opts || !opts.username || !opts.password)
    throw(errors.arguments('No credentials passed!'));

  logger.info("logger authorize in api accounts.js 1");
  if (keys.get().api)
    throw(errors.get('API_KEY_SET'))

  logger.info("logger authorize in api accounts.js 2");
  request.get('/profile.json?lang=' + lang, opts, function(err, resp) {
    if (err) return cb(err);

    var body = resp.body;

    logger.info(`look at it, el perro panzon... Todo chiquito y todo panzon: ${resp.statusCode}`);
