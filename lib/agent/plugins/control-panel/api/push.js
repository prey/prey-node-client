var keys    = require('./keys'),
    errors  = require('./errors'),
    request = require('./request'),
    common  = require('./../../../common'),
    logger     = common.logger.prefix('push');

var check_keys = function() {
  if (!keys.present())
    throw errors.get('MISSING_KEY');
}

var post = function(url, data, opts, cb) {
  var opts = opts || {};

  if (opts.status) {
    var stats = JSON.stringify(opts.status);
    opts.headers = { 'X-Prey-Status': stats };
    delete opts['status'];
  }
  if (!opts.user_agent)
    opts.user_agent = common.system.user_agent;

  request.post(url, data, opts, cb)
}

exports.response = function(data, opts, cb) {
  check_keys();
  var url = format_url('response');
  logger.info("response data" + JSON.stringify(data));
  logger.info("response url" + JSON.stringify(url));
  logger.info("response opts" + JSON.stringify(opts));
  post(url, data, opts, cb);
}

exports.event = function(data, opts, cb) {
  check_keys();
  var url = format_url('events');
  logger.info("events data" + JSON.stringify(data));
  logger.info("events url" + JSON.stringify(url));
  logger.info("events opts" + JSON.stringify(opts));
  post(url, data, opts, cb);
}

exports.data = function(data, opts, cb) {
  check_keys();
  var url = format_url('data');
  logger.info("data data" + JSON.stringify(data));
  logger.info("data url" + JSON.stringify(url));
  logger.info("data opts" + JSON.stringify(opts));
  post(url, data, opts, cb);
}

exports.report = function(data, opts, cb) {
  check_keys();
  var url = format_url('reports');
  logger.info("report data" + JSON.stringify(data));
  logger.info("report url" + JSON.stringify(url));
  logger.info("report opts" + JSON.stringify(opts));
  post(url, data, opts, cb);
}

function format_url(endpoint) {
  var format  = '.json';
  return '/devices/' + keys.get().device + '/' + endpoint + format;
}
