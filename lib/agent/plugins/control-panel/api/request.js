...
var is_temporary_error = function(err, resp) {
  ...
  if (err)
    retry = (codes.indexOf(err.code) !== -1 || err.message.match('socket hang up'));
  else
...
var send = function(attempt, method, path, data, options, cb) {
  ...
  defaults.client.request(method, url, data, opts, function(err, resp, body) {
    ...
    if (err && is_network_down(err)) {
      ...
      // if no proxy was used but one is set, retry request using it
      if (!opts.proxy && defaults.try_proxy) {

        options.proxy = defaults.try_proxy;
