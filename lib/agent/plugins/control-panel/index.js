...
module.exports.send_info_encrypt = function(cb) {
  ...
  if(os_name == 'windows'){
    ...
    //return cb && cb(new Error('Action only allowed on Windows'));
    return (cb && typeof(cb) == 'function') && cb(new Error('Action only allowed on Windows'));
  }
}
