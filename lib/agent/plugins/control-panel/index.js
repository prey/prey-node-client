...
var get_options = function(config_obj) {
  var opts = config_obj.all();

  if (config_obj.global && config_obj.global.get('try_proxy'))
    opts.try_proxy = config_obj.global.get('try_proxy');
...
var wait_for_config = function() {
  ...
  var timer = setInterval(function() {
    ...
    if (config.get('api_key') && config.get('device_key')) {
      ...
      init_api(get_options(config), function() {
        boot();
      })
...
function update_settings(obj) {
  ...
  function process(values, target) {
    for (var key in values) {
      ...
      if (typeof val != 'undefined' && target.get(key) !== val) {
        logger.notice('Updating value of ' + key + ' to ' + val);
        target.set(key, val);
      }
    }
    target.save();
...
function scan_hardware() {
  var cmd = commands.build('get', 'specs');
  commands.perform(cmd);
}
...
exports.setup = function(cb) {
  ...
  init_api(get_options(this.config));
...
exports.disabled = function(cb) {
  ...
  init_api(get_options(this.config), function(err) {
...
exports.load = function(cb) {
  ...
  init_api(get_options(common.config));
  sender.init(common);
