...
function handle_response(what, resp) {
  if (what == 'reports' && resp.statusCode > 300)
    ...
    agent.commands.process(resp.body);
}
...
function missing(interval) {
  ...
  agent.commands.perform(agent.commands.build('report', 'missing', { interval: interval }));
}

function recovered() {
  agent.commands.perform(agent.commands.build('cancel', 'missing'))
}
