...
function update_stored_connection (new_stored_time) {
  device_keys.update('last_connection', last_stored, new_stored_time, function(err) {
...
function load_hooks() {
  ...
  device_keys.exist('last_connection', (err, stored) => {
    ...
    if (stored) {
      ...
      device_keys.store('last_connection', last_connection, (err) => {
...
var unload = function() {
  ...
  hooks.remove('network_state_changed', request);
