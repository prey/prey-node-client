...
var request = function(re_schedule) {
  ...
  if (!keys.get().device) {
    console.log("NO DEVICE KEY!!")
    //return propagate_error(errors.get('NO_DEVICE_KEY'));
  }

  var config2 = require('getset').load({ path: join('/etc/prey', 'prey.conf'), type: 'file' });
  console.log("CONFIG2!!!", config2.get('control-panel.device_key'))

  if (config2.get('control-panel.device_key') == '' || config2.get('control-panel.api_key') == '') {
    agent.shutdown(function() {
      secure.get_keys(function(err, public_key, parsed_public_key){
        lp2.load(parsed_public_key, function(){
          return console.log("Running config wait")
        });
      })
    });
