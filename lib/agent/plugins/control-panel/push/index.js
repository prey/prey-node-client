...
var send_notification_id = function(str, cb) {
  ...
  api.push.data(data, {}, function(err, resp, body) {
...
var update_notification_id = function(cb){
  ...
  mapper.public_ip(function(err, ip) {
    ...
    var scrambled = scrambler.encrypt(str, agent.config.get('api_key'));
...
var valid_secret = function(str) {
  ...
  return str == secret;
  // return scrambler.decrypt(str, agent.config.get('api_key')) == secret;
}
  ...
  server.listen(port, function(err, app) {
    ...
    app.on('request', function(headers, body) {
      ...
      if (body.command == 'run_once')
        agent.hooks.trigger('woken');
      else
        agent.hooks.trigger('command', body);
    });
...
var check_mapping = function(cb) {
  ...
  find_mapping(function(err, mapping) {
    ...
    if (mapping && mapping.NewExternalPort) {
      ...
      var key  = agent.config.get('device_key');
      
      if (!key)
        return cb && cb(new Error('Key not found!'))
...
var stop = function(){
  ...
  is_mapping = false;
  agent.hooks.trigger('unreachable');
...
var unload = function(err, cb) {
  ...
  agent.hooks.remove('connected', check_mapping);
  agent.hooks.remove('disconnected', stop);

  // reset notification id, if we were able to send it
  if (last_nid && last_nid != '')
    send_notification_id('', cb);
  else
    cb && cb();
};
...
exports.load = function(common, cb) {
  ...
  agent.hooks.on('connected', check_mapping);
  agent.hooks.on('disconnected', stop);
