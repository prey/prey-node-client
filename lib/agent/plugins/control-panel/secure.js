...
exports.generate_keys = function(cb) {
  device_keys.exist(['private', 'public'])
  .catch(err => { return cb(new Error("Error reading stored security keys")) })
  .then(values => {
...
exports.open_config = function(device_key, cb) {
  setTimeout(function() {
    ...
    exports.generate_keys(function(err) {
      ...
      // For linux
      system.spawn_as_logged_user('xdg-open', [link], {}, function(err, child) {
        setTimeout(function() {
          child.kill();
        }, 10000);
        return cb && cb(err);
