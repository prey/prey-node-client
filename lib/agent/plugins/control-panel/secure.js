...
exports.generate_keys = function(cb) {
  new_storage.do('all', {type: 'keys'}, (err, values) => {
    ...
    // Create and store new keys
      ...
      new_storage.do('set', {type: 'keys', id: 'public_key', data: {value: Buffer.from(public_key).toString('base64')}}, (err) => {
        if (err) return cb(new Error("Error storing public security key"));
        new_storage.do('set', {type: 'keys', id: 'private_key', data: {value: Buffer.from(private_key).toString('base64')}}, (err) => {
...
exports.reset_keys = function(cb) {
  new_storage.do('del', {type: 'keys', id: 'private_key'}, (err) => {
    if (err) return cb(err);
    new_storage.do('del', {type: 'keys', id: 'private_key'}, (err) => {
      if (err) return cb(err);
      public_keys = {};
