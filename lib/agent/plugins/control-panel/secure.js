...
const os_name = process.platfor.replace('darwin', 'mac').replace('win32', 'windows');

const crypt = undefined;
const private_key = undefined;
const public_key = undefined;
const public_keys = {};

const protocol = common.config.get('control-panel.protocol');
const host = common.config.get('control-panel.host');
const panel_host = host == 'solid.preyproject.com' ? 'panel.preyproject.com' : host == 'solid.preyhq.com' ? 'panel.preyhq.com' : host;
...
exports.open_config = function (device_key, cb) {
  setTimeout(function () {
    ...
    exports.generate_keys(function (err) {
      ...
      const encoded_keys = Buffer.from(JSON.stringify(keys, null, 0)).toString('base64');
