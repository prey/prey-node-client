...
exports.generate_keys = function(cb) {
  storage2.do('all', {type: 'keys'}, (err, values) => {
  ...
  // device_keys.exist(['private', 'public'], function(err, values) {
    ...
    if (values.length > 0) {
      private_key = values[0].value;
      public_key = values[1].value;
    ...
    // Create and store new keys
      ...
      // storage2.do('set', {type: 'keys'}, (err, values) => {
      // device_keys.store(['public', 'private'], [public_key, private_key], function(err) {
      storage2.do('set', {type: 'keys', data: {key: 'public_key', value: public_key }} , function(err) {
        storage2.do('set', {type: 'keys', data: {key: 'private_key', value: private_key }} , function(err) {
