...
exports.notify_action = function(status, id, name, opts, err, out) {
  ...
  if (out && name == "factoryreset") body.reason = { "factoryreset": out.data }
  if (out && name == "fullwipe") body.reason = { "fullwipe": out.data }



  if (err) body.reason = err.message;
  if (err && (name == 'fullwipe' || name == 'fullwipewindows')) body = { command : body.command, target :opts.target , status : 'stopped', reason : { [opts.target] : 1} };
  if (err && (name == 'factoryreset')) {body.reason = { 'factoryreset' : 1};body.status = 'stopped' };
