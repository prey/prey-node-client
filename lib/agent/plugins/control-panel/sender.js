...
var make_request = function(what, data, opts, cb) {
  ...
  api.push[what](data, opts, function(err, resp) {
    if (data.opts) {
      if (data.opts.messageID) {
        resp.headers['X-Prey-Correlation-ID'] = data.opts.messageID;
        resp.headers['X-Prey-Device-ID']      = keys.get().device;
        resp.headers['X-Prey-State']          = "PROCESSED";
      }
      if (data.opts.device_job_id) {
        var job = { job_id: data.opts.device_job_id };
        api.push['response'](job);
      }
    }
