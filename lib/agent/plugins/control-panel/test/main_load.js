...
describe('main.load', function() {
  ...
  // we also stub the interval.load, push.load and api.devices.get.status
  ...
  describe('with invalid protocol', function() {
    ...
    describe('passing callback', function() {
      ...
      it('does not set the invalid protocol', function(done) {
        ...
        var keys_stub = sinon.stub(api.keys, 'get', function() {
          return { api: 'foobar', device: '123123' };
        })

        var needle_stub = sinon.stub(needle, 'request', function(method, url, opts, data, cb) {
  ...
  describe('with valid host and protocol', function() {
    ...
    describe('if setup() failed', function() {
      ...
      describe('and running on background', function() {
        ...
        it ('waits for config', function() {

          var called = false;

          // playing with fire here!
          var timeout_stub = sinon.stub(global, 'setTimeout', function(fn, delay) { 
            // make sure we check for the real setTimeout, not the one we're setting below!
            if (delay === 10000) 
              called = true;
          });
