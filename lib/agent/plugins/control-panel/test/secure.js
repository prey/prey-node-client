  ...
  describe('fetch private and public keys', function() {
    ...
    describe('with keys stored', function() {
      before(function(done) {
        storage2.init('keys', tmpdir() + '/store2.db', () => {
          storage2.do('set', {type: 'keys', id: 'public_key', data: {value: Buffer.from(public_key).toString('base64') }} , function(err) {
            storage2.do('set', {type: 'keys', id: 'private_key', data: {value: Buffer.from(private_key).toString('base64') }} , function(err) {
            done();
            })
          })
        });
      ...
      after((done) => {
        storage2.erase(tmpdir() + '/store2.db', done);
      })
    ...
    describe('with no keys stored', function() {
      before(function(done) {
        storage2.init('keys', tmpdir() + '/store2.db', done)
      })
      ...
      it('generate, store and returns new keys', function(done) {
        secure.generate_keys(function(err) {
          ...
          storage2.do('query', {type: 'keys', column : 'id', data: 'public_key' }, function (err,rows) {
            let key = rows[0];
            should.exist(key);
