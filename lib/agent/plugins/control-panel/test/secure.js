...
describe('secure()', function() {
  ...
  describe('fetch private and public keys', function() {
    ...
    describe('with keys stored', function() {
      before(function(done) {
        storage.do('set', {type: 'keys', data: {key: 'public_key', value: public_key }} , function(err) {
          storage.do('set', {type: 'keys', data: {key: 'private_key', value: private_key }} , function(err) {
           done()
          })
        })
      })
    ...
    describe('with no keys stored', function() {
      before(function(done) {
        storage.do('del_key', { type: 'keys', key: "public_key" }, function () {
          storage.do('del_key', { type: 'keys', key: "private_key" }, function () {
            done();
          })
        })
      })
      ...
      it('generate, store and returns new keys', function(done) {
        secure.generate_keys(function(err) {
          ...
          storage.do('query', {type: 'keys',column : 'key', data: 'public_key' }, function (err,rows) {
            let key = rows[0];
            should.exist(key);
            key.value.should.equal(publicKey);
            done();
          })
        });
      })
