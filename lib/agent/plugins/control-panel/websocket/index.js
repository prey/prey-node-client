...
var util   = require('util'),
    ...
    logger = common.logger.prefix('websocket'),
...
function retry_queued_responses() {
...
exports.startWebsocket = function() {
  ...
  var proxy      = config.get('try_proxy'),
    ...
    // protocol = "http",
    // host = "60c83fb58ef0.ngrok.io",
    // host = "10.179.253.56",
    // host = "10.179.249.196",
      device_key = keys.get().device,
      api_key    = keys.get().api;

  protocol = protocol == "https" ? "wss" : "ws"   // CONSULTAR SI TIENE QUE SER ASÍ, FUNCIONABA BIEN SIN ESTA LINEA
  // protocol = 'ws'
  ...
  // console.log("USER AGENT!", common.system.user_agent)
  ...
  status_trigger.get_status((err, status) => {
    ...
    //   console.log("WS ON CLOSE")
      if (exports.re_schedule) {
        setTimeout(exports.startWebsocket, 5000)
      }
    });
    ...
    ws.on('message', function incoming(data) {
      ...
      // su parseo y luego chequear los casos de abajo

      console.log("data web socket")
      console.log("data web socket")
      console.log("data web socket")
      console.log(data)
      console.log("data web socket")
      ...
      if (Array.isArray(data)) {
        ...
        if (data.status && data.status == "OK") {
          ...
          if (value) {
            storage2.do('update', { type: 'commands', id: value.reply_id, columns: value.body.status+'_resp', values: 1 }, (err) => {
            // storage2.update('commands', value.reply_id, value.body.status+'_resp', 1, (err) => {
...
exports.notify_action = (status, id, action, opts, time, resp_id) => {
  ...
  // ver donde se borra el registro cuando se notifica el stopped --> es on message mas arriba ^^
