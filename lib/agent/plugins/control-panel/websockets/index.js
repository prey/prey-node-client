...
let ws;
let pingTimeout;
let pingTimeoutRetry;
let emitter;
let setSearchingForClues = null;
...
exports.heartbeatTimed = (timeToRetry) => {
  clearTimeout(pingTimeoutRetry);
  pingTimeoutRetry = setTimeout(() => {
    restartWebsocketCall();
  }, timeToRetry);
};
...
const searchingForClues = () => {
  if (!ws || !ws.readyState || ws.readyState !== 1) {
    logger.info('Im searching for ws!');
    exports.heartbeatTimed(19000);
  }
};
...
exports.startWebsocket = () => {
  loadHooks();
  setSearchingForClues = setInterval(searchingForClues, 20000);
  ...
  statusTrigger.get_status((err, status) => {
    ...
    ws.on('ping', () => {
      exports.heartbeat();
      logger.info("PING BEFORE");
      if (!ws || !ws.readyState || ws.readyState !== 1) return;
      logger.info("PING AFTER");
      ws.pong();
    });
  });
};
...
exports.notify_status = (status) => {
  ...
  logger.info("NOTIFICANDO STATUS before");
  if (!ws || !ws.readyState || ws.readyState !== 1) return;
  logger.info("NOTIFICANDO STATUS after");
...
const clearIntervals = (aliveTimeReset = false) => {
  ...
  if(setSearchingForClues && aliveTimeReset) clearInterval(setSearchingForClues);
}
