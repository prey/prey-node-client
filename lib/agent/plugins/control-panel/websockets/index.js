...
const loadHooks = () => {
  storage.do('query', { type: 'keys', column: 'id', data: 'last_connection' }, (err, stored) => {
    ...
    if (stored && stored.length > 0) {
      ...
      storage.do('set', { type: 'keys', id: 'last_connection', data: { value: lastConnection }}, (err) => {
        if (err) logger.error('Error storing the last connection time');
