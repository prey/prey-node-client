...
exports.notifyAck = (ackId, type, retries = 0) => {
  if (retries >= retriesMax) {
    storage.do('del', { type: 'ack', id: ackId });
  ...
  if (ackResponse.length === 0) {
    ack.verifyIfExistId(ackId, (err, exist) => {
      if (err) return;
      if (!exist) {
        ack.registerAck({ ack_id: ackId, type, retries: 0 }, (errRegister, register) => {
          if (errRegister) return new Error(errRegister);
          exports.responsesAck.push(register);
          exports.sendAckToServer(register);
        });
      } else {
        exports.sendAckToServer({ ack_id: ackId, type });
      }
    });
  } else {
    ack.verifyIfExistId(ackId, (err, exist) => {
      if (err) return;
      if (!exist) {
        ack.registerAck(ackResponse[0], (errRegisterToVerify, register) => {
          if (errRegisterToVerify) return new Error(errRegisterToVerify);
          exports.responsesAck.push(register);
          exports.sendAckToServer(register);
        });
      } else {
