...
//_tr('modules',requested.device.modules);


/* 
  requested.device.modules[0].module.forEach(function(mod) {
    Object.keys(mod).forEach(function(report) {
      if (report !== '$')
        self.emit('report', report, mod[report]);
    });
  });
*/

  self.emit('report','status',{});
  self.emit('report','location',{});
/*
    ...
    for(var report in requested.modules) {
      _tr('emitting '+report)
  ...
  this.check_keys = function(callback){
    if(this.config.device_key === ""){
      ...
      if(this.config.api_key === "")
        callback(_error("No API key found."));
      else
