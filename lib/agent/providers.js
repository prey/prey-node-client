...
const hooks = /** @type {import('events').EventEmitter & { trigger: Function, remove: Function, unload: Function }} */ (
  require('./hooks')
);
...
function get(nameGet, extra, extra2, timesLimit = 0) {
  ...
  exports.map((err, getters) => {
    ...
    if (getters[name]) {
      // bypassCache callers require a guaranteed-fresh read, so they must never
      // be coalesced onto an in-flight shared fetch. They run their own getter
      // and get a private delivery that skips the shared callbacks queue.
      ...
      const deliver = (errCb, result) => {
        if (bypass) {
          // Private delivery: only this caller, never the shared queue.
          if (typeof cb === 'function') cb(errCb, result, name, timesLimit + 1);
          else if (errCb) hooks.trigger('error', errCb);
      ...
      try {
        if (options) {
          getters[name](options, deliver);
        } else {
          getters[name](deliver);
        }
      } catch (e) {
