...
const wifiNativeMatchThreshold = 0.3; // km (300m) - WiFi confirms native when within this
...
const bootstrapWin32Location = (cb) => {
  ...
  // Only skip the WiFi call if a trusted baseline already exists from a previous session
  if (!win32LastTrustedLocation) {
    logger.info('[WIN32-BOOTSTRAP] No trusted baseline — calling WiFi regardless of cached wifi location');
  ...
  storage.do('query', { type: 'keys', column: 'id', data: 'last_wifi_location' }, (err, rows) => {
    if (!err && rows && rows.length > 0) {
      ...
      if (existing && typeof existing.lat === 'number') {
        logger.info('[WIN32-BOOTSTRAP] Trusted baseline exists — seeding history from last_wifi_location');
...
const normalWin32Location = (lastValid, cb, timeoutFn) => {
  ...
  const attemptNative = () => {
    ...
    platform.get_location((err, result) => {
      ...
      wifi((wifiErr, wifiResult) => {
        if (wifiErr) {
          logger.info('[WIN32-NORMAL-VERIFY] Wifi verification failed. Using native candidate anyway.');
        ...
        if (nativeWifiDist <= wifiNativeMatchThreshold) {
          logger.info(`[WIN32-NORMAL-VERIFY] Wifi agrees with native (${(nativeWifiDist * 1000).toFixed(0)}m ≤ ${wifiNativeMatchThreshold * 1000}m). Picking best accuracy.`);
          const best = pickBestAccuracy(nativeCandidate, wifiResult);
          logger.debug(`[WIN32-NORMAL-VERIFY] Selected: ${best.method} (accuracy=${best.accuracy}m)`);
          return saveToLocationHistory(best, (saveErr) => cb(saveErr, best));
        }
        logger.info(`[WIN32-NORMAL-VERIFY] Wifi disagrees with native (${(nativeWifiDist * 1000).toFixed(0)}m > ${wifiNativeMatchThreshold * 1000}m). Using wifi result.`);
        saveToLocationHistory(wifiResult, (saveErr) => cb(saveErr, wifiResult));
      });
    });
  };
