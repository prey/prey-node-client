  ...
  needle.post(GEO_ENDPOINT, dataWifiAccessPoints, opts, (err, resp, body) => {
    try {
      if (err) {
        ...
        dataWifiToSave.partialEnd = `needle.post - Error after POST ${GEO_ENDPOINT} - Status code: ${resp.statusCode}`;
        saveDataWifi(dataWifiToSave);
        return cb(err);
      }
      ...
      checkResponse(body, (errCheckResponse, stdout) => {
        if (errCheckResponse) {
          ...
          dataWifiToSave.partialEnd = `checkResponse - Error after POST ${GEO_ENDPOINT} - Status code: ${resp.statusCode}`;
        ...
        if (stdout.geolocation) {
          ...
          dataWifiToSave.partialEnd = `checkResponse - Got geolocation as the response from POST ${GEO_ENDPOINT} - Status code: ${resp.statusCode}`;
          saveDataWifi(dataWifiToSave);
        ...
        if (!stdout.endpoint) {
          dataWifiToSave.error = JSON.stringify(errCheckResponse);
          dataWifiToSave.partialEnd = `checkResponse - No endpoint or geolocation at response from POST ${GEO_ENDPOINT} - Status code: ${resp.statusCode}`;
          saveDataWifi(dataWifiToSave);
        ...
        needle.post(url, dataWifiConsiderIp, options, (errorPostUrl, _resp, bodyPostUrl) => {
          if (errorPostUrl) {
            ...
            dataWifiToSave.partialEnd = `Second needle.post - Error from POST ${url} - Status code: ${_resp.statusCode}`;
            saveDataWifi(dataWifiToSave);
          ...
          if (bodyPostUrl && bodyPostUrl.error) {
            ...
            dataWifiToSave.partialEnd = `Second needle.post - Error in bodyPostUrl from POST ${url} - Status code: ${_resp.statusCode}`;
            saveDataWifi(dataWifiToSave);
          ...
          checkResponse(bodyPostUrl, (errorCheckResponse, stdoutCheckResponse) => {
            ...
            needle.put(GEO_ENDPOINT, locData, optsPut, (errPut) => {
              ...
              if (errPut) {
                dataWifiToSave.error = `error service ${GEO_ENDPOINT}  PUT: ${JSON.stringify(errPut)} - Status code: ${_resp.statusCode}`;
              ...
              dataWifiToSave.result = JSON.stringify(geolocation);
