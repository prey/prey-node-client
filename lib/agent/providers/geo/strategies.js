...
const geoip = (cb) => {
  ...
  needle.get('http://ipinfo.io/geo', (err, _resp, body) => {
    ...
    logger.info('Got location via geoip');
    const geoIPData = {
...
function processResponse(coords, cb) {
  ...
  if (!coords.location || (!coords.location.lat && !coords.location.latitude)) {
    logger.info('Couldnt get any geoposition data. Try moving around a bit.');
...
const sendData = (list, cb) => {
  logger.info('Sending AP data to location service');
  if (!(list && Array.isArray(list) && list.length > 1)) {
    logger.info('No access points found. There is online 1 or 0 access points in list.');
  ...
  logger.info(`dataWifiAccessPoints: ${JSON.stringify(dataWifiAccessPoints)}`);
  ...
  logger.info(`opts before first POST: ${JSON.stringify(opts)}`);
        ...
        needle.post(url, dataWifiAccessPoints, options, (errorPostUrl, _resp, bodyPostUrl) => {
          ...
          checkResponse(bodyPostUrl, (errorCheckResponse, stdoutCheckResponse) => {
            if (errorCheckResponse) {
              logger.info(`205 errorCheckResponse: ${errorCheckResponse}`);
              return cb(errorCheckResponse);
            }

            const geolocation = stdoutCheckResponse;
            logger.info(`geolocation: ${JSON.stringify(geolocation)}`);
            const locData = {
              geolocation,
              wifiAccessPoints: filteredList,
              provider,
            ...
            needle.put(GEO_ENDPOINT, locData, optsPut, (errPut) => {
              logger.info('Sending location data to prey service');
              if (errPut) {
...
const native = (cb) => {
  ...
  platform.get_location((err, res) => {
    if (err) return cb(err);
    logger.info('Got location via native geoloc');
