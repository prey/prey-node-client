...
function processResponse(coords, cb) {
  const lat = coords.location?.lat ?? coords.location?.latitude;
  const lng = coords.location?.lng ?? coords.location?.longitude;

  if (!coords.location || !Number.isFinite(lat) || !Number.isFinite(lng)) {
      ...
      if (resp && resp.statusCode === 429) {
        storage.do('query', { type: 'keys', column: 'id', data: 'last_wifi_location' }, (errorStorage, storedData) => {
          ...
          try {
            ...
            dataWifiToSave.partialEnd = `needle.post - Error after POST ${GEO_ENDPOINT} - Status code: ${statusCode}`;
            saveDataWifi(dataWifiToSave);
            const cachedLocation = JSON.parse(storedData[0].value);
            return processResponse(cachedLocation, cb);
          } catch (e) {
      ...
      checkResponse(body, (errCheckResponse, stdout) => {
        ...
        needle.post(url, dataWifiConsiderIp, options, (errorPostUrl, _resp, bodyPostUrl) => {
          ...
          if (bodyPostUrl && bodyPostUrl.error) {
            ...
            return cb(new Error(`Error in body response from location server: ${JSON.stringify(bodyPostUrl.error)}`));
          }
          // eslint-disable-next-line consistent-return
...
const maybePromoteToAnchor = (wifiResult) => {
  if (isAnchorEligible(wifiResult)) {
    ...
    const serialized = JSON.stringify(wifiResult);
    storage.do('set', {
      type: 'keys', id: 'last_trusted_location', data: { value: serialized },
    }, (e) => {
      if (e && e.message && e.message.includes('Already registered')) {
        storage.do('update', {
          type: 'keys', id: 'last_trusted_location', columns: 'value', values: serialized,
        }, (e2) => {
          if (e2) logger.debug('[WIN32-ANCHOR] Unable to update anchor in storage');
        });
      } else if (e) {
        logger.debug('[WIN32-ANCHOR] Unable to persist anchor to storage');
      }
    });
  } else {
  ...
  const attemptNative = () => {
    ...
    platform.get_location((err, result) => {
      ...
      if (!accuracyOk) {
        ...
        return wifi((wifiErr, wifiResult) => {
          if (wifiErr || !wifiResult) {
            logger.debug('[WIN32-NORMAL] ERROR: Both native and wifi failed');
            return cb(wifiErr || new Error('[WIN32-NORMAL] wifi returned no result'));
          }
      ...
      if (distance <= changeDetectionThreshold) {
        ...
        if (win32NativeOnlyCount >= 3) {
          if (isDisabled('wifi')) {
            ...
            wifi((wifiErr, wifiResult) => {
              win32NativeOnlyCount = 0;
              const wifiFailed = wifiErr || !wifiResult;
              sendGeoTelemetry({
                ...
                wifi_accuracy_m: wifiFailed ? null : (wifiResult.accuracy ?? null),
                native_wifi_delta_m: wifiFailed
                  ? null
                  : Math.round(calcDistanceKm(result, wifiResult) * 1000),
                outcome: wifiFailed ? 'unverified' : 'wifi_accepted',
    ...
    if (!errTrusted && trustedRows && trustedRows.length > 0) {
      try {
        ...
        if (trusted && typeof trusted.lat === 'number') {
          ...
          logger.warn('[WIN32-ANCHOR] last_trusted_location contains invalid data, clearing.');
          storage.do('del', { type: 'keys', id: 'last_trusted_location' }, () => {});
        }
      } catch (e) {
