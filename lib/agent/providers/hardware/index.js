...
exp.saveDataHardwareChanged = (first, dataTrack) => {
  if (first) {
    exp.setDataHardwareChanged(dataTrack);
  } else {
    exp.updateDataHardwareChanged(dataTrack);
...
const saveData = (dataTrack) => {
  ...
  // storage.do('set', { type: 'keys', id: 'hardware',
  // data: { value: JSON.stringify(dataTrack) } }, (err) => {
  //   if (err) logger.error('Unable to save hardware data');
  // });
  exp.saveDataHardwareChanged(true, dataTrack);
};
...
exp.track_hardware_changes = (dataTrack) => {
  ...
  utilStorage.getDataDbKey('hardware', (err, storedDt) => {
    ...
    if (exp.diffCount > 0) {
      ...
      exp.saveDataHardwareChanged(false, dataTrack);
