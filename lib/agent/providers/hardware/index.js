...
exp.get_rp_module = function(callback) {
  if(os_name == 'windows' && gte(system.os_release, "10.0.0")){
    os_functions.get_recovery_partition_status(function(err, available) {
      if (err)
        callback(err)
      else
      {
        let recovery_partition_info = {};
        recovery_partition_info.available = available; 
        callback(null, recovery_partition_info);
      }
    });
  }
  else{
