...
exp.get_check_full_wipe = (cb) => {
  ...
  system.spawn_as_admin_user(nodeBin, data, (errFirst, childWinsvcCb) => {
    if (errFirst) {
      return cb(errFirst, undefined, 'check_full_wipe');
    }
    if (typeof childWinsvcCb === 'function') {
      ...
      childWinsvcCb(action, data, (errCheckFullWipeCb, checkFullWipeCb) => {
        ...
        if (checkFullWipeCb.error) {
          return cb(new Error(checkFullWipeCb.message), { compatible: false }, 'check_full_wipe');
        }
        cb(null, { compatible: true }, 'check_full_wipe');
      });
    }
  });
};
