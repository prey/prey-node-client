...
const system = require('../../../system/linux');
    ...
    stdout.toString().split('Memory Device').forEach(function(block) {
      ...
      if (block.match(/\tSize: \d/)) {
        ...
        var obj = {
          ...
          size  : parseInt(block.match(/Size: (.+)/)[1]) * 1024     // in MBs
        }
...
// Variables to store the current session type and display
let currentSessionType = null;
let currentSessionDisplay = null;

// Verifies if the session type or display has changed
// Returns: {hasChanges: boolean, sessionType: string, display: string}
exports.check_session_changes = (callback) => {
  let hasChanges = false;

  system.get_session_type((errType, sessionType) => {
    system.get_session_display((errDisplay, display) => {
      if (!errType && sessionType) {
        if (currentSessionType !== null && currentSessionType !== sessionType) {
          hasChanges = true;
        }
        currentSessionType = sessionType;
      }

      if (!errDisplay && display) {
        if (currentSessionDisplay !== null && currentSessionDisplay !== display) {
          hasChanges = true;
        }
      ...
      callback(null, {
        ...
        display: currentSessionDisplay
      });
    });
  });
};

exports.get_ubuntu_session_type = system.get_ubuntu_session_type;
exports.get_ubuntu_session_display = system.get_ubuntu_session_display;
