...
exports.get_firmware_info = function(callback) {
  ...
  get_system_profiler_data('SPHardwareDataType', function(err, sp_data){
    ...
    var data = {
      ...
      mb_version    : (system.is_m1_or_m2())?system.get_info_chip():sp_data['smc_version_(system)'],
...
exports.get_osquery_runing = (cb) => {
  exec('launchctl list com.prey.osquery.agent | awk "/PID/ {print $3}"', (err, stdout, stderr) => {
    if (err) cb(null, undefined, 'osquery_running');
    if (stderr) cb(null, undefined, 'osquery_running');
    const matchResult = stdout.match(/\d+/);
    if (matchResult && !isNaN(Number(matchResult[0])))  { cb(null, true, 'osquery_running'); }
    else { cb(null, false, 'osquery_running'); }
  });
}
