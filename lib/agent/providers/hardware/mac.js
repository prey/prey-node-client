...
exports.get_firmware_info = (callback) => {
  ...
  getSystemProfilerData('SPHardwareDataType', (err, spData) => {
    ...
    getSystemProfilerData('SPiBridgeDataType', (errSpi, spiData) => {
      logger.info(`SPiBridgeDataType: ${JSON.stringify(spiData)}`);
      if (errSpi) return callback(errSpi);
      let modelNameSecurityChip = '';
      if (spiData && Object.prototype.hasOwnProperty.call(spiData, 'model_name')) {
        modelNameSecurityChip = spiData.model_name;
      }
      const data = {
      ...
      logger.info(`DATA TOTAL: ${JSON.stringify(data)}`);
      callback(null, data);
    }, true);
  });
};
