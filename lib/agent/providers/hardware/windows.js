  ...
  const fetch = (key, section, value) => {
    wmic.get_value(section, value, null, (err, resp) => {
      ...
      if (key === 'device_type') {
        ...
        // if (res === 'Desktop') {
        logger.info('1');
        exec('wmic systemenclosure get ChassisTypes', (errorChassisTypes, Chassistypes) => {
          logger.info('2');
          if (errorChassisTypes) logger.info(`Error on getting Chassistype: ${errorChassisTypes}`);
          const laptopTypes = [9, 10, 14];
          const typeNumber = Number(Chassistypes.match(/\d+/g));
          laptopTypes.includes(typeNumber);
          // if (laptopTypes.includes(typeNumber)) {
            res = 'Laptop';
            logger.info(`Chassistypes: ${typeNumber}, type: ${res}`);
          // } else {
            exec('powershell @(Get-CimInstance -ClassName Win32_Battery).Count -gt 0', (errorhasBattery, hasBattery) => {
              logger.info('3');
              if (errorhasBattery) logger.info(`Error on getting Chassistype: ${errorhasBattery}`);
              // eslint-disable-next-line no-extra-boolean-cast
              if (Boolean(hasBattery)) res = 'Laptop';
              logger.info(`hasBattery: ${Boolean(hasBattery)}, type: ${res}`);
            });
          // }
        });
        // }
