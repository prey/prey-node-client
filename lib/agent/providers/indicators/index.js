...
exports.get_uptime = function (callback) {
  callback(null, Math.floor(os.uptime()));
};
...
/**
 * Get battery info. Accepts an optional options object (e.g. { bypassCache: true })
 * which is forwarded to the platform getter so event-driven callers — like the
 * power trigger — can force a fresh read past the TTL cache. Called generically by
 * providers.get as (options, cb) when options are present, or (cb) otherwise, so it
...
exports.get_battery_status = function (optsOrCb, maybeCb) {
  ...
  os_functions.get_battery_status(opts, (err, stdout) => {
