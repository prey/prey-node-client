...
var battery_fallback = function(callback) {
  ...
  exec(cmd, { timeout: 8000 }, function(err, stdout) {
    ...
    exec(cmdInfo, { timeout: 8000 }, function(err, stdout){
      if(err) return callback(err);
...
/**
 * Try upower, then fallback to /proc/acpi if upower does not exist. This platform
 * has no TTL cache, so any options object (e.g. { bypassCache: true }) is accepted
 * and ignored; it exists only to keep a uniform (options, cb) / (cb) contract with
 * providers.get.
...
exports.get_battery_status = function(optsOrCb, maybeCb){
  ...
  var callback = typeof optsOrCb === 'function' ? optsOrCb : maybeCb;
...
exports.get_remaining_storage = function(callback) {
  ...
  exec("df -kh / | tail -1", { timeout: 8000 }, function(err, stdout){
    if (err) return callback(err);
