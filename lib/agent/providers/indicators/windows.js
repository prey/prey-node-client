...
exports.get_storage_encryption_status = function(callback) {
  exec("powershell.exe -Command \" Get-BitLockerVolume | foreach {$_.MountPoint + ' ' + $_.ProtectionStatus } \"", function(err, stdout){
    if (err) return callback(err);
    
    var volumes = stdout.toString().trim().split(os.EOL);
    ...
    volumes.forEach(v => {      
      if (v.includes("On")) {
        info.encrypted.push(v.split(/\s+/)[0])
      } else {
        info.unencrypted.push(v.split(/\s+/)[0])
      }
    });
