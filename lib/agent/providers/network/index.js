...
function get_active_access_point_strength(cb) {
  os_functions.get_active_access_point_strength((err, signal_strength) => {
    if (!err) active_access_point.signal_strength = signal_strength;
    return cb(null, active_access_point);
  })
}
...
exp.get_active_access_point = function(callback) {
  if (active_access_point) {
    // Refresh the signal strength data when is the same access point
    return get_active_access_point_strength(callback);
  }
  ...
  var done = function(err, ap) {
    getting_ap = false;
    if (!err && Object.keys(ap).length > 0)
      active_access_point = ap;
