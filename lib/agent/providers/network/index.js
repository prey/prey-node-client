...
exp.get_active_access_point = function(callback) {
  osFunctions.get_active_access_point(function(err, stdout) {
    if (err) {
...
exp.get_connection_type = (callback) => {
  network.get_active_interface((err, stdout) => {
    if (err) return callback();
    let connectionType = stdout.type !== 'Wireless' ? 'wired' : 'wireless';
    status.set_status('connection_type', connectionType);
    return callback(null, connectionType);
  });
};
