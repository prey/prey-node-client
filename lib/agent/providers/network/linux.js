...
exports.get_access_points_list = (callback, attempt, useNmcli = true) => {
  if (!useNmcli) return exports.getAcessPointAlternative(callback, attempt);
  // eslint-disable-next-line consistent-return
  sudo('nmcli -t -f BSSID,SSID,CHAN,SIGNAL,SECURITY device wifi list', (err, stdout, stderr) => {
    if (err || stderr) return exports.getAcessPointAlternative(callback, attempt, false);
