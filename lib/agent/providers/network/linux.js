...
exports.get_access_points_list = (callback, attempt, useNmcli = true) => {
  ...
  exec('nmcli -t -f BSSID,SSID,CHAN,SIGNAL,SECURITY device wifi list', (err, stdout, stderr) => {
    if (stderr) logger.debug(`nmcli stderr (informational): ${stderr.toString().trim()}`);
    if (err) {
      logger.error(`nmcli error: ${err.message} — falling back to iwlist`);
      return exports.getAcessPointAlternative(callback, attempt);
    }
    ...
    exec('nmcli device wifi rescan', (rescanErr) => {
      if (rescanErr) logger.debug(`nmcli rescan warning: ${rescanErr.message}`);
      // Wait for the scan to complete before fetching the new list
      setTimeout(() => {
        exec('nmcli -t -f BSSID,SSID,CHAN,SIGNAL,SECURITY device wifi list', (err2, stdout2, stderr2) => {
          if (stderr2) logger.debug(`nmcli rescan stderr (informational): ${stderr2.toString().trim()}`);
          if (err2) {
            logger.error(`nmcli error after rescan: ${err2.message} — falling back to iwlist`);
            return exports.getAcessPointAlternative(callback, attempt);
          }
          ...
          logger.debug(`nmcli fresh scan: got ${freshList.length} APs`);
          if (freshList.length < list.length) {
            logger.debug(`nmcli fresh scan returned fewer APs than cached (${freshList.length} < ${list.length}), using cached list`);
          }
