...
exports.get_access_points_list = (callback, attempt, useNmcli = true) => {
  ...
  logger.debug(`get_access_points_list:\nuseNmcli: ${useNmcli}\nattempt: ${attempt}`);
  if (!useNmcli) {
    logger.debug('Using getAcessPointAlternative');
  ...
  exec('nmcli -t -f BSSID,SSID,CHAN,SIGNAL,SECURITY device wifi list', (err, stdout, stderr) => {
    if (err || stderr) {
      logger.debug(`There was an error with command nmcli: ${err}`);
    ...
    logger.debug('Returning wifi list from nmcli command');
