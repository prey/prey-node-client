...
exports.get_access_points_list = (callback, attempt, useNmcli = true) => {
  ...
  // eslint-disable-next-line consistent-return
  exec('sudo nmcli -t -f BSSID,SSID,CHAN,SIGNAL,SECURITY device wifi list', (err, stdout, stderr) => {
    if (stderr) {
    ...
    exec('sudo nmcli device wifi rescan', () => {
      ...
      setTimeout(() => {
        exec('sudo nmcli -t -f BSSID,SSID,CHAN,SIGNAL,SECURITY device wifi list', (err2, stdout2) => {
          if (err2) {
            return exports.getAcessPointAlternative(callback, attempt);
          }
