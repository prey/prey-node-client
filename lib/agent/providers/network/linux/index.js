 ...
 **/
var have_wifi = exports.have_wifi = function(callback) {
...
exports.get_access_points_list = function(wifi_device, callback) {
  ...
  if (!wifi_device || wifi_device === '')
    return callback(_error('No wifi device found.'));

  have_wifi(function(err, wifi) {
    if (err || !wifi) return callback(_error(err || 'No Wifi device found.'));
