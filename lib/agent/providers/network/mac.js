...
var get_ap_list = function(out, cb) {   // Primer caso out=''
  current_attemp++;
  if (current_attemp >= attemps) return cb(null, '');
  else if (out != '') return cb(null, out);
  else {
    exec(airport_cmd + ' -s', function(err, stdout) {
      get_ap_list(stdout, cb);
    });
  }
}
...
exports.get_access_points_list = function(callback) {
  ...
  get_ap_list('', function(err, stdout) {
