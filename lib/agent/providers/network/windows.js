...
exports.parse_access_points_list_netsh = function(out) {
  ...
  var build_ap = function(base, router) {
    var obj = {
      ssid        : base['0'].toString().trim() == '' ? '(Unknown)' : base['0'],
      security    : base['2'] != 'Open' ? base['2'] : null,
      mac_address : router['0']
    }
    ...
    if (router['1'])
      obj.signal_strength = parseInt(router['1']) - 100;

    if (router['3'])
      obj.channel = parseInt(router['3']);
