...
exports.get_active_access_point = function(callback) {
  if (release >= 6.0) {
    exec('netsh wlan show interfaces', (err, stdout) => {
      ...
      try {
        var ssid            = stdout.toString().match(/SSID\s+:\s?(.+)/)[1],
            mac_address     = stdout.toString().match(/BSSID\s+:\s?(.+)/)[1],
            signal_strength = parseInt(stdout.toString().match(/Signal\s+:\s?(.+)/)[1]) - 100,
            channel         = parseInt(stdout.toString().match(/Channel\s+:\s?(.+)/)[1]),
            security        = stdout.toString().match(/Authentication\s+:\s?(.+)/)[1];
...
exports.parse_access_points_list_netsh = function(out) {
  ...
  var build_ap = function(base, router) {
    var obj = {
      ssid        : base[SSID_KEYS.SSID].toString().trim() == '' ? '(Unknown)' : base['0'],
