 ...
 * */
exports.get_wireless_interfaces_list = (callback) => {
  const query = 'nic where "Name like \'%Wireless%\'" get NetConnectionID';

  const gte = common.helpers.is_greater_or_equal;

  if (gte(common.os_release, '10.0.0')) si.networkInterfaceDefault((defaultNetwork) => callback(null, defaultNetwork));
  else {
    // eslint-disable-next-line consistent-return
    wmic.run(query, (err, o) => {
      if (err) return callback(err);

      const list = o.split('\n').splice(1).map((n) => n.trim());
      callback(null, list);
    });
  }
};
      ...
      try {
        ...
        // When the data isn't consistent we're using the old method
          ...
          exports.get_access_points_list((err, aps) => {
            ...
            const paddedMac = macAddress.toString().trim().replace(/(^|:)(?=[0-9a-fA-F](?::|$))/g, '$10');

            const apsFiltered = aps.filter((ap) => ap.macAddress === paddedMac);
 ...
 * { ssid:"",security:true,quality:23,signalStrength:54,noise_level:24}
 *
 * autowc actually returns {macAddress,ssid,signalStrength,channel,signal_to_noise}
...
exports.parse_access_points_list_netsh = (out) => {
  ...
  const buildAp = (base, router) => {
    const obj = {
      ...
      macAddress: router[BSSID_KEYS.BSSID],
    };

    // signal is shown as '94%', so we need to substract 100 to get a consistent behaviour with
    // OSX and Linux's signalStrength integer
    // eslint-disable-next-line max-len
    if (router[BSSID_KEYS.SIGNAL]) obj.signalStrength = parseInt(router[BSSID_KEYS.SIGNAL], 10) - 100;
