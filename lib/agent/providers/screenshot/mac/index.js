 ...
 */
const deleteScreenshot = (cb) => {
  // eslint-disable-next-line consistent-return
  run_as_logged_user(`rm -f ${screenshotUtilData.screenshotPath} ${screenshotUtilData.tmpScreenshot}`, [], (err) => {
    if (err) return cb(err);
    cb();
  });
};
 ...
 * @return {undefined} - No return value.
 */
const mvAndCopyFile = (cb) => {
  run_as_logged_user(`mkdir ${screenshotUtilData.screenshotDir}`, [], () => {
    // eslint-disable-next-line consistent-return
    run_as_logged_user(`cp ${screenshotUtilData.screenshotPath} ${screenshotUtilData.tmpScreenshot}`, [], (err) => {
...
exports.get_screenshot = (callback) => {
  ...
  common.system.get_logged_user((err, user) => {
    ...
    getIdUser(user, (errorUserId) => {
      ...
      // eslint-disable-next-line consistent-return
      deleteScreenshot((errorDeletescreenshots) => {
