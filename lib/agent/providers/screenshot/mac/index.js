...
const callSocket = (callback) => {
  socket.writeMessage(nameArray[4], (err, data) => {
    if (err) return done(new Error("Couldn't get screenshot_1"), null, callback);
    try {
      const logger = require('../../../common').logger.prefix('[][][][][]');
      logger.info(`JSON.stringify(data) : ${JSON.stringify(data)}`);
      if (typeof data.result !== 'string')
        return done(new Error("Couldn't get screenshot_2"), null, callback);
      done(null, data.result, callback);
    } catch (_ex) {
      
      done(new Error(`Couldn't get screenshot_3 - ${_ex}`), null, callback);
    }
...
exports.get_screenshot = (callback) => {
  configutil.getDataDbKey('skippedPermissions', (err, skippedPermissions) => {
    if (err) return callSocket(callback);

    try {
      if (skippedPermissions && JSON.parse(skippedPermissions[0].value).screenshot == "true")
        return done(new Error('Screenshot permission skipped'), null, callback);
    } catch (e) {
      return callSocket(callback);
    }
