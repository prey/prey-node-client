 ...
 */
const deletePicture = (cb) => {
  // eslint-disable-next-line consistent-return
  run_as_logged_user(`rm -f ${pictureUtilData.picturePath} ${pictureUtilData.tmpPicture}`, [], (err) => {
    if (err) return cb(err);
    cb();
  });
};
...
const mvAndCopyFile = (cb) => {
  run_as_logged_user(`mkdir ${pictureUtilData.pictureDir}`, [], () => {
    // eslint-disable-next-line consistent-return
...
exports.get_picture = (callback) => {
  ...
  common.system.get_logged_user((err, user) => {
    ...
    getIdUser(user, (errorUserId) => {
      ...
      // eslint-disable-next-line consistent-return
      deletePicture((errorDeletePictures) => {
