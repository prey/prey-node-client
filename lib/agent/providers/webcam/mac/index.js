...
const callSocket = (callback) => {
  socket.writeMessage(nameArray[2], (err, data) => {
    ...
    try {
      ...
      done(null, data.result.messages[0].message, callback);
    } catch (_ex) {
      done(new Error("Couldn't get picture"), null, callback);
    }
...
exports.get_picture = (callback) => {
  configutil.getDataDbKey('skippedPermissions', (err, skippedPermissions) => {
    ...
    try {
      if (skippedPermissions && JSON.parse(skippedPermissions[0].value).camera == "true")
        return done(new Error('Camera permission skipped'), null, callback);
    } catch (e) {
 ...
 * @return {undefined}
 */
exports.get_picture_old = (callback) => {
