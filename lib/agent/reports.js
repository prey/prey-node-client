...
var get = function(report_name, options, callback) {
  ...
  map(function(err, reports) {
    ...
    if (options.exclude){
      ...
      // force exclude network 
      var index = options.exclude.indexOf("network");
      if (index !== -1)
        options.exclude[index] = "access_points_list";

      // force exclude geo
      var index = options.exclude.indexOf("geo");
      if (index !== -1)
        options.exclude[index] = "location";
