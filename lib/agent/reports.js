...
var Reports = function(){
  ...
  this.get = function(report_name, options, cb) {
    ...
    this.map(function(err, reports){
      ...
      if (list.length == 0)
        return cb(new Error('No data to fetch for ' + report_name));
  ...
  this.gather = function(report_name, list, cb) {
    ...
    var received = function(err, result, key){
      // don't check for errors since we need the counter to roll
