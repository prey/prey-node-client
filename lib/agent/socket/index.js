...
const getOwnerShip = (cb) => {
  exec(`ls -l ${socketFile}`, (error, stdout, stderr) => {
    ...
    if (stderr) {
      logger.error(`Error en la salida estándar: ${stderr}`);
      cb(stderr);
    }
    const owner = stdout.split(' ')[2]; // El propietario estará en la tercera posición en la salida
    cb(null, owner);
  });
};
/**
...
const createConnectionSocket = (messageToSendSocket, cb) => {
  ...
  try {
    ...
    getOwnerShip((err, owner) => {
      ...
      if (owner.localeCompare('prey') !== 0) return cb(new Error('The socket file is not owned by Prey'));
