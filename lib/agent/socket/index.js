...
const handleConnection = (messageToSendSocket, cb) => {
  ...
  const connectionTimeout = setTimeout(() => {
    if (!dataReceived && !socketConnection.destroyed) {
      logger.error('Socket connection timeout');
      socketConnection.removeAllListeners();
      socketConnection.destroy();
      if (typeof cb === 'function') {
        cb(new Error('Socket connection timeout'));
      }
    }
  ...
  socketConnection.on('end', () => {
    clearTimeout(connectionTimeout);
    socketConnection.removeAllListeners();
    if (!dataReceived && typeof cb === 'function') {
      cb(new Error('Connection ended without receiving data'));
    }
  });
};
/**
...
const addAndWait = (functionName, cb, time) => {
  ...
  if (!exports.intervalToSendMessages) {
    ...
    // Create cleanup interval to prevent endless loops if application runs for long time
    if (!exports.cleanupInterval) {
      exports.cleanupInterval = setInterval(() => {
        // Clear stale messages older than 5 minutes
        const currentTime = new Date().getTime();
        exports.messagesData = exports.messagesData.filter(msg => {
          if (msg.time && (currentTime - msg.time > 5 * 60 * 1000)) {
            if (typeof msg.cbAttached === 'function') {
              msg.cbAttached(new Error(`Message timeout for ${msg.functionName}`));
            }
            return false;
          }
          return true;
