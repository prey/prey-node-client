  ...
  try {
    ...
    if (verifyConsistencyData(dataObj) && messageToSendSocket.message === exports.currentMessage) {
      if (nameFunction.localeCompare(exports.currentMessage.functionName) === 0) {
        logger.info(`Completed because data match: ${exports.currentMessage.id}`);
...
const handleConnection = (messageToSendSocket, cb) => {
  ...
  logger.info(`Writing message: ${messageToSendSocket.id}`);
...
const createConnectionSocket = (messageToSendSocket, cb) => {
  try {
    if (!checkSocketFileExists()) {
      logger.info(`The socket file does not exist: ${messageToSendSocket.id}`);
    ...
    logger.info(`Error between ownership and local socket: ${messageToSendSocket.id}`);
...
const sendMessage = (messageToSend, cb) => {
  ...
  logger.info(`trying to send message with id: ${messageToSend.id} - ${JSON.stringify(messageToSend)}`);
  if (Object.prototype.hasOwnProperty.call(functionObject, messageToSend.functionName)) {
    logger.info(`passed the hasownproperty allegations: ${messageToSend.id}`);
...
const tryToSendNew = () => {
  ...
  if (((new Date()).getTime() - exports.currentMessage.time) >= exports.currentMessage.timeLimitPerMessage) {
    logger.info(`completed because time exceeded for ${exports.currentMessage.id}`);
    exports.currentMessage.completed = true;
    if (typeof exports.currentMessage.cbAttached === 'function') {
      logger.info(`Time exceeded for ${exports.currentMessage.id}`);
  ...
  if (exports.messagesData.length === 0) {
    logger.info('exports.messagesData.length = 0');
    exports.currentMessage = null;
    return;
  }
  if (exports.currentMessage.completed) {
    logger.info(`completed ${exports.currentMessage.id} - ${exports.currentMessage.completed}`);
    ...
    if (exports.messagesData.length === 0) {
      logger.info('exports.messagesData.length = 0 -- 2');
      exports.currentMessage = null;
      return;
    }
    // eslint-disable-next-line prefer-destructuring
    exports.currentMessage = exports.messagesData[0];
    logger.info(`2 - new message to send ${exports.currentMessage.id} - ${JSON.stringify(exports.currentMessage)}`);
...
const addAndWait = (functionName, cb, time) => {
  ...
  if (!exports.currentMessage) {
    ...
    logger.info(`1 - new message to send ${exports.currentMessage.id} - ${JSON.stringify(exports.currentMessage)}`);
