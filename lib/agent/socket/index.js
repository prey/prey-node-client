...
let messagesData = [];
let currentMessage;
let activeToSend = false;
let intervalToSendMessages = null;
const timeLimitPerMessage = 7000;

const setActiveMessages = (setActive) => {
  activeToSend = setActive;
};
...
const createConnectionSocket = (messageToSendSocket, cb) => {
  try {
    ...
    if (!existsFile) {
      return cb(new Error('The socket file does not exist'));
    }
    // eslint-disable-next-line consistent-return
      ...
      socketConnection.on('data', (data) => {
        try {
          ...
          if (verifyConsistencyData(dataObj)) {
            ...
            logger.debug(`Function's name: ${dataObj.function}`);
...
const tryToSendNew = () => {
  ...
  if (currentMessage.completed) {
    ...
    sendMessage(currentMessage.functionName, currentMessage.cbAttached);
  }
};
/**
...
exports.setActiveMessages = setActiveMessages;
