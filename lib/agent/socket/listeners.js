...
const sendDataToEndpoint = (wifiLocationPermission, nativeLocationPermission, cb) => {
...
const shouldSendCallApiWindows = (dataWifiLocationBefore, data) => dataWifiLocationBefore.localeCompare('') === 0
  || (dataWifiLocationBefore.localeCompare('false') === 0 && data[1].localeCompare('Allow') === 0)
  || (dataWifiLocationBefore.localeCompare('true') === 0 && data[1].localeCompare('Allow') !== 0);
...
const reactToCheckLocationPerms = (data) => {
  ...
  switch (exports.osName) {
    case 'windows':
      try {
        logger.info('Windows - reactToCheckLocationPerms');
        logger.info(`Windows - dataWifiLocationBefore: ${dataWifiLocationBefore} v/s ${data[1]}`);
        if (shouldSendCallApiWindows(dataWifiLocationBefore, data)) shouldSend = true;
        if (data[1].localeCompare('Allow') === 0) wifiLocationPermission = 'true';
        exports.getDataFromPermissionFile('wifiLocation', wifiLocationPermission, () => {
          logger.info('Windows - checking if should send!');
          if (shouldSend) {
            logger.info('Windows - sending!');
            sendDataToEndpoint(wifiLocationPermission, 'false', () => {});
          }
        });
      } catch (error) {
        console.log(error);
      ...
      break;
    case 'mac':
      logger.info('MacOS - reactToCheckLocationPerms');
      exports.getDataFromPermissionFile('nativeLocation', data[1].result, () => {
        logger.info(`MacOS - dataNativeLocationBefore ${dataNativeLocationBefore} v/s data: ${data[1].result}`);
        if (dataNativeLocationBefore.localeCompare(data[1].result) !== 0) shouldSend = true;
        exports.isWifiPermissionActive((output) => {
          logger.info(`MacOS - dataWifiLocationBefore ${dataWifiLocationBefore} v/s output: ${output}`);
          if (dataWifiLocationBefore.localeCompare(output) !== 0) shouldSend = true;
          exports.getDataFromPermissionFile('wifiLocation', stringBooleanOrEmpty(output), () => {
            logger.info('MacOS - checking if should send!');
            if (shouldSend) {
              logger.info('MacOS - sending!');
              sendDataToEndpoint(output.toString(), data[1].result, data[2]);
            }
          });
        });
      });
