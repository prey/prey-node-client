...
exports.is_wifi_on = (cb) => {
  var cmd = `networksetup -getairportpower ${net_interface}`
  exec(cmd, (err, stdout) => {
    var status = stdout.split('Wi-Fi Power (en0): ').pop().trim(),
        enabled = status == 'On' ? true : false;

    if (!enabled) return exports.enable_wifi(cb);
    return cb();
  })
}

exports.enable_wifi = (cb) => {
  var restart_wifi = (err) => {
  ...
  net_interface ? restart_wifi() : get_interface(restart_wifi);
}
