...
var delete_profile = (ssid, cb) => {
  get_existing_profiles((err, current_profiles) => {
    ...
    if (!ssid || exports.init_profiles.indexOf(ssid) != -1 || current_profiles.indexOf(ssid) == -1 || (connected_to && connected_to.ssid == ssid))
      return cb(new Error("Nothing to delete " + ssid));

    os_functions.delete_profile(ssid, (err) => {
      if (!err) {
        logger.debug("Profile " + ssid + " successfully deleted");
        previous = null;
      }
      return cb && cb(err);
    })
...
var create_profile = (ssid, cb) => {
  get_existing_profiles((err, current_profiles) => {
    ...
    os_functions.create_profile(ssid, (err) => {
      if (!err) logger.debug("Profile " + ssid + " successfully created");
      cb(null);
    });
  })
}
