...
var check_zones = function() {
  ...
  zones.forEach(function (zone) {
    ...
    if (distance * 1000 > zone.radius) { // outside
      ...
      new_storage.do('update', { type: 'geofences', id: zone.id, columns: 'state', values: 'outside' }, (err) => {
          ...
          new_storage.do('update', { type: 'geofences', id: zone.id, columns: 'state', values: 'inside' }, (err) => {
          if (err) logger.error(err);
