...
var check_zones = function() {
  ...
  zones.forEach(function (zone) {
    ...
    if (distance * 1000 > zone.radius) { // outside
      ...
      storage.update(zone.id, zone.name, zone.state, 'outside', function(err) {
        if (err) logger.error(err);
        if (zone.state == 'inside' && zone.direction !== 'in') {
          logger.info('Device left the geofence ' + zone.name + '! Notifying')
          push_event('geofencing_out', zone.id, location);
        }
        checking = false;
        zone.state = 'outside';
      });
...
exports.start = function(opts, cb) {
  ...
  hooks.on('new_location', function(coords) {
    ...
    if (zones && zones.length > 0 && (coords.method != 'wifi' || coords.accuracy > 300))
    ...
    if (coords.delta && coords.delta < 50) return;
