...
var start_polling = () => {
  check_hostname();
  poll_timer = setInterval(check_hostname, POLL_INTERVAL);
};
  ...
  .then(current_name => {
    storage.do('query', {type: 'keys', column: 'id', data: 'hostname'}, (err, stored) => {
      ...
      if (stored && stored.length > 0) {
        ...
        // Guard against corrupted data: a hostname never starts with [ or {
        const trimmed = stored_name ? stored_name.trim() : '';
        if (!trimmed || trimmed[0] === '[' || trimmed[0] === '{' || trimmed[0] === '"') {
          logger.error(`Stored hostname contains invalid data, resetting to current hostname`);
          return storage.do('update', { type: 'keys', id: 'hostname', columns: 'value', values: current_name }, done);
        }
  ...
  setTimeout(() => {
    try {
      triggers.watch('hostname', (err, hostname) => {
        if (err) {
          logger.warn('OS hostname watcher unavailable: ' + err.message + '. Falling back to polling.');
          emitter = new Emitter();
          start_polling();
          return cb(null, emitter);
        }
        hostname.on('state_changed', () => {
          check_hostname();
        });
      ...
      logger.warn('Failed to spawn OS trigger process: ' + e.message + '. Falling back to polling.');
      emitter = new Emitter();
      start_polling();
      cb(null, emitter);
    }
