...
const MAX_NEXT_WINDOW_CHECK_MS = 60 * 60 * 1000; // cap wait-for-window at 1h to absorb DST drift
const MAX_LOC_CALLBACKS = 5;
const MIN_FORCE_SPACING_MS = 1000; // forceLocation must never run faster than 1x/second
const FORCE_THROTTLE_WARN_INTERVAL_MS = 60 * 1000; // warn about throttling at most once a minute

const DAYS = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday'];

/**
 * Extracts hours, minutes, and weekday from a Date in a given IANA timezone.
 * Falls back to local time when timezone is null/undefined/invalid.
 * @param {Date} date
 * @param {string|null|undefined} timezone
 * @returns {{ hours: number, minutes: number, dayOfWeek: number }}
 */
const getTimeComponents = (date, timezone) => {
  if (!timezone) {
    return { hours: date.getHours(), minutes: date.getMinutes(), dayOfWeek: date.getDay() };
  }
  try {
    ...
    return {
      ...
      dayOfWeek: DAY_MAP[get('weekday')],
    };
  } catch (_) {
    logger.warn(`[force-location] Invalid timezone '${timezone}', using local time`);
    return { hours: date.getHours(), minutes: date.getMinutes(), dayOfWeek: date.getDay() };
  }
};
 ...
 * @param {string|null|undefined} [timezone] - IANA timezone string (e.g. 'America/Edmonton').
 *   When provided, schedule times are evaluated in this timezone instead of device local time.
 * @returns {{ shouldSend: boolean }}
 */
const checkSchedule = (schedule, timezone) => {
  const { hours, minutes, dayOfWeek } = getTimeComponents(new Date(), timezone);
...
const fetchLocation = (typeFetch, callback) => {
  ...
  if (callback && typeFetch !== 'force' && typeof callback === 'function') {
    if (locCallbacks.length >= MAX_LOC_CALLBACKS) {
      logger.warn('locCallbacks queue full, rejecting duplicate location request');
      return callback(new Error('Location fetch already in progress'));
    }
    locCallbacks.push(callback);
  }
  ...
  const done = (err, coords) => {
    ...
    } else if (err && callBackStored) {
      callBackStored.callback(err);
    }
    locCallbacks = [];
    checking = false;
  };
 ...
 * @param {Date} now
 * @param {string|null|undefined} [timezone] - IANA timezone string for account-local time.
 * @returns {number|null}
 */
const msUntilNextWindowStart = (schedule, now, timezone) => {
  const { hours, minutes, dayOfWeek } = getTimeComponents(now, timezone);
  const currentMinutes = hours * 60 + minutes;
  const [startH, startM] = schedule.start_at.split(':').map(Number);
  const startMinutes = startH * 60 + startM;

  for (let offset = 0; offset < 7; offset++) {
    const candidateDay = DAYS[(dayOfWeek + offset) % 7];
 ...
 * @returns {number}
 */
const msUntilWindowEnd = (schedule, now, timezone) => {
  const { hours, minutes } = getTimeComponents(now, timezone);
  const currentMinutes = hours * 60 + minutes;
  const [startH, startM] = schedule.start_at.split(':').map(Number);
  const [endH, endM] = schedule.end_at.split(':').map(Number);
  const startMinutes = startH * 60 + startM;
...
let lastForceRun = 0;
let lastForceThrottleWarn = 0;

const forceLocation = () => {
  // Defense-in-depth: forceLocation must never run in a tight loop. If anything — a scheduling
  // bug, a config-change storm firing restartForceInterval, or duplicate listeners — drives it
  // faster than once per second, throttle it and warn at most once a minute instead of flooding
  // the log (and hammering SQLite via writeStorage) with thousands of identical lines.
  const now = Date.now();
  ...
  if (schedule) {
    const timezone = config.getData('timezone') || null;
    const { shouldSend } = checkSchedule(schedule, timezone);
    if (!shouldSend) {
      logger.info('Force location skipped: outside tracking schedule window');
      return;
    }
  } else if (!config.getData('control-panel.location_aware')) {
    // No schedule + location_aware=false: sendLocation would discard the send anyway.
    logger.info('Force location skipped: no schedule and location_aware is false');
    return;
  }
...
const scheduleNextWindow = (schedule) => {
  const timezone = config.getData('timezone') || null;
  const { shouldSend } = checkSchedule(/** @type {any} */ (schedule), timezone);
  if (shouldSend) {
    // We may have entered the window while waiting (e.g. after DST) — start immediately.
    scheduleForceInWindow(schedule); // eslint-disable-line no-use-before-define
    return;
  }
  const msToStart = msUntilNextWindowStart(schedule, new Date(), timezone);
  if (msToStart === null) {
  ...
  // Cap at 1 h so a DST transition never causes us to miss a window start by more than 1 h.
  const msToWait = Math.min(msToStart, MAX_NEXT_WINDOW_CHECK_MS);
  logger.info(`Next force location window in ${Math.round(msToStart / 60000)} min, re-checking in ${Math.round(msToWait / 60000)} min`);
  forceTimeoutId = setTimeout(() => scheduleNextWindow(schedule), msToWait);
};
...
const scheduleForceInWindow = (schedule) => {
  ...
  // checkSchedule treats end_at as inclusive (the whole end_at minute is in-window), but
  // msUntilWindowEnd returns 0 on that minute. A 0 ms window-end timeout fires on the next
  // tick, scheduleNextWindow sees shouldSend still true, and re-enters scheduleForceInWindow —
  // flooding forceLocation every tick for the whole minute. Wait until the end of the end_at
  // minute (also guards against negative values from clock/DST drift).
...
exports.start = (opts, cb) => {
  ...
  config.onDataChange('timezone', restartForceInterval);
...
exports.stop = () => {
  ...
  config.offDataChange('timezone', restartForceInterval);
  hooks.remove('mac_address_changed');
  hooks.remove('get_location');
  hooks.remove('get_location_request');
  if (emitter) {
