...
var send_location = (type, location) => {
  ...
  if (control_panel.get_setting('location_aware')) return post_it(data);
...
var fetch_location = (type, callback) => {
  ...
  var trigger_event = (type, new_location) => {
    ...
    if (Object.keys(current).length == 0 || type == 'interval') {
      // There isn't a previous location or we're forcing it
    ...
    var better_loc = () => {
      var is_better = false;
      if (!current.accuracy && !!new_location.accuracy) is_better = true;
      else if (
        !!current.accuracy &&
        !!new_location.accuracy &&
        new_location.accuracy < current.accuracy
      )
        is_better = true;
      return is_better;
    };
