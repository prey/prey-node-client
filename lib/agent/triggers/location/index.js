...
const post_it = (data) => {
  ...
  devices.post_location(data, (err, state) => {
    ...
    if (state !== null && state !== controlPanel.get_setting('location_aware')) {
      controlPanel.update_setting('location_aware', state);
    }
  });
};
...
const send_location = (type, location) => {
  ...
  if (controlPanel.get_setting('location_aware')) return post_it(data);
  ...
  devices.get.status((err, resp) => {
    ...
    const aware = result.settings.local.location_aware;
    if (!aware) return;
    return post_it(data);
  });
};
