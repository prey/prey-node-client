...
const fetchLocation = (typeFetch, callback) => {
  ...
  const tryFetchLocation = (doneCallback, retryCount) => {
    callFetchLocation(doneCallback, (geoInfo) => {
      ...
      coords.method = 'wifi';
      coords.accuracy = 300;
      logger.info(`coords fetched: ${JSON.stringify(coords)}`);
      if (coords.accuracy) {
        logger.info(`IT HAS ACCURACY: ${coords.accuracy}`);
        if (coords.accuracy > accuracyThreshold) {
          logger.info(`accuracy ${coords.accuracy} is worse than threshold ${accuracyThreshold}`);
          if (typeFetch.localeCompare('client-start') === 0) {
            logger.info(`client start compared to threshold ${accuracyThreshold}`);
          ...
          logger.info('about to check mac-address or force');
          if (typeFetch.localeCompare('mac-address') === 0 || (typeFetch.localeCompare('force') === 0)) {
            logger.info(`mac-address or force compared to threshold ${accuracyThreshold}`);
            if ((retryCount + 1) < maxRetries) {
              setTimeout(() => {
                tryFetchLocation(doneCallback, retryCount + 1);
              }, timeToWaitForSecond);
