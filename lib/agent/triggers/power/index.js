...
var emitter,
    attempt = 0,
    checking = false,
    previous = /** @type {{ state?: string }} */ ({});

/**
 * @param {boolean} [first] when true, only seed `previous` without emitting.
 */
var check_battery_status = (first) => {

  // bypassCache: this read follows an OS power event, so it must reflect the
  // real current state — a stale cached value would silently miss the event.
  providers.get('battery_status', { bypassCache: true }, (err, current) => {
    // `current` can be undefined (e.g. the no-battery circuit-breaker path calls
    // back with no data); guard before dereferencing current.state below. Reset
    // the re-entrancy flags first: otherwise an errored/empty read after a
    // `state_changed` event would leave `checking` stuck true and silently ignore
    // every future power event.
    if (err || !current || !emitter) {
      checking = false;
      attempt = 0;
      return;
    }
    ...
    if ((previous.state == current.state || (previous.state != 'discharging' && current.state != 'discharging')) && os_name == 'mac') {
      if (attempt == 4) {
        checking = false;
        attempt = 0;
        return;
