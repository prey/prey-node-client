...
let emitter;
exports.timeoutGetStatusMs = 60 * 1000 * 3; // 3 minutes timeout
...
const checkBatteryStatus = (statusInBattery) => {
  ...
  try {
    battery_level.current = parseInt(statusInBattery.battery_status.percentage_remaining, 10);
  } catch (e) {
...
const verifyContent = (headerValue) => {
  ...
  keysToVerify.forEach((key) => {
    if (headerValue[key] !== null && typeof headerValue[key] === 'object') {
    ...
    timeoutGetStatus = setTimeout(() => {
      ...
      if (callbacksList.length >= 1) {
        callbacksList.forEach((element) => {
          if (element && element.cb && typeof element.cb === 'function') {
    ...
    if (callbacksList.length >= 1) {
      callbacksList.forEach((element) => {
        try {
          ...
          logger.error(`Error executing status callback: ${e}`);
        }
      });
    }
  });
...
exports.start = (opts, cb) => {
  ...
  hooks.on('disconnected', () => {
    exports.statusCallbacks = [];
    if (timeoutGetStatus) clearTimeout(timeoutGetStatus);
