...
var Tunnel = function(local_port, remote_host, remote_port){
  ...
  this.open = function(){
    ...
    try {
      keys = {
        key: fs.readFileSync(__dirname + '/../../test/ssl.key').toString(),
        cert: fs.readFileSync(__dirname + '/../../test/ssl.cert').toString()
      };
    } catch(e) {
      return this.closed(new Error("Unable to read SSL keys."));
    }
