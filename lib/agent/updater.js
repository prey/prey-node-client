...
var update_client = function (new_version, cb) {
    ...
    data
      ...
      .forEach(function (line) {
        ...
        if (line.match('YOUARENOTMYFATHER')) {
          ...
          common.packageController.update_version_attempt(
            common.version,
            new_version,
            false,
            true,
  ...
  child.on('exit', function (code) {
    let existsNewVersion = exists(join(versions_path, new_version));
    exports.upgrading = false;

    if (existsNewVersion && cb && typeof cb == 'function')
      return cb && cb(new Error('Version already installed'));
    var err;
    if (code != 0)
      err = new Error(
...
exports.check_for_update_winsvc = (cb) => {
  ...
  sys_win.get_winsvc_version((err, current_service_version) => {
    ...
    exports.get_stable_version_winsvc((err, service_version_stable) => {
      ...
      /** Perform the update. */
      exports.update_winsvc(
        updater_path + ' -v=' + current_service_version,
        (err_update) => {
          if (err_update)
            return cb(
              new Error('error to update winsvc,' + err_update.message)
            );
...
var check_for_update = function (cb) {
  ...
  common.packageController.new_version_available(
    branch,
    common.version,
    function (err, new_version) {
      if (err || !new_version) {
        common.packageController.check_update_success(
          common.version,
          versions_path,
          function (err) {
            return (
...
exports.check = function (id, target, opts, cb) {
  ...
  switch (target) {
    case 'reset':
      ...
      common.packageController.delete_attempts((err) => {
    ...
    case 'activate':
      ...
      common.packageController.activate_version(opts.version);
      done();
      break;
    ...
    case 'delete':
      ...
      common.packageController.delete_version(opts.version);
      break;

    case 'restart':
      // restart client
      logger.info('Restarting client');
      common.packageController.restart_client();
      done();
      break;
