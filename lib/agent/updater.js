...
var update_client = function(version, cb){
  ...
  if (process.platform == 'win32') {
    var conf_script = join(system.paths.package, 'lib', 'conf', 'cli.js');
    var bin_path    = join(system.paths.package, 'bin', 'node.exe');
    var args        = ['"' + conf_script + '"', 'config', 'upgrade'];
  } else {
    var bin_path    = system.paths.package_bin; // /foo/bar/bin/prey
    var args        = ['config', 'upgrade'];
  }
  ...
  child.on('exit', function(code) {
    exists(join(versions_path, version), function(exists) {
      ...
      var err = new Error('Upgrade to ' + version + ' failed.');
      err.stack = out.join('\n');
      return cb && cb(err);
    });
  });
