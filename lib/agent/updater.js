  ...
  if (process.platform == 'win32') {
    ...
    sys_win.get_winsvc_version((err, service_version) => {
      ...
      if (service_version && gte(service_version, "1.2.0")){
       ...
       else return cb(new Error("Error to get winsvc version, version is less than 1.2.0"));
      })
  }
  else {
    return cb(new Error("Only for Windows"))
    }
  }
...
var check_for_update = function(cb) {
  ...
  logger.info('Checking forcheck_update_success:' + err);
