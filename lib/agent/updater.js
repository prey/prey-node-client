  ...
  config.load(() => {
    ...
    common.package.new_version_available(branch, common.version, (err, newVersion, downloadsUrl) => {
      ...
      if (err || !newVersion) {
        ...
        needle.put(updatingHost, null, { timeout: 4500 }, (err) => {
          if (!err) return proceedWithUpdate();
          logger.warn('Local service notification failed, retrying in 3s: ' + err.message);
          setTimeout(() => {
            needle.put(updatingHost, null, { timeout: 4500 }, (retryErr) => {
              if (retryErr) logger.warn('Local service notification failed after retry: ' + retryErr.message);
              proceedWithUpdate();
            });
          }, 3000);
        });
