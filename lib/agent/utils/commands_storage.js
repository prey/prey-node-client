...
var queries = {                                                                    // UNIQUE?
  ...
  DELETE_KEY: (table, key)          => { return `DELETE FROM ${table} WHERE key = '${key}'` },
...
storage_fns.set = (opts, cb) => {
  init(opts.type, null, (err) => {
    ...
    console.log(queries.INSERT(opts.type, opts.data))
...
storage_fns.del_key = (opts, cb) => {
  init(opts.type, null, (err) => {
    db_comm.run(queries.DELETE_KEY(opts.type, opts.key), function(err) {
      var e = err && err.code == 'SQLITE_READONLY' ? SQLITE_ACCESS_ERR : err
      return cb && cb(e);
    })
  })
