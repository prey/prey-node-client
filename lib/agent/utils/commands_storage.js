...
var queries = {                                                                  
  CREATE: (table)              => { 
    // console.log("!!!",`CREATE TABLE IF NOT EXISTS ${table} (${types[table].schema})` )
...
var init = function(type, path, cb) {
  ...
  // if (first_init) {
  //   var existing_db = new sqlite3.Database(storage_path, function() {
  //     console.log("PRE RECOVER")
  //     recover_db(existing_db, (err) => {
  //       console.log("AFTER RECOVER")
  //       // if (err) logger.warn('Unable to recover old database data');
  //       first_init = false;
  //       return create_db();
  //     })
  //   });
...
var recover_db = function(db, cb) {
  ...
  get_tables((tables) => {
    ...
    async.series(array, (err) => {
      // console.log("holi!!!", Object.keys(Object.values(data)[0][0]))
      // console.log("chau!!!", Object.keys(Object.values(data)[0][0])[0])

      console.log("DATA!!!", data)

      if (Object.keys(Object.values(data)[0][0]).length > 1) {
        console.log("ID!!", Object.values(data)[0][0].id)
        return cb();
      }

      console.log("SIGUIÓ DE LARGO!!")

      // console.log("hwge", Object.keys(Object.values(data)[0][0])[0])
        ...
        array2.push((callback) => {
          ...
          db.all(queries.DROP(table), (err) => {
            db.run(queries.CREATE(table), () => {
              ...
              // if (data[table].length == 0) return callback();
              ...
              data[table].forEach((row) => {
                ...
                // if (table == 'keys')      id = Object.keys(value)[0].split('-')[0];
                if (table == 'versions')  id = Object.keys(value)[0].split('-')[1];

                console.log("HOLIs")

                exports.do('set', {type: table, id: id, data: db_data})
              });
  ...
  //   let tables_length = tables.length;

  //   let current_table = 0;

  //   if (tables_length == 0) return cb();
  ...
  //       // if (err) return cb(err);
  //       console.log("3", columns.length, table)
  //       // The table exists and has the old format

  //       console.log("OE YAPO!", current_table, tables_length)
  //       if (columns.length == 0 && current_table == tables_length) {
  //         console.log("CB TABLAS VACIAS")
  //         return cb();
  //       }

  //       if (columns.length > 1 && current_table == tables_length) {
  ...
  //             console.log("HOLI")
  //             // if (err) return cb(err);
  //             console.log("5 ABOUT TO CREATE!", table)
  ...
  //               if (rows.length == 0 && table_index == tables.length -1) {
  //                 return cb();
  //               }
  ...
  //                 // console.log("Defrgthy", table, current_table == tables_length, row_index == rows.length -1)
  //                 // console.log(current_table, tables_length)

  //                 exports.do('set', {type: table, id: id, data: data}, (err) => {
  //                   console.log("AFTER SET!!")
  //                   if (table_index == tables.length -1 && row_index == rows.length -1) {
  //                     console.log("RETRUN CB!!")
  //                     return cb();
  //                   }
  //                 });
  //               })
