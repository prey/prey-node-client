...
var types = {
  ...
  triggers: {
    schema: 'id TEXT PRIMARY KEY, name TEXT, synced_at INTEGER, last_exec INTEGER, automation_events TEXT, automation_actions TEXT',
    keys: ['id', 'name', 'synced_at', 'last_exec', 'automation_events', 'automation_actions'],
    values: (data) => { return `'${data.id}', '${data.name}', '${data.synced_at}', '${data.last_exec}', '${JSON.stringify(data.automation_events)}', '${JSON.stringify(data.automation_actions)}'` }
  },
  keys: {
    schema: 'key TEXT PRIMARY KEY, value TEXT',
    keys: ['key', 'value'],
    values: (data) => {return `'${data.key}', '${data.value}'`}
  }
  // },
  // versions: {
...
var queries = {                                                                    // UNIQUE?
  // CREATE: (table) => { return `CREATE TABLE IF NOT EXISTS ${table} (id TEXT PRIMARY KEY, command TEXT, target TEXT, options TEXT, started_resp INTEGER, started INTEGER, stopped_resp INTEGER, stopped INTEGER )` },
...
// Starting the client process we cant be totally sure about the state
// encryption.status[storage_path] = null;

var storage_queue = async.queue((task, cb) => {

  console.log("storage_queue")
  console.log("storage_queue")
  console.log("storage_queue")
  console.log(task)
  console.log("storage_queue")
...
exports.do = (perform, opts, cb) => {
  ...
  console.log("do")
  console.log("do")
  //clearTimeout(encryptTimeout);  // detener timeout para cifrado si es q está corriendo...
...
storage_fns.set = (opts, cb) => {
  ...
  // if (!type || Object.keys(types).indexOf(type) == -1 || !id || !data)
  //   return cb()
    
    if (opts.id) opts.data.id = opts.id
    // init(type, '', function(err) {
    // (table, id, command, target, options)

    // console.log("SETTT!", id, data)
    console.log("TO INSERT")
    console.log("TO INSERT")
    console.log("TO INSERT")
    console.log("TO INSERT")
    console.log("TO INSERT!",opts.type, opts.data )
    console.log("TO INSERT")
    console.log("TO INSERT")
    console.log("TO INSERT")
    console.log("TO INSERT")  
...
// exports.update = (type, id, columns, values, cb) => {
storage_fns.update = (opts, cb) => {
  // console.log("UPDATE!!", type, id, columns, values)
  // (table, id, changes)
  /*
  UPDATE table
...
// storage_fns.query = (type, column, data, cb) => {
storage_fns.query = (opts, cb) => {
  init(opts.type, null, (err) => {
    ...
    db_comm.all(queries.QUERY(opts.type, query), function(err, rows) {
      if (err) {
        console.log("error al consultar query")
        if (err.code != 'ENOENT')
          return cb(err, null);
...
exports.recover_db = function(db, cb) {
  ...
  // FALTA RETORNAR CB CUANDO TERMINE
