      ...
      if (line.includes(reqPreyConf[count].word)) {
        if (RegExp(reqPreyConf[count].regex).exec(line)) {
          if (reqPreyConf[count].regex.toString().includes('=')) {
            ...
            let dataToInsert = '';
            if (preyConfDataToWrite) dataToInsert = preyConfDataToWrite[reqPreyConf[count].name];
            else {
              dataToInsert = reqPreyConf[count].default || reqPreyConf[count].default === false ? reqPreyConf[count].default : '';
            }
    ...
    getDataDb('preyconf', (err, stored) => {
      ...
      if (stored && stored.length > 0) {
        storage.do('update', {
          ...
          if (errUpdate) logger.error(`Error while updating preyconf: ${errUpdate}`);
          return cb();
        });
      } else {
        storage.do('set', { type: 'keys', id: 'preyconf', data: { value: JSON.stringify(dataToSave) } }, (errSetting) => {
          if (errSetting) logger.error(`Error while setting preyconf: ${errSetting}`);
          return cb();
        });
      }
    });
