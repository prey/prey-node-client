...
var recover_db = function(db, cb) {
  var get_tables = (cb) => {
    db.all(queries.TABLES(), (err, data) => {
      ...
      tables = tables.filter((x) => { return !x.includes('autoindex') })
      return cb(tables);
    });
  }
  ...
  get_tables((tables) => {
    ...
    async.series(array, (err) => {
      // If there's only one column it has the old format
      let keys_stored = Object.keys(data); // tables (names) in the db: ['commands', 'files', ...]

      // No tables stored OR one of the tables has more than one column (new format)
      if (keys_stored.length == 0 || Object.keys(Object.values(data[keys_stored[0]])[0]).length > 1) {
        return cb();
      }
