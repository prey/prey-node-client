...
const storage_queue = async.queue((task, cb) => {
  storage_fns[task.perform](task.opts, (err, result) => {
    // Close the database connection after each operation
    if (dbComm) {
      dbComm.close((closeErr) => {
...
process.on('exit', () => {
  if (dbComm) {
    dbComm.close((err) => {
      if (err) console.error('Error closing database on exit:', err);
    });
  }
});

// Handle signals to close database properly
['SIGINT', 'SIGTERM', 'SIGHUP'].forEach((signal) => {
  process.on(signal, () => {
    if (dbComm) {
      dbComm.close((err) => {
        if (err) console.error(`Error closing database on ${signal}:`, err);
        process.exit(0);
      });
    } else {
      process.exit(0);
