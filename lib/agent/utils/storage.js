...
var types = {
  ...
  responses: {
    schema: 'id TEXT PRIMARY KEY, action_id TEXT, status TEXT, action TEXT, time INTEGER, retries INTEGER',
    keys: ['id', 'action_id', 'status', 'action', 'time', 'retries'],
    values: (data) => { return `'${data.id}', '${data.action_id}', '${data.status}', '${data.action}' , '${data.time}', 0` }
  },
  geofences: {
...
var recover_db = function(db, cb) {
  ...
  get_tables((tables) => {
    ...
    db.all(queries.INFO(tables.find(element => element == 'responses')), (err, columns) => {
      ...
      db.all(queries.CREATE('responses'), function(err, rows) {});
    });
