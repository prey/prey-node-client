...
const deleteUnwantedCharacters = (textToAnalyze) => {
  if (typeof(textToAnalyze) === 'string'){
    return textToAnalyze.replace("'", "");
  }
  else if (typeof(textToAnalyze) === 'object'){
    for (const key in textToAnalyze) {
      textToAnalyze[key] = deleteUnwantedCharacters(textToAnalyze[key]);
    }
  ...
  commands: {
    ...
    values: (data) => {
      return `'${data.id}', '${data.command}', '${data.target}', '${JSON.stringify(
        deleteUnwantedCharacters(data.options)
      )}' , 0, 'NULL', 0, 'NULL'`;
    }
  },
  geofences: {
  ...
  triggers: {
    ...
    values: (data) => {
      return `'${data.id}', '${data.name}', '${data.persist}', '${data.synced_at}', '${
        data.last_exec
      }', '${JSON.stringify(deleteUnwantedCharacters(data.automation_events))}', '${JSON.stringify(deleteUnwantedCharacters(data.automation_actions))}'`;
    }
  },
  keys: {
