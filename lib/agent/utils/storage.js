...
var save = function(type, obj_add, cb) {
  var stmt = db_comm.prepare(queries.INSERT(type));
  var add  = new Buffer(JSON.stringify(obj_add, null, 0)).toString('base64');
...
var drop = function(type, obj_del, cb) {
  var del  = new Buffer(JSON.stringify(obj_del, null, 0)).toString('base64');
...
exports.del = function(key, cb) {
  check_type(key, function(err, type) {
    ...
    load(type, function(err, db) {
      ...
      return found ? drop(type, obj_del, cb) : cb && cb();
    })
  });
}

exports.update = function(key, obj_del, obj_add, cb) {
  check_and_init(key, function(err, type) {
    if (err) return cb && cb(err);

    drop(type, obj_del, function(err) {
      if (err) return cb() && cb(err);
      save(type, obj_add, cb);
    })
  });
