...
var save_geo = function(type, add, cb) {
  ...
  stmt.run(add, function(err) {
    var e = err && err.code == 'SQLITE_READONLY' ? 'Access denied to db, must run as prey user' : err
    stmt.finalize();
    return cb() && cb(e);
  });
}
...
exports.all = function(type, cb) {
  console.log("ALL")
  var path = join(common.system.paths.config, 'commands.db');
  console.log("TYPE:", type)
  init(type, path, function(err) {
