...
const logger = require('../../common').logger.prefix('utilstorage');
  ...
  storage.do('query', { type: 'keys', column: 'id', data: 'wifiDataStored' }, (err, stored) => {
    ...
    if (stored && stored.length > 0) {
      ...
      try {
        ...
        logger.error(`Error parsing wifi data JSON: ${parseError}`);
        return;
      }
...
const retrieveDataWifi = (callback) => {
  storage.do('query', { type: 'keys', column: 'id', data: 'wifiDataStored' }, (err, stored) => {
    if (err || !stored || stored.length === 0) {
      return callback('');
    }
    return callback(stored[0].value);
  });
};
