var shared = {"panel":{"authorize":function(opts, cb) {                  return cb(new Error('panel.authorize called with opts: ' + JSON.stringify(opts)))                }}};
...
exports.signup = function(values, cb) {
  ...
  var accept_opts = ['yes', 'y', 'true'],
      terms_check = values['-t'] ? (accept_opts.indexOf(values['-t'].toLowerCase()) == -1 ? false : true) : false,
      age_check   = values['-a'] ? (accept_opts.indexOf(values['-a'].toLowerCase()) == -1 ? false : true) : false;
