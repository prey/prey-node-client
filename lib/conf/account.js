...
const setApiKeyAndDeviceKeyRegister = (keys, cb) => {
  // ensure the plugin is loaded if registration succeeds
  const success = () => cb();

  // eslint-disable-next-line consistent-return
  shared.keys.set_api_device_key({ api: keys.api_key, device: keys.device_key }, (errApiDevice) => {
...
exports.authorize = (values, cb) => {
  ...
  shared.panel.authorize(opts, (err, key) => {
    ...
    if (opts.device_key && opts.device_key !== '') return setApiKeyAndDeviceKeyRegister(opts, cb);
...
exports.signup = (values, cb) => {
  ...
  const terms_check = values['-t'] ? (accept_opts.indexOf(values['-t'].toLowerCase()) != -1) : false;
  const age_check = values['-a'] ? (accept_opts.indexOf(values['-a'].toLowerCase()) != -1) : false;
...
exports.setup = (values, cb) => {
  ...
  if (shared.keys.is_api_key_set() && !run_again) return cb(new Error('Account already set up! Run with -f/--force to continue anyway.'));

  shared.panel.setup('control-panel', (err) => {
    if (err) return cb(err);

    log('Credentials verified.');
    shared.keys.set_device_key('', (err) => {
      if (err) return cb(err);
