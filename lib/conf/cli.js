...
if (process.env.BUNDLE_ONLY)
  // @ts-ignore — top-level return is valid in CommonJS but not recognized by tsc
  return process.exit();
...
const { ensureAndCreateDb } = require('../agent/startup');
    ...
    config.load(() => {
      ...
      cli.command('account', 'Prey account management.', function(sub) {
        ...
        sub.command('authorize', 'Validates auth credentials, and stores API key if authorized.', function(cmd) {
          ...
          cmd.parameters(['-d', '--device-key'], 'DEVICE_KEY')
      ...
      cli.command('hooks', 'Pre/post installation hooks.', function(sub) {
        ...
        sub.command('set_watcher', 'Sets up a parallel process that restores the prey user.', function(cmd) {
          ...
          if (process.platform != 'darwin') {
            return shared.log('Service available only for macOS');
          }
          ...
          run(cmd, (values, cb) => daemon.set_watcher(cb));
        });
      ...
      cli.command('skipped', 'View or modify Prey settings', function(sub) {
        sub.command('list', 'Shows all settings info for skipped permissions.', function(cmd) {
          run(cmd, skipped.list);
        });

        sub.command('read', 'Returns value for specified permission.', function(cmd) {
          cmd.keyword('key', 'Key to look for.');
          run(cmd, skipped.read);
        });

        sub.command('update', 'Updates value for a specific permission.', function(cmd) {
          cmd.keyword('key', 'Key to replace.');
          run(cmd, skipped.update);
        });
      ...
      cli.command('set_debug', 'Set debug environment variable.', function(cmd) {
        cmd.keyword('value', 'Value to set to debug.');
        if (process.getuid && process.getuid() !== 0) {
          const line = new Array(80).join('=');
          const msg  = `\n  You are running this script as an unprivileged user. Please take the necessary steps to run it with elevated rights.`;
          shared.log(line + msg + line);
          return process.exit(0);
        }
        run(cmd, log.envFileCreate);
      });

      cli.command('send_action', 'Send action to localhost for Prey to run it.', function(cmd) {
        cmd.keyword('key', 'Key to look for.');
        if (process.getuid && process.getuid() !== 0) {
          const line = new Array(80).join('=');
          const msg  = `\n  You are running this script as an unprivileged user. Please take the necessary steps to run it with elevated rights.`;
      ...
      cli.command('update_switcher', 'Update Prey sudoers file (creates version 51, removes version 50).', function(cmd) {
        if (process.platform != 'linux') {
          return shared.log('Only available on Linux/Ubuntu');
        }

        if (process.getuid && process.getuid() !== 0) {
          var line = new Array(80).join('=');
          var msg  = '\n  You are running this script as an unprivileged user';
              msg += '\n  so we cannot update the sudoers configuration.';
              msg += '\n  Please run this command with elevated rights:\n';
              msg += '\n  $ sudo prey config update_switcher\n';

          shared.log(line + msg + line);
          return process.exit(0);
        }

        run(cmd, switcher.update);
      });
