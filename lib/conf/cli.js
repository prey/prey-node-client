...
require(path.join(__dirname, '..', 'utils', 'exit'));
...
var retrieve_old_keys = function(file, cb) {
...
var set_api_key_and_register = function(key, cb) {
  var setup_opts = { config: common.config };

  settings.set_api_key(key, function(err){
    if (err) return cb(err);
    common.setup(setup_opts, function(err) {
      if (err)
        return settings.set_api_key('', cb);
...
var show_gui_and_exit = function(force) {
...
var run = function (cb) {
  ...
  cli.command('upgrade', 'Downloads and installs greater Prey version, if any.', function(cmd){
    cmd.parameters(['-v', '--version'], 'Version to install. Defaults to latest.');
    cmd.start(function(values) {
      ...
      if (values['-v'])
        return pacekage.get_version(values['-v'], destination, done);
  ...
  cli.command('account', 'Prey account management', function(sub){
    ...
    sub.command('authorize', 'Validates auth credentials, and stores API key if authorized.', function(cmd){
      ...
      cmd.start(function(values){
        ...
        api.accounts.authorize(opts, function(err, key) {
          if (err || !key)
      ...
      cmd.start(function(){
        verify_current_keys(function(err){
          if (err) // invalid keys, so just continue
            return hooks.pre_uninstall(cb);

          api.devices.unlink(function(err) {
            if (!err) // all good, so lets clear current device key
              config.update('device_key', '');
  ...
  cli.command('gui', 'Opens GUI configurator to set up account.', function(cmd){
    ...
    cmd.start(function(values){
      ...
      if (old_config && old_config != '') {
        ...
        retrieve_old_keys(old_config, function(err) {
          if (!err)
            console.log('Configuration restored from previous client!');
