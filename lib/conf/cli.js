...
var activate_new_version = function(version, cb){
  ...
  exec(version_bin + ' config activate', function(err){
    if (!err) return cb();

    console.log('Failed. Rolling back!');

    // something went wrong while upgrading.
    versions.remove(new_version, function(e){
      cb(e || err);
    });
  });
}
...
//        versions.set_current(version || 'this', cb);
  ...
  cli.command('account', 'Prey account management', function(sub){
    ...
    sub.command('authorize', 'Validates credentials, and stores API key if valid.', function(cmd){
    ...
    sub.command('verify', 'Verifies API/Device keys, either current or passed via command.', function(cmd){
      cmd.options(['-c', '--current'], 'Check current ones.')
      cmd.options(['-u', '--update'], 'Update values in config, if valid (no effect with --current).')
      cmd.parameters(['-a', '--api-key'], 'API Key.')
