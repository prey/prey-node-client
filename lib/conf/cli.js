...
// the keys module requires ./agent/common, so we need to override
// the program.logfile param to avoid stepping over the running agent's log
common.program.logfile = system.tempfile_path('prey-config.log');
var keys = require('./../agent/keys');
...
var kill_if_running = function(cb) {
  ...
  pidfile.read(common.pid_file, function (err, running) {
    if (err || !running) return cb();

    log('Process still running! Sending ' + signal + ' to PID ' + running.pid);

    var e;
    try { process.kill(running.pid, signal) } catch (ex) { e = ex }
    return cb(e);
  });
}
...
//        versions.set_current(version || 'this', cb);
  ...
  cli.command('hooks', 'Pre/post installation hooks.', function(sub){
    ...
    sub.command('pre_uninstall', 'Runs pre uninstallation hooks.', function(cmd){
      cmd.start(function(){
        hooks.pre_uninstall(function(err){
          if (err) return cb(err);
