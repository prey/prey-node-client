...
function run (callback) {
  ...
  var cli_controller = require('./cli_controller')(log, callback);
   ...
   *
   * @summary Opens GUI configurator to set up account
   */
  ...
  /**
   * @map     [./bin/prey] config hooks
   *
   * @summary Display the hooks menu
   */
  cli.command('hooks', 'Pre/post installation hooks.', function (sub) {
    /**
     * @map     [./bin/prey] config hooks post_install
     *
     * @summary Called after the files have been copied, loads prey services
     */
    ...
    /**
     * @map     [./bin/prey] config hooks pre_install
     *
     * @summary Unloads prey service and removes .plist file
     */
  ...
  cli.command('gui', 'Opens GUI configurator to set up account.', function (cmd) {
    cmd.start(function () {
      cli_controller.show_gui_and_exit();
    });
  });
