 ...
 *
 * @summary  Returns current active (symlinked) version
 *
 * @api      public
 */
controller.version_current = function () {
 ...
 *
 * @summary  Returns local version from which script was called
 *
 * @api      public
 */
controller.version_this = function () {
 ...
 *
 * @summary  Shows list of installed versions
 *
 * @api      public
 */
controller.version_list = function () {
 ...
 *
 * @summary  Symlinks [version], setting it as the active one
 *
 * @api      public
 */
...
/**
 * @map      [./bin/prey] config settings list
 *
 * @summary  Shows all available settings in config file
 *
 * @api      public
 */
controller.settings_list = function () {
  if (!settings.present())
    return callback(new Error('Config file not found!'));
  return log(settings.list());
}

/**
 * @map      [./bin/prey] config settings read
 *
 * @summary  Returns value for specified setting
 *
 * @api      public
 */
controller.settings_read = function (values) {
  if (!settings.present())
    return callback(new Error('Config file not found!'))

  var key = values.positional[0];
  if (!key) return callback(new Error('Key required.'));
...
/**
 * @map      [./bin/prey] config settings update
 *
 * @summary  Updates setting in config file
 *
 * @api      public
 */
controller.settings_update = function (values) {
  if (!settings.present())
    return callback(new Error('Config file not found!'))
  ...
  if (!val) return callback(new Error('Please provide a value.'))
  ...
  return settings.update(key, val, callback);
}

/**
 * @map      [./bin/prey] config settings toggle
 *
 * @summary  Toggles boolean setting
 *
 * @api      public
 */
controller.settings_toggle = function (values) {
  if (!settings.present())
    return callback(new Error('Config file not found!'))
  ...
  if (typeof val != 'boolean')
    return callback(new Error(key + ' is not boolean.'));

  settings.update(key, !val, updated_settings);

  function updated_settings (err) {
    return callback(err,
              key + ' toggled: ' + val.toString()
              + ' -> ' + (!val).toString());
  }
}

/**
 * @map      [./bin/prey] config check
 *
 * @summary  Verifies that current installation is fine and dandy
 *
 * @api      public
 */
controller.check = function () {
  ...
  if (!settings.present())
    return callback(new Error('Config file not present!'))

  // if we have versions support, check if symlinked
  if (paths.versions && !fs.existsSync(paths.current))
    return callback(new Error('Current version not set in ' + paths.current))

  // check that prey bin exists
  if (!fs.existsSync(paths.current_bin))
    return callback(new Error('Prey bin not found in ' + paths.current_bin))
  ...
  if (!keys.api_key || keys.api_key == '')
    ...
    return callback(new Error('Device Key not found! Run Prey to register device.'))

  remote.verify(keys, verified_keys);

  function verified_keys (err) {
    if (err) return callback(err);
    return callback();
  }
}
...
/**
 * @map      [./bin/prey] config run
 *
 * @summary  Runs Prey. Registers device if API is set but no Device key
 *
 * @api      public
 */
