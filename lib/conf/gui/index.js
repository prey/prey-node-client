...
exports.show_and_exit = function(values, cb) {

  var args     = [],
      force    = values && values['-f'] === true,
  ...
  if (os_name == 'windows') {
    ...
    var mavericks_or_newer = parseFloat(release()) >= 13;
  ...
  var show = function() {
    shared.keys.verify_current(function(err) {
      ...
      if (err && err.code == 'INVALID_CREDENTIALS')
        shared.keys.set_api_key(''); // clears both API and device keys

      if (err && err.code == 'INVALID_DEVICE_KEY')
        shared.keys.set_device_key(''); // invalid, so clear it out

      exports.show_and_exit(force);
    })
  }
