...
const run_synced = require('./utils/run_synced');
...
var activate_new_version = function(version, cb) {
  ...
  run_synced(bin, args, opts, function(err, code) {
    ...
    // remove new packageController & undo pre_uninstall
...
// install local packageController into version path
exports.local = function(values, cb) {
  ...
  packageController.install(file, destination, function(err, new_version) {
...
exports.remote = function(values, cb) {
  ...
  if (wants_version && wants_version != 'edge' && wants_version != 'stable')
    return packageController.get_version(wants_version, destination, done);
  ...
  packageController.get_latest(branch, latest_installed, destination, done);
}
