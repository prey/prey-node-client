...
exports.remote = function(values, cb) {
  ...
  var done = function(err, new_version) {
    ...
    setTimeout(function() {
      ...
      // on macOS, reduce the delay significantly because activate() handles daemon unload/reload,
      // and we need to activate before WatchPaths triggers a restart.
      // the max unload time is 10 seconds so on other platforms we'll wait a tiny bit further.
      const activateDelay = process.platform === 'darwin' ? 2000 : 11111;
      setTimeout(function() {
