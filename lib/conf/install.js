...
const { restore } = require('../agent/utils/storage/restore');
...
const dbExists = () => {
  if (fs.existsSync(join(paths.install, 'commands.db'))) return paths.install;
  return paths.config;
};

const changeLocationDb = (values, cb) => {
  let destinationPath = "";
  if (!values['-p']) return cb();
  destinationPath = values["positional"][0];
  console.log(JSON.stringify(values));
  console.log(destinationPath);
  const dbDestination = destinationPath.localeCompare('install') === 0 ? paths.install : paths.config;
  const dbAlreadyPath = dbExists();
  if (dbAlreadyPath.localeCompare(dbDestination) === 0) return cb(new Error(`DB is in ${dbDestination} already`));
  fs.copyFileSync(dbAlreadyPath, join(paths.temp, 'prey', 'commands.db'))
  restore((errorDB) => {
    if (errorDB) return cb(errorDB);
    exec('taskkill /f /im wpxsvc.exe', () => {
      exec(`wmic process where "ExecutablePath='${paths.install}\\current\\bin\\node.exe'" delete`, () => {
        fs.unlink(dbAlreadyPath, (errorUnlinked) => {
          if (errorUnlinked) return cb(new Error(`There was an error trying to delete db in ${dbAlreadyPath}. ${errorUnlinked}`));
          cb();
        });
      });
    });
...
exports.changeLocationDb = changeLocationDb;
