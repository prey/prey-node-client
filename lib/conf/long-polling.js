...
var request = function() {
  ...
  function attach_listeners(request) {
    request.on('data', function(data) {
      ...
      if (data.api_key) {
        logger.info("llego data.api_key y voy a decrypt and notify");
    ...
    request.request.on('error', function(err) {
      clear_current_request();
      if (options.proxy) {
        logger.info('Trying to reconnect without proxy');
  ...
  function check_for_reschedule() {
    ...
    if (schedule) {
      logger.info("Re-scheduling request");
      request();
    }
  }
};
...
function clear_current_request() {
  if (current_request) {
    logger.info("Clearing current request");
  if (current_request.request) {
    logger.info("Aborting current request");
