...
const secure  = require('./../agent/plugins/control-panel/secure');
const logger  = common.logger.prefix('new-config');

const current_request = undefined;
...
const request = function() {
  ...
  const request_start = undefined;
  const response_delay = undefined;
  const resp_timeout = undefined;

  const domain_conf = common.config .get('control-panel.host');
  const protocol_conf = common.config .get('control-panel.protocol');

  logger.info(`TESTING domain ${domain_conf}`);
  logger.info(`TESTING protocol ${protocol_conf}`);

  const protocol = protocol_conf;
  const host = domain_conf;
  const base = `${protocol}://${host}`;
  const url  = `${base}/api/v2/devices/sso_config/${encoded_public_key}`;
  ...
  function attach_listeners(request) {
    request.on('data', function(data) {
      ...
      if (Array.isArray(data) && data.length == 0) {
        ...
        setTimeout(check_for_reschedule, 2 * 1000);
      }
