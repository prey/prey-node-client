...
var request = function(re_schedule) {
  ...
  var code = new Buffer(encoded_public_key).toString('base64');
  
  var base  = protocol + '://' + host,
      // url   = base + '/api/v2/devices/client_configuration';
      url   = base + '/api/v2/devices/sso_config/' + code;
  ...
  function attach_listeners(request) {
    request.on('data', function(data) {
      ...
      util.isBuffer(data) ? data = { api_key: data.toString().split('[')[1].split(']')[0] } : data;
