...
var releases_url     = 'http://s3.amazonaws.com/prey-releases/',
...
package.install = function (zip, dest, cb) {
  ...
  // Don't load at startup to avoid fstream
  var unzip = require('./../utils/unzip');
  unzip(zip, dest, unzippedFile);

  function unzippedFile (err) {
    if (err) return cb(err);

    var version     = path.basename(zip).match(/([\d\.]+)/)[1],
        new_path    = path.join(dest, 'prey-' + version),
        final_path  = path.join(dest, version);
    fs.rename(new_path, final_path, renamedFile);
  }

  function renamedFile (err) {
    if (err) return cb(err);
