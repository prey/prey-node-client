...
package.download_release = function(version, cb){
  var package_url = releases_url + version + '/prey-' + version + package_format;
...
package.install = function(zip, dest, cb){
  ...
  unzip(zip, dest, function(err){
    ...
    var version = path.basename(zip).match(/([\d\.]+).zip/)[1],
        new_path = path.join(dest, 'prey-' + version);

    fs.rename(new_path, path.join(dest, version), function(err){
      cb(err, version);
    })
  });
}
