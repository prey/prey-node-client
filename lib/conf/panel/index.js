var join          = require('path').join,
    ...
    long_polling  = require(join(control_panel, 'long-polling')),
...
var show = function() {
  ...
  secure.open_config(device_key, function(err) {
    ...
    if (err) {
      log("Error launching panel config: " + err)
      gui.check_and_show([]);
    }
  });
}
...
exports.check_and_show = function(values, cb) {
  ...
  shared.keys.verify_current(function(err) {
    ...
    if (device_key)
      force ? force_new_config() : log("Device already set! Run with -f/--force to reconfigure this device.");
