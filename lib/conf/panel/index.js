...
var show = function() {
  if (os_name == 'linux') return gui.check_and_show([]);
...
exports.check_and_show = function(values) {
  ...
  exports.reset_old_keys(reset, function(err) {
    ...
    shared.keys.verify_current(function(err,out) {
      if (err) {
        if (err.code == 'INVALID_CREDENTIALS' || err.code == 'INVALID_DEVICE_KEY') {
          exports.force_new_config();
        }
      } else {
        log('Valid existing keys found. Proceeding anyway.')
        exports.device_key = shared.keys.get().device;
        force || reset ? exports.force_new_config() : log("Device alrseady set! Run with -f/--force to reconfigure this device.");
      }
