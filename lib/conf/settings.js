...
const listInmutable = [
  'control-panel.send_location_on_connect',
  'control-panel.permissions.wifi_location',
  'control-panel.permissions.native_location'];
...
exports.update = (values, cb) => {
  const key = values.positional[0];
  if (!key) { return cb(new Error('Key required.')); }
  if (listInmutable.includes(key)) {
    return cb(new Error(`The key \`${key}\` is not modifiable.`));
  }
