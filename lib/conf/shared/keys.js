...
exports.set = (keys, cb) => {
  ...
  if (keys.api)
    config.set('control-panel.api_key', keys.api.api_key);

  if (typeof keys.device !== 'undefined' && keys.device !== '')
    config.set('control-panel.device_key', keys.device.device_key);
  config.save(cb);
};
