...
function debug_log(msg) {
  try {
    ...
    fs.appendFileSync(debug_log_path, '[' + ts + '] [version_manager] ' + msg + '\n');
  } catch(e) {}
}
...
versions.set_current = function (version, cb) {
  debug_log('set_current() called with version=' + version);
  debug_log('  paths.versions=' + paths.versions + ', paths.current=' + paths.current);

  if (!paths.versions) {
    debug_log('  paths.versions is empty, returning early');
    return cb();
  }
  ...
  if (osName !== 'mac' && versions.current() == version) {
    ...
    debug_log('  ALREADY_CURRENT, returning error');
    return cb(error);
  }

  var full_path = get_version_path(version);
  let exists = fs.existsSync(full_path)
  debug_log('  full_path=' + full_path + ', exists=' + exists);
  ...
  if (osName === 'mac' && can_symlink()) {
    debug_log('  STRATEGY: macOS copy (fresh inodes)');
    ...
    debug_log('  tmp_copy=' + tmp_copy);
    debug_log('  old_current=' + old_current);

    // Copy entire directory to create new inodes for all binaries
    var cp_start = Date.now();
    debug_log('  cp_r START: ' + full_path + ' -> ' + tmp_copy);
    cp_r(full_path, tmp_copy, function(err) {
      var cp_dur = Date.now() - cp_start;
      if (err) {
        debug_log('  cp_r FAILED after ' + cp_dur + 'ms: ' + err.message);
        return cb(err);
      }
      debug_log('  cp_r DONE in ' + cp_dur + 'ms');
      ...
      debug_log('  rename #1 START: ' + paths.current + ' -> ' + old_current);
      var rename1_start = Date.now();
      fs.rename(paths.current, old_current, function(err) {
        var rename1_dur = Date.now() - rename1_start;
        if (err && err.code !== 'ENOENT') {
          debug_log('  rename #1 FAILED after ' + rename1_dur + 'ms: ' + err.message + ' (code=' + err.code + ')');
          rmdir(tmp_copy, function() {});
          return cb(err);
        }
        debug_log('  rename #1 DONE in ' + rename1_dur + 'ms' + (err ? ' (ENOENT, no previous current)' : ''));

        debug_log('  rename #2 START: ' + tmp_copy + ' -> ' + paths.current);
        var rename2_start = Date.now();
        fs.rename(tmp_copy, paths.current, function(err) {
          var rename2_dur = Date.now() - rename2_start;
          if (err) {
            debug_log('  rename #2 FAILED after ' + rename2_dur + 'ms: ' + err.message);
            // Try to restore old current before reporting error
            fs.rename(old_current, paths.current, function() {});
            rmdir(tmp_copy, function() {});
            return cb(err);
          }
          debug_log('  rename #2 DONE in ' + rename2_dur + 'ms');

          log('Successfully created new copy of version ' + version + ' with fresh inodes');
          debug_log('  SUCCESS: new copy of version ' + version + ' is now current');

          // Clean up old current in background (don't block activation)
          debug_log('  cleanup START: remove_path(' + old_current + ')');
          remove_path(old_current, function() {
            debug_log('  cleanup DONE: ' + old_current + ' removed');
          });
          cb();
        });
      });
