...
versions.set_current = function (version, cb) {
  ...
  if (osName === 'mac' && can_symlink()) {
    ...
    cp_r(full_path, tmp_copy, function(err) {
      ...
      // Remove existing current (may be a symlink or directory from previous install)
      remove_current(function(err) {
...
// Removes current path whether it's a symlink or a directory
function remove_current(cb) {
  fs.lstat(paths.current, function(err, stat) {
    if (err) return cb(err);
    if (stat.isSymbolicLink()) {
      fs.unlink(paths.current, cb);
    } else if (stat.isDirectory()) {
      rmdir(paths.current, cb);
    } else {
      fs.unlink(paths.current, cb);
    }
  });
