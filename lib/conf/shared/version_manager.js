...
versions.set_current = function (version, cb) {
  ...
  if (can_symlink()) {
    // Atomic swap: create a temporary symlink then rename over the existing one.
    // fs.rename() is atomic on POSIX, so there's no window where the path doesn't exist.
    // This prevents launchd WatchPaths from seeing a missing symlink during updates.
    var tmp_link = paths.current + '.tmp_' + process.pid;
    fs.symlink(full_path, tmp_link, 'junction', function(err) {
      if (err) return cb(err);
      fs.rename(tmp_link, paths.current, function(err) {
        if (err) {
          // Clean up temp symlink if rename fails
          fs.unlink(tmp_link, function() { cb(err); });
          return;
        }
        cb();
      });
