...
const sysPaths = require('../system/paths');
...
// Wrapper used on sudo-rs systems (Ubuntu 26+) — avoids wildcard in sudoers args.
// Use the stable 'current' symlink so the path survives auto-updates to new versions.
const PREY_SU_WRAPPER = path.join(sysPaths.current, 'lib', 'system', 'linux', 'prey-su');
 ...
 */
const isSudoRs = (callback) => {
  exec('sudo -V', { timeout: 5000 }, (err, stdout, stderr) => {
    const output = (stdout || '') + (stderr || '');
    callback(output.toLowerCase().includes('sudo-rs'));
  });
};
...
const createNewFile = (additionalCommands, callback) => {
  ...
  fs.access(SUDOERS_FILE_51, fs.constants.F_OK, (err) => {
    ...
    // Build sudoers line: on sudo-rs (Ubuntu 26+) wildcards are unsupported,
    // so use the prey-su wrapper instead of the 'su [A-z]*' wildcard entries.
    isSudoRs((sudoRs) => {
      let sudoersArgs;

      if (sudoRs) {
        if (!fs.existsSync(PREY_SU_WRAPPER)) {
          shared.log(`ERROR: prey-su wrapper not found at: ${PREY_SU_WRAPPER}`);
          return callback(new Error(`prey-su wrapper not found at: ${PREY_SU_WRAPPER}`));
        }
        shared.log('sudo-rs detected: using prey-su wrapper instead of su wildcard entries');
        sudoersArgs = additionalCommands.concat([PREY_SU_WRAPPER]).join(', ');
      } else {
        sudoersArgs = SUDOERS_ARGS;
...
/**
 * On sudo-rs systems (Ubuntu 26+), remove v51 sudoers file if it contains
 * wildcard entries incompatible with sudo-rs, so createNewFile can regenerate it.
 */
const migrateWildcardFile = (callback) => {
  isSudoRs((sudoRs) => {
    if (!sudoRs) return callback(null);
    shared.log('sudo-rs detected: removing');
    exec(`rm -rf "${SUDOERS_FILE_51}"`, { timeout: 10000 }, (rmErr) => {
      if (rmErr) {
        return callback(new Error(`Failed to remove incompatible sudoers file: ${rmErr.message}`));
      }
      return callback(null);
    });
  });
...
exports.update = (values, callback) => {
  ...
  getAdditionalCommands((additionalCommands) => {
    ...
    removeOldFile((removeErr) => {
      ...
      // Step 3: On sudo-rs, remove v51 if it has incompatible wildcard entries.
      // Even if removal fails, we proceed with createNewFile — the file will
      // be recreated below and any removal error is logged for the operator.
      migrateWildcardFile((migrateErr) => {
        if (migrateErr) {
          shared.log(`WARNING: could not remove old sudoers file: ${migrateErr.message}`);
        }

        // Step 4: Create new file (version 51)
        createNewFile(additionalCommands, (createErr, created) => {
          ...
          // Step 5: Test impersonation (optional, won't fail if test fails)
