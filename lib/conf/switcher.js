...
const getAdditionalCommands = (callback) => {
  ...
  const checkTool = (tool) => {
    exec(`which ${tool.cmd}`, (err, stdout) => {
      if (!err && stdout.trim()) {
        results.push({
          path: stdout.trim(),
          order: tool.order
...
const removeOldFile = (callback) => {
  fs.access(SUDOERS_FILE_50, fs.constants.F_OK, (err) => {
    ...
    exec(`rm -rf "${SUDOERS_FILE_50}"`, (error) => {
      if (error) {
...
const createNewFile = (additionalCommands, callback) => {
  ...
  fs.access(SUDOERS_FILE_51, fs.constants.F_OK, (err) => {
    ...
    exec('mkdir -p /etc/sudoers.d', (mkdirErr) => {
      if (mkdirErr) {
      ...
      exec('grep -q "^#includedir.*/etc/sudoers.d" /etc/sudoers || echo "#includedir /etc/sudoers.d" >> /etc/sudoers', (grepErr) => {
        if (grepErr) {
...
const testImpersonation = (callback) => {
  ...
  exec("cat /etc/passwd | grep -E 'home.*bash' | tail -1 | cut -d':' -f1", (err, stdout) => {
    if (err || !stdout.trim()) {
