...
const migrateWildcardFile = (callback) => {
  isSudoRs((sudoRs) => {
    ...
    fs.access(SUDOERS_FILE_51, fs.constants.F_OK, (err) => {
      ...
      fs.readFile(SUDOERS_FILE_51, 'utf8', (readErr, content) => {
        if (readErr || !content.includes('[A-z]*')) {
          return callback(null);
        }

        shared.log('sudo-rs detected: removing sudoers file with incompatible wildcard entries');
