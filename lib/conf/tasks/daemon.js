...
if (is_windows) {
  ...
  if (process.platform == 'linux') {
    ...
    // upstart: start on startup and when exited with code 0, only if not shutting down
    daemon_opts.up_start_on = 'startup or (stopped prey-agent EXIT_STATUS=0 and runlevel [2345])';

    // sleep 15 seconds after stopped, before restarting (e.g. when not stopped by upstart itself) 
