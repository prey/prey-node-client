...
if (is_windows) {
  ...
  if (process.platform == 'linux') {
    ...
    daemon_opts.start_on = 'startup or (stopped prey-agent EXIT_STATUS=0 and runlevel [2345])';

    // when process stops with error code 0, restart after 15 seconds
    daemon_opts.pre_stop_script  = 'echo "1" > /tmp/stopped-${JOB}';
    daemon_opts.post_stop_script = '[ ! -f "/tmp/stopped-${JOB}" ] && sleep 15\n  rm -f /tmp/stopped-${JOB}';
