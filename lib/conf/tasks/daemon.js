...
var run = function (cmd, cb) {
  exec(cmd, { timeout: 15000 }, cb);
};

var INSTALL_LOG = '/tmp/installation_prey.log';
var log_install = function (msg) {
  try {
    var ts = new Date().toISOString().replace('T', ' ').replace('Z', '');
    fs.appendFileSync(INSTALL_LOG, '[' + ts + '] [daemon] ' + msg + '\n');
...
exports.install = function (cb) {
  log_install('daemon.install: START');
  log_install('  key=' + (daemon_opts && daemon_opts.key) + ' bin=' + (daemon_opts && daemon_opts.bin) + ' user=' + (daemon_opts && daemon_opts.user));
  var next = function (err) {
    ...
    if (err && err.code !== 'EBUSY') {
      log_install('daemon.install next: FAILED (pre-ensure): ' + err);
      return cb(err);
    }
    ...
    satan.ensure_created(daemon_opts, function (err) {
      if (err) {
        log_install('daemon.install satan.ensure_created: FAILED: ' + err);
        return cb(err);
      }
      log_install('daemon.install satan.ensure_created: OK — starting in 500ms');
      // wait one sec, then start. or half.
      setTimeout(function () {
        satan.start(daemon_opts.key, function (startErr) {
          log_install('daemon.install satan.start: ' + (startErr ? 'FAILED: ' + startErr : 'OK'));
          cb(startErr);
        });
      }, 500);
    });
...
exports.set_watcher = (cb) => {
  log_install('daemon.set_watcher: START — calling trigger_set_watcher');
  let default_cb = cb;
  if (!cb) {
    default_cb = (err) => {
      if (err) {
        shared.log(err);
        log_install('daemon.set_watcher: FAILED: ' + err);
      } else {
        shared.log('Prey owl watcher set up!');
        log_install('daemon.set_watcher: OK');
      }
    };
  }
