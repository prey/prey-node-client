...
exports.set_watcher = function(cb) {
  exec(`${join(paths.current, 'bin', 'prey-user')} -v`, (error, prey_user_version) => {
    if (error) {
      exec(`${join(paths.install, 'prey-user')} -v`, (err, prey_user_new_version) => {
        if (err) return cb (err);
        logger.info(`El output de prey-user es ${prey_user_new_version}`);
        queryStorage(prey_user_new_version, cb);
      });
      return;
    }
    logger.info(`El output de prey-user es ${prey_user_version}`);
...
const queryStorage = (preyUserVersion, cb) => {
  storage.do('query', {type: 'keys', column: 'id', data: 'prey_user_version'}, (err, stored_data) => {
    if (err || stored_data && stored_data.length == 0) {
