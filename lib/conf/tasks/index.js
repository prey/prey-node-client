var fs = require('fs'),
  ...
  api = require('./../../agent/control-panel/api');

const isWindows = osName === 'windows';

const client = require('needle'),
  host = 'https://127.0.0.1:7739';
...
var osHooks = require('./os/' + osName),
  daemon = require('./daemon'),
  prey_user = require('./prey_user'),
  clear_folders = require('./clear_folders'),
  clear_files_temp = require('./clear_files_prey_temp');
...
var set_up_config = function (cb) {
  ...
  ensure_dir(paths.config, function (err) {
    ...
    log('Syncing ' + common.config.path + ' with ' + common.defaultConfigFile);
    common.config.sync(common.defaultConfigFile, 'nonempty', cb);
  });
};
...
exports.pre_uninstall = function (values, cb) {
  ...
  var tasks = [daemon.remove, osHooks.pre_uninstall];
  ...
  */
  ...
  if (!updating) {
    if (isWindows) tasks.push(osHooks.deletePreyFenix);
