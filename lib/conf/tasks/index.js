  ...
  const finish = () => {
    ...
    exports.chmodr(paths.current, 0o755, function (err) {
      ...
      verify_node_signature(function() {
        ...
        client.put(host, null, { timeout: 4500 }, (err) => {
          if (!err) return notifyDone();
          log('Local service notification failed, retrying in 3s: ' + err.message);
          setTimeout(() => {
            client.put(host, null, { timeout: 4500 }, (retryErr) => {
              if (retryErr) log('Local service notification failed after retry: ' + retryErr.message);
              notifyDone();
            });
          }, 3000);
        });
  ...
  if (osName === 'mac') {
    ...
    exec(`launchctl unload /Library/LaunchDaemons/${daemon_key}.plist`, function(err) {
      ...
      setUpVersion('this', function(err) {
        ...
        // Reload daemon immediately after activation to prevent WatchPaths race
        ...
        exec(`launchctl load /Library/LaunchDaemons/${daemon_key}.plist`, function(err) {
          ...
          // Clean up old versions AFTER reloading daemon to avoid delay
          cleanup_old_versions(function() {
            log('Cleanup completed in background');
          });
