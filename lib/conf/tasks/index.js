...
const verify_node_signature = function (cb) {
  ...
  exec(`codesign -dvvv "${node_path}" 2>&1`, (err, stdout, stderr) => {
    const output = stdout + stderr;
    log(`Signature details:\n${output}`);

    // Verify signature
    exec(`codesign --verify --verbose=4 "${node_path}" 2>&1`, (err, stdout, stderr) => {
      ...
      exec(`spctl -a -t exec -vv "${node_path}" 2>&1`, (err, stdout, stderr) => {
...
const setUpVersion = function (version, cb) {
  ...
  setUpConfig((err) => {
    ...
    shared.version_manager.set_current(version, (err) => {
      if (err) {
        if (err.code == 'ALREADY_CURRENT') log('Warning: This version is already set as current.');
        else return cb(err);
      }
      finish();
    });
