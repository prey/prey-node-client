...
const setUpVersion = function (version, cb) {
  ...
  const finish = () => {
    ...
    exports.chmodr(paths.current, 0o755, function (err) {
      ...
      // On macOS, force the kernel to re-read the code signature from disk
      // to avoid "embedded signature doesn't match attached signature" errors
      // when launchd restarts the process after an update.
      const refreshCodeSignature = (next) => {
        if (!is_mac) return next();
        const nodeBin = require('path').join(paths.current, 'bin', 'node');
        log('Refreshing code signature cache for node binary...');
        exec(`codesign --verify "${nodeBin}"`, () => {
          next();
        });
      };

      refreshCodeSignature(() => {
  ...
  // On macOS, unload the daemon before modifying files to prevent
  // WatchPaths from triggering a premature restart during activation.
  const unloadDaemonIfMac = (next) => {
    if (!is_mac) return next();
    log('Unloading daemon before activation to prevent premature restart...');
    exec('launchctl unload /Library/LaunchDaemons/com.prey.agent.plist', () => {
      // Ignore errors (daemon may already be unloaded)
      next();
    });
  };
