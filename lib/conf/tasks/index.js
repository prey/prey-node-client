...
function log_install(msg) {
  try {
    ...
    fs.appendFileSync(INSTALL_LOG, `[${ts}] [tasks/index] ${msg}\n`);
  } catch (e) {}
}
...
exports.post_install = (values, cb) => {
  log_install('=== post_install: START ===');

  const ready = (err) => {
    if (err) {
      log_install(`post_install ready: ERROR from prey_user.create: ${err}`);
      return cb(err);
    }
    log_install('post_install ready: prey_user created OK — running task series');

    const tasks = [
      (next) => {
        log_install('  task: daemon.install START');
        daemon.install((e) => {
          log_install(`  task: daemon.install ${e ? `FAILED: ${e}` : 'OK'}`);
          next(e);
        });
      },
      (next) => {
        log_install('  task: osHooks.post_install START');
        osHooks.post_install((e) => {
          log_install(`  task: osHooks.post_install ${e ? `FAILED: ${e}` : 'OK'}`);
          next(e);
        });
      },
      (next) => {
        log_install('  task: clear_folders.start START');
        clear_folders.start((e) => {
          log_install(`  task: clear_folders.start ${e ? `FAILED: ${e}` : 'OK'}`);
          next(e);
        });
      },
      (next) => {
        log_install('  task: clear_files_temp.start START');
        clear_files_temp.start((e) => {
          log_install(`  task: clear_files_temp.start ${e ? `FAILED: ${e}` : 'OK'}`);
          next(e);
        });
      },
    ];
    ...
    async.series(tasks, (err) => {
      if (err) {
        log_install(`post_install task series FAILED: ${err}`);
        return cb(err);
      }
      log_install('post_install task series: all done');

      const finished = (watcherErr) => {
        if (watcherErr) log_install(`post_install daemon.set_watcher FAILED: ${watcherErr}`);
        else log_install('post_install daemon.set_watcher: OK');
        log('Sweet! Please run `prey config panel` to link your device.');
        log_install('=== post_install: COMPLETE ===');
        return cb && cb();
      };

      if (is_mac) {
        log_install('post_install: calling daemon.set_watcher (mac only)');
  ...
  if (isWindows) { return setUpVersion('this', ready); }

  log_install('post_install: calling prey_user.create');
...
exports.pre_uninstall = (values, cb) => {
  ...
  if (!updating) {
    if (isWindows) {
      ...
      tasks.push(osHooks.cleanup_registry_keys);
    }
