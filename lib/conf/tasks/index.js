...
exports.post_install = function(values, cb) {
  ...
  var ready = function(err) {
    ...
    // Insert the mac watcher installer into tasks
    if (is_mac) tasks.push(daemon.set_watcher);
...
exports.pre_uninstall = function(values, cb) {
  ...
  if (is_mac) tasks.unshift(daemon.remove_watcher);
...
exports.set_watcher = function(values, cb) {

  var ready = function(err) {
    if (err) return cb(err);

    log('Sweet! Prey owl watcher set up');
    cb && cb();
  }

  if (process.platform != 'darwin') {
    return log('Service only available for macOS');
  }

  daemon.set_watcher(ready);
}
