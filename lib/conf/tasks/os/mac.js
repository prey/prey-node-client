...
function find_and_kill(my_pid, cb) {
  const installing_version = common.version;
  debug_log(`  find_and_kill(): my_pid=${my_pid}`);
    ...
    exec('ps -eo pid,args', (err, stdout) => {
      ...
      unique.forEach((pid) => {
        try {
          ...
          log(`Killed stale daemon process ${pid} with SIGKILL during upgrade.`);
          debug_log(`  find_and_kill(): killed PID ${pid} OK`);
        } catch (e) {
          log(`Failed to kill daemon process ${pid}: ${e.message}`);
  ...
  get_last_wings_version((err, last_wings_version) => {
    ...
    if (last_wings_version === installing_version) {
      const skipMsg = `Skipping kill: latest prey.log version (${last_wings_version}) matches installing version.`;
      log(skipMsg);
