...
exports.copyCurrentToTmpPath = `/bin/cp ${currentPathPreyUser} ${installVersionsPath}.tmp`;
exports.atomicReplacePreyUser = `/bin/mv -f ${installVersionsPath}.tmp ${installVersionsPath}`;
...
const testExistingConfigurations = (cb) => {
  exec(exports.existsNewPath, { timeout: 5000 }, (_error, stdout) => {
    if (stdout && stdout.trim() !== '') {
      ...
      exec(exports.existsOldpath, { timeout: 5000 }, (errorOldpath, stdoutOldpath) => {
        ...
        exec(exports.deleteInstallPreyUserBinary, { timeout: 10000 }, (err) => cb && cb(`Delete prey binary in /prey/: ${err == null ? '' : err}`));
      });
      exec(`launchctl unload ${pathLaunchDaemon}${newWatcherKey}`, { timeout: 15000 }, (errorUnload) => {
        ...
        exec(exports.copyCurrentToTmpPath, { timeout: 10000 }, (errorCopy) => {
          if (errorCopy) logger.error(`Error when copying file to tmp: ${errorCopy}`);
          exec(exports.atomicReplacePreyUser, { timeout: 10000 }, (errorMove) => {
            if (errorMove) logger.error(`Error when replacing prey-user binary: ${errorMove}`);
...
const activeWatcher = (cb) => {
  exec(exports.copyToDestination, { timeout: 10000 }, () => {
