...
const new_watcher_key = 'com.prey.new_owl';
const user_prey_path = '/Users/prey';
...
const path_launch_daemon = '/Library/LaunchDaemons/';
const watcher_opts = {
  key: new_watcher_key,
...
const remove_watcher = (cb) => {
  satan.ensure_destroyed(new_watcher_key, () => {
    satan.ensure_destroyed(old_watcher_key, (error) => {
      exec(`/usr/bin/dscl . -delete "${user_prey_path}"`, () => cb && cb(error));
    });
  });
};

exports.remove_single_watcher = (watcher_key, cb) => {
  satan.ensure_destroyed(watcher_key, () => cb && cb(error));
};
