...
const testExistingConfigurations = (cb) => {
  exec(exports.existsNewPath, (error, stdout) => {
    if (stdout && stdout.trim() !== '') {
      // look for old .plist and remove it
      exec(exports.existsOldpath, (_error, _stdout) => {
        if (!_stdout || _stdout.trim() === '') {
          return cb && cb(`com.prey.owl.plist doesn't exist: ${_error == null ? '' : _error}`);
        }
      ...
      exec(exports.copyCurrentToInstallVersionPath, (err_copy) => {
      });
    } else {
      activeWatcher(cb);
    }
