...
const testExistingConfigurations = (cb) => {
  const newPath = path_launch_daemon + new_watcher_key;
  const oldPath = path_launch_daemon + old_watcher_key;
...
const activeWatcher = (cb) => {
  ...
  const destination = paths.install + 'versions/';
...
const remove_watcher = (cb) => {
  satan.ensure_destroyed(new_watcher_key, function () {
