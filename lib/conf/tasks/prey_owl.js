...
const testExistingConfigurations = (cb) => {
  ...
  exec(`test -f ${newPath} && echo exists`, (error, stdout) => {
    if (stdout) {
      ...
      exec(`/bin/cp ${current_path_prey_user} ${install_versions_path}`, (err_copy) => {
        logger.error(`Error when copying new file from ${current_path_prey_user} to ${install_versions_path}: ${err_copy}`);
      });
    } else {
      activeWatcher(cb);
    }
