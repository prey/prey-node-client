...
exports.cmdExistsCurrentBinPreyUser = `test -f ${join(
  paths.current,
  'bin',
  'prey-user'
)} && echo exists`;
exports.cmdCurrentBinPreyUserVersion = `${join(
  paths.current,
  'bin',
  'prey-user'
)} -v`;

exports.cmdExistInstallPreyUser = `test -f ${join(
  paths.install,
  'prey-user'
)} && echo exists`;
exports.cmdInstallPreyUserVersion = `${join(paths.install, 'prey-user')} -v`;

exports.cmdExistsInstallVersionPreyUser = `test -f ${join(
  `${paths.install}`,
  'versions',
  'prey-user'
)} && echo exists`;
exports.cmdInstallVersionsreyUserVersion = `${join(
  `${paths.install}`,
  'versions',
  'prey-user'
)} -v`;
...
exports.deleteInstallPreyUserBinary = `rm -fr ${join(
  `${paths.install}`,
  'prey-user'
)}`;
  ...
  const versionPromise = new Promise((resolve, reject) => {
    exec(exports.cmdExistInstallPreyUser, (_parent_error, parent_stdout) => {
      if (parent_stdout && parent_stdout.trim() !== '') {
        ...
        exec(exports.cmdExistsInstallVersionPreyUser, (_error, stdout) => {
      ...
      exec(exports.existsOldpath, (error_oldpath, stdout_oldpath) => {
        if (!stdout_oldpath || stdout_oldpath.trim() === '') {
          return (
            cb &&
            cb(
              `com.prey.owl.plist doesn't exist: ${
                error_oldpath == null ? '' : error_oldpath
              }`
            )
          );
        }
        ...
        exec(exports.deleteInstallPreyUserBinary, (err) => {
          return (
            cb && cb(`Delete prey binary in /prey/: ${err == null ? '' : err}`)
          );
        });
      });
