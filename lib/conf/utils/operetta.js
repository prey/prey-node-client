...
const Operetta = function (args, scope) {
  if (args) {
    this.args = args;
  } else if (process.argv[0].match(/node(\.exe)?$/)) this.args = process.argv.slice(2);
  ...
  this.parse = function (listener) {
    const operetta = this; let
      parameter;

    const sing = function (argument, data) {
      ...
      if (argument == 'positional') {
        ...
        if (operetta.keyword_name) operetta.values[operetta.keyword_name] = data;
      } else {
    ...
    const analyze = function (option, data) {
    ...
    while (operetta.args.length > 0) {
      ...
      const m = operetta.re.exec(current);
      ...
      if (m) {
        ...
        if (m[2]) {
          const options = m[1][1] + m[2];

          for (i in options) {
            const a = operetta.params[`-${options[i]}`];
            if (a) {
...
Operetta.prototype.start = function (callback) {
  ...
  if (this.args.length == 0 && Object.keys(this.commands).length > 0) return operetta.usage();

  // if run without arguments, and either root or command that requires them, show help
  if (this.args.length == 0 && (!this.parent || Object.keys(this.params).length > 0)) return operetta.usage();

  // if helps is passed
  if (this.args[0] && (this.args[0] == '-h' || this.args[0].match(/(--)?help$/))) return operetta.usage();
  ...
  if (command) {
    ...
    const child = new Operetta(operetta.args, arg);
...
Operetta.prototype.bind = function (args, description, listener, type) {
  if (args) {
    ...
    if (!operetta.width) operetta.width = sargs.length + 5;

    if (sargs.length > operetta.width) throw new Error(`Argument length too long: ${sargs}`);
...
Operetta.prototype.banner = function () {
  const scopes = this.get_scope();
  const command = scopes.filter((x) => !!x).join(' ');
  const has_commands = Object.keys(this.commands).length > 0;

  const last = this.keyword_name ? `[${this.keyword_name}]` : has_commands ? '[command]' : '';
...
Operetta.prototype.usage = function (values, listener) {
  ...
  const usage = [this.banner(), help].join('\n');
