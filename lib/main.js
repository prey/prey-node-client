...
var Main = {
	...
	initialize_action_hooks: function(){
		...
		ActionsManager.on('event_triggered', function(trigger_name, data){
			self.triggers[trigger_name] = data || {};
		});
	...
	initialize: function(callback){
		...
		console.log("\n");
		logger.info("  PREY " + common.version + " spreads its wings!\n");
		logger.info("  Current time: " + this.started_at.toString())
		logger.info("  Running on a " + common.os_name + " system as " + this.running_user);
	...
	notify_events: function(){
		...
		if(this.events.length > 0 || this.triggers.length > 0) {

			var data = {events: this.events, triggers: this.triggers};
