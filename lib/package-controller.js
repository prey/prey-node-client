...
packageController.delete_attempts = (cb) => {
...
packageController.get_update_data = (cb) => {
  const data = {
    public_ip: null,
    country: null,
    location: {
...
// eslint-disable-next-line consistent-return
packageController.newVersion_available = (branch, current, cb) => {
...
packageController.get_latest = (branch, currentVersion, dest, cb) => {
  if (!currentVersion || !dest) throw new Error('Missing current version and/or destination.');
  // eslint-disable-next-line consistent-return
  packageController.newVersion_available(branch, currentVersion, (err, version) => {
...
packageController.check_update_success = (newVersion, versionsPath, cb) => {
  // eslint-disable-next-line consistent-return
  storage.do('all', { type: 'versions' }, (err, db) => {
    ...
    if (dbVersion && dbVersion.notified === 0) {
      ...
      packageController.delete_older_versions(oldVersion, newVersion, versionsPath);
      // eslint-disable-next-line consistent-return
