...
const downloadFromURL = (url, cb) => {
  ...
  needle.get(url, { output: file }, (err, resp, data) => {
    if (err || resp.statusCode !== 200) {
      return (
        cb && cb(err || new Error(`Unexpected response: \n\n${data.toString()}`))
      );
    }
  ...
  downloadFromURL(url, (err, file) => {
    ...
    verifyChecksum(version, release, file, (errVerifyCheckSum, valid) => {
      if (errVerifyCheckSum || !valid) {
        return fs.unlink(
          file,
          () => cb && cb(errVerifyCheckSum || new Error(`Invalid checksum for file: ${release}`)),
        );
      }
...
packageController.deleteAttempts = (cb) => {
  storage.do('clear', { type: 'versions' }, (err) => {
    if (err) {
      return cb(
        new Error(`Error deleting update attempts registry: ${err.message}`),
      );
    }
    return cb && cb(err);
  });
    ...
    storage.do('clear', { type: 'versions' }, (err) => {
      ...
      storage.do(
        ...
        (errSet) => {
          if (errSet) {
            return cb(
              new Error('Couldnt open local database, update cancelled'),
            );
          }
          return cb(null, true);
        },
...
packageController.downloadInstall = (version, dest, cb) => {
  ...
  if (fs.existsSync(finalPath)) {
    switch (ongoingAttempt % 10) {
      case 3:
        setTimeout(() => {
          packageController.restart_client();
        }, 3000);
        break;
      case 5:
        setTimeout(() => {
          packageController.activate_version(version);
        }, 3000);
        break;
      case 7:
        setTimeout(() => {
          packageController.delete_version(version);
        }, 3000);
        break;
      default:
        break;
  ...
  downloadVerify(version, (err, file) => {
    if (err) return cb(err);
    packageController.install(
      file,
      dest,
      (errPackageInstall, installedVersion) => {
...
packageController.getVersion = (version, dest, cb) => {
  ...
  packageController.updateVersionAttempt(
    ...
    (err, update) => {
      if (err) return cb(err);
      if (update) {
        packageController.downloadInstall(
          version,
          dest,
          (errDownloadInstall) => {
...
packageController.checkUpdateSuccess = (newVersion, versionsPath, cb) => {
  ...
  storage.do('all', { type: 'versions' }, (err, db) => {
    ...
    if (dbVersion && dbVersion.notified === 0) {
      ...
      packageController.deleteOlderVersions(
        oldVersion,
        newVersion,
        versionsPath,
      );
      ...
      sendUpdateEvent(
        ...
        (errUpdate) => {
          if (errUpdate) {
            return cb(
              new Error(
                `Error sending the update success event: ${errUpdate.message}`,
              ),
            );
          }
          ...
          storage.do('clear', { type: 'versions' }, (errClear) => {
            if (errClear) {
              return cb(
                new Error(
                  `Error deleting update attempts registry: ${err.message}`,
                ),
              );
            }
...
packageController.deleteOlderVersions = (
  ...
  fs.readdir(versionsPath, (_err, allVersions) => {
    ...
    const allVersionsFiltered = allVersions.filter(
