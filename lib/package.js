var fs              = require('fs'),
    ...
    arch            = require('arch')(),
    tmpdir          = osName == 'windows' ? process.env.WINDIR + '\\Temp' : '/tmp';
...
const download_file = (version, _arch, cb) => {
  ...
  var release = ['prey', osName, version, _arch].join('-') + package_format,
...
package.restart_client = () => {
  ...
  var restart_cmd = osName == 'windows' ? 'taskkill /F /PID ' : 'kill -9 ',
      pid = fs.readFileSync(common.pidFile);
