...
var delete_older_versions = function(old_ver, new_ver, versions_path) {
  // Get all the versions from the directory, then exclude the new and the previous one
  fs.readdir(versions_path, function(err, all_versions) {
    all_versions = all_versions.filter(function(version) {
      return version != old_ver && version != new_ver;
    })
...
package.download_install = function(version, dest, cb) {
  ...
  if (fs.existsSync(final_path)) {
    write('v' + version + ' already exists ' + dest + ", trying to delete it...");
    remove(final_path, function() {
      if (fs.existsSync(final_path))
...
package.check_update_success = function(new_version, versions_path, cb) {
  ...
  storage.all('versions', function(err, db) {
    ...
    if (db[key] && !db[key].notified) {
      ...
      delete_older_versions(old_version, new_version, versions_path);
