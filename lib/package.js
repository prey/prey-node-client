...
var move = function(from, to, cb) {
  ...
  function like_a_boss(attempt) {
    fs.rename(from, to, function(err) {
      if (err) log(`Error on attempt ${attempt} when moving directory: ${err.message}`);
...
package.check_update_success = function(new_version, versions_path, cb) {
  storage.do('all', {type: 'versions'}, (err, db) => {
    ...
    const logger = require('./agent/common').logger.prefix('[][][][][]');
    var db_version = db.find(x => x.id == new_version);   // -> undefined si no existe la version
    logger.info('GETTING UPDATE SUCCESS');
    if (db_version) {
      logger.info(`find db_version: ${typeof db_version === 'object' ? JSON.stringify(db_version) : db_version} `);
      storage.do('query', { type: 'keys', column: 'id', data: 'recentlyUpdated' }, (err, stored) => {
        logger.info(`is there an stored value? ${typeof stored === 'object' ? JSON.stringify(stored) : stored} `);
        if (err) {
          logger.info(`There was an error trying to get recentlyUpdated: ${err}`);
          return;
        }
        if (stored && stored.length > 0) {
          logger.info(`Updating recentlyUpdated with value: ${new Date().toISOString()}`);
          storage.do('update', { type: 'keys', id: 'recentlyUpdated', columns: 'value', values: new Date().toISOString() }, (errUpdate) => {
            if (errUpdate) logger.error(`Error while setting recentlyUpdated: ${errUpdate}`);
          });
        } else {
          logger.info(`Creating recentlyUpdated with value: ${new Date().toISOString()}`);
          storage.do('set', { type: 'keys', id: 'recentlyUpdated', data: { value: new Date().toISOString() } }, (errSetting) => {
            if (errSetting) logger.error(`Error while setting recentlyUpdated: ${errSetting}`);
          });
        }
      });
    }
