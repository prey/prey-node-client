    ...
    if (db_version) {
      storage.do('query', { type: 'keys', column: 'id', data: 'recentlyUpdated' }, (err, stored) => {
        ...
        if (stored && stored.length > 0) {
          storage.do('update', { type: 'keys', id: 'recentlyUpdated', columns: 'value', values: new Date().toISOString() }, () => {});
        } else {
          storage.do('set', { type: 'keys', id: 'recentlyUpdated', data: { value: new Date().toISOString() } }, () => {});
        }
      });
    }
