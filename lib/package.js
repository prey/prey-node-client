...
const host            = common .config.get('control-panel.host');
const npm_package_url = 'https://registry.npmjs.org/prey';

let releases_host;
if (host === 'solid.preyproject.com')
  releases_host = 'https://downloads.preyproject.com';
else if (host === 'solid.preyhq.com')
  releases_host = 'https://downloads-stg.preyhq.com';

const releases_url    = `${releases_host}/prey-client-releases/node-client/`;
const latest_text     = 'latest.txt';
const checksums       = 'shasums.json';
const package_format  = '.zip';
...
const checksum_for = function(file, cb) {
  var error,
...
const unpack = function(zip, dest, cb) {
...
const move = function(from, to, cb) {
...
const send_update_event = (type, status, old_version, new_version, attempt, error, cb) => {
...
releases.verify_checksum = function(version, filename, file, cb) {
  ...
  const url = releases_url + version + '/' + checksums;
  log('Fetching checksums: ' + url);

  needle.get(url, { parse: true }, function(err, resp) {
    if (err) return cb(err);

    const checksum = parse_sums(resp.body)[filename];
    if (!checksum)
  ...
  if (os_name === 'mac'){
    cp.exec('sysctl sysctl.proc_translated', (err, stdout) => {
      if(err) {
        ...
        download_file(version, stdSlice === '1' ? 'arm64' : arch, cb);
      }
    });
  }else{
    download_file(version, arch, cb);
...
package.update_version_attempt = (old_version, new_version, attempt_plus, set_notified, error, cb) => {
  ...
  storage.do('all', {type: 'versions'}, (err, db) => {
    ...
    const db_version = db.find(x => x.id == new_version);

    if (db_version) {
      const current_attempt  = db_version.attempts;
      const already_notified = db_version.notified;
      const new_attempt = current_attempt;
...
package.get_update_data = (cb) => {
  ...
  const data = {public_ip: null, country: null, location: {lat: null, lon: null}};
...
package.new_version_available = function(branch, current, cb) {
  const method = `get_${branch}_version`;
  ...
  releases[method](function(err, upstream_version) {
    ...
    const ver = is_greater_than(upstream_version, current) && upstream_version;
    cb(null, ver);
  })
}
...
package.download_install = function(version, dest, cb) {
  const final_path = path.join(dest, version);
...
package.install = function(zip, dest, cb) {
  ...
  if (!zip.match(/prey-(\w+)-([\d\.]+)/))
    return cb(new Error("This doesn't look like a Prey package: " + zip));
      ...
      version    = path.basename(zip).match(/([\d\.]+)/)[1],
...
package.restart_client = () => {
  ...
  const restart_cmd = os_name == 'windows' ? 'taskkill /F /PID ' : 'kill -9 ';
  const pid = fs.readFileSync(common.pid_file);
