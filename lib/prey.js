...
var Main = {
	...
	run: function(){
		...
		var start_wait_time = (!process.env.TERM || process.env.TERM == 'dumb') ? common.helpers.randomBetween(1, 59) : 0;
	...
	no_connection: function(){
		...
		var last_response_file_path = common.helpers.tempfile_path(config.last_response_file);

		path.exists(last_response_file_path, function(exists){
			if(exists) {
	...
	process: function(response_body, offline){
		...
		ResponseParser.parse(response_body, parser_options, function(parsed){
			...
			self.check_client_version(function(){
				...
				if(!self.marked_as_missing() || !self.requested.report) {
//				if(!self.requested.report || Object.keys(self.requested.report).length == 0) {
	...
	process_main_config: function(){
		...
		if(this.requested.delay) 
			this.update_delay(this.requested.delay);
	...
	update_delay: function(requested_delay){
		...
		common.os.get_current_delay(function(current_delay){
			...
			// if device is marked as missing, the execution delay is set every X minutes
			if(self.marked_as_missing()){
