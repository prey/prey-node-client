...
var Main = {
	...
	no_connection: function(){
		...
		if(path.existsSync(common.helpers.tempfile_path(this.config.last_response_file))){
	...
	process: function(response_body, offline){
		...
		ResponseParser.parse(response_body, parser_options, function(parsed){
			...
			if(!offline && self.requested.offline_actions)
				common.helpers.save_file_contents(config.last_response_file, response_body);
	...
	missing: function(){
		try {
			...
			return self.response_status == config.missing_status_code;
		}
	},
	...
	get_info: function(requested_info){
		...
		provider.get_many(requested_info, function(traces){
			Notifier.send(traces, self.requested.destinations.traces);
		});
	...
	notify_events: function(){
		...
		if(Object.keys(this.actions).length > 0 || Object.keys(this.events).length > 0) {
			...
			var notification = Notifier.send(events, self.requested.destinations.events);
