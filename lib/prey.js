...
var common = require('./prey/common'),
		...
		fs = require("fs"),
		util = require("util"),
		System = require('./prey/plugins/providers/system'),
		hooks = require('./prey/hook_dispatcher'),
		Connection = require('./prey/connection'),
		PluginLoader = require('./prey/plugin_loader'),
...
var Main = {
	...
	run: function(){
		...
		var start_wait_time = (!process.env.TERM || process.env.TERM == 'dumb') 
			? common.helpers.randomBetween(1, 59) 
			: 0;
	...
	initialize_hooks: function(){
		...
		ActionsManager.on('action_returned', function(action_name, success, data){
			...
			if(data) this.send_data(action_name, data);
	...
	load_driver: function(driver_name, driver_options){
		...
		var driver_name = driver_name || 'http';
		logger.info("Loading " + driver_name + " driver...");
		// hooks.trigger('driver_start');

		driver = require('./prey/plugins/drivers/' + driver_name);
		self.drivers[driver_name] = driver;
		
		driver.on('actions',  this.start_actions); // command_name, options
		driver.on('command',  this.handle_incoming_command); // command_name, options
		driver.on('delay',    this.update_delay); // new_delay
		driver.on('driver',   this.load_driver); // driver_name, driver_options
		driver.on('endpoint', this.load_transport);
		driver.on('setting',  this.update_setting); // key, value
		driver.on('settings', this.update_settings); // key, value
		driver.on('version',  this.check_client_version); // current_release
	...
	load_transport: function(endpoint_name, options){
		
		PluginLoader.load_transport(endpoint_name, options, null, function(loaded_transport){
			
			if(loaded_transport)
				self.transports[endpoint_name] = loaded_transport;
			else
				logger.error("Unable to load transport for endpoint " + endpoint_name);
	...
	update_delay: function(requested_delay){
		...
		common.os.get_current_delay(function(current_delay){
			...
			} else if(parseInt(current_delay) == NaN || !current_delay.one_hour){
	...
	start_actions: function(actions_array){
		...
		Main.load_modules(actions_array, function(modules){
	...
	send_data: function(context, data){
		...
		var message = Notifier.send(data, self.transports[context]);

		message.once('sent', function(destinations){
