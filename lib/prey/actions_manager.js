...
var ActionsManager = function(){
	...
	this.load_and_start = function(actions_array, skip_running){
	...
	this.load_actions = function(array, callback){
		...
		array.forEach(function(requested_module){
			...
			loader.load_action(requested_module.name, function(err, loaded_module){

					if(err)
						logger.error(err);
					else {
						loaded_module.options = requested_module.options;
	...
	this.initialize = function(enabled_actions){
		...
		enabled_actions.forEach(function(action_module, i){
	...
	this.queue = function(action_module, options){
		...
		logger.debug('[actions] Queueing action ' + action_module.name);
	...
	this.start_action = function(action_module, options){
		...
		this.emit('action_started', action_module.name, action_module.started_at);

		this.instances[action_module.name] = action_module.start(options || {}, function(err, data){
	...
	this.action_returned = function(action_module, err, returned_data){
		...
		if(action_module.events && !err)
			this.wait_until_finished(action_module);
		else
