...
var Agent = self = {
	...
	prepare_for_engage: function(){
		...
		this.initialize(function(){
			...
			if(!config.auto_update || !process.env.TRIGGER)
				return self.engage();
	...
	get_report: function(report_name, options, interval){

		if(this.reports)
			this.reports = require('./reports');

		var ocurrence = interval ? 'on' : 'once';

		this.reports[occurence](report_name, function(err, data){
			if(err) return self.log_error(err);
			if(data) self.send_data(report_name, data);
		});

		this.reports.get(report_name, options, interval);
	...
	start_actions: function(actions_array, skip_running){

		if(!self.action_hooks_loaded)
			self.load_action_hooks();

		actions.load_and_start(actions_array, skip_running);
	...
	start_actions_by_name: function(list, skip_running){
		...
		self.start_actions(actions, skip_running);
	},

	start_action_by_name: function(action_name, options){
		self.start_actions([{name: action_name, options: options}], true)
	},
