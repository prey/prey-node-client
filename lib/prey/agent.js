...
*/
	...
	load_driver: function(driver_name, driver_options){
		...
		plugins.load_driver(driver_name, {}, null, function(loaded_driver){
			
			if(!loaded_driver) 
				return self.log_error("Unable to load driver " + driver_name);

			self.setup_driver(loaded_driver, driver_options || loaded_driver.config);

		});

	},
	
	setup_driver: function(driver, options){

		logger.info("Initializing " + driver.name + " driver...");
