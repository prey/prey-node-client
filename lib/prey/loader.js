...
var Loader = function(){
	...
	this.load = function(plugin_name){
		...
		this.empty_config_file = path.join(this.plugin_path, default_config_file);
		this.plugin_config_file = path.join(common.config_path, this.plugin_type + 's', this.plugin_name + ".js");
...
*/
	...
	this.loaded = function(mod){
		...
		var self = this;
		// mod.options = this.plugin_options;
		...
		this.load_config(function(err, config){
			...
			logger.debug("Plugin " + self.plugin_name + " loaded!")
			self.done(null, mod);
		});
	...
	this.load_config = function(callback){
		...
		var self = this, plugin_config;

		try { // try to load it from /etc/prey/[plugin_type]/[plugin_name]

			plugin_config = require(this.plugin_config_file);

		} catch(e) { // not found. if module has config file, copy it

			fs.exists(this.empty_config_file, function(exists){
				if(!exists) return;

				// return immediately so the callback below doesn't get fired twice
				return helpers.copy_file(this.empty_config_file, this.plugin_config_file, function(err){

					if(err) return callback(new Error("Couldn't copy " + self.plugin_name + " config file to " + self.plugin_config_file));

					callback(null, require(self.plugin_config_file));
				});
	...
	this.call_npm = function(method, plugin_name){
		...
		return this.failed(new Error("Kaboom!"));
