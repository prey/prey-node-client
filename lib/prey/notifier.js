...
var Notifier = function(){
	...
	this.get_default_transports = function(){
		var transports = [];
		config.transports.forEach(function(transport_name){
			PluginLoader.load_transport(transport_name, {}, null, function(loaded_transport){
	...
	this.send = function(data, transports){
		...
		transports.forEach(function(transport){
			...
			transport.once('end', function(success){
				...
				if(success) successful.push(transport.name);
