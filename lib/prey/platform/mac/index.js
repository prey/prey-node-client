...
exports.auto_connect = function(callback){
	...
	var network_service, airport_name,
	...
	var toggle_airport = function(direction, cb){
		exec('networksetup -setnetworkserviceenabled ' + network_service + ' ' + direction, function(err, stdout){
			if(err) return cb(err);			
			exec('networksetup -setairportpower ' + airport_name + ' ' + direction, cb)
		})
	}

	var connect_to_ap = function(ap, callback){
		var cmd = 'networksetup -setairportnetwork ' + airport_name + ' ' + ap.ssid;
		exec(cmd, callback);
	}
