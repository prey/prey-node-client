...
var ControlPanelDriver = function(options){
	...
	this.process = function(response_body, offline){
		...
		parser.parse(response_body, {key: this.config.api_key}, function(err, requested){
			...
			if(requested.info && requested.info.location && requested.delay)
				self.set_interval_for_reports(requested.delay, 'location', requested.info.location);
			else if(!self.interval.loop && (!requested.delay || !requested.info.location))
				self.stop_reports_interval();
	...
	this.set_interval_for_reports = function(delay, name, options){

		if(delay < 1000) delay = delay * 60 * 1000; // in case the delay is in minutes

		if(this.interval.delay == delay) return;
		else if(this.interval.loop) clearInterval(this.interval.loop);

		this.interval.loop = setInterval(function(){
			self.log('Requesting new ' + name + ' report!')
			self.emit('get', name, options);
		}, delay);

		this.interval.delay = delay;

	};

	this.stop_reports_interval = function(){
