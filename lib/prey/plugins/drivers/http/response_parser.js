...
var ResponseParser = {
	...
	build_new_schema: function(original, callback){
		...
		if(original.configuration.post_url)
			data.endpoints.report.control_panel.url = original.configuration.post_url;
			...
			if(module_data.type == 'report'){
				...
				for(key in module_options){
					...
					if(/^get_/.test(key)){
						...
						if(!report[key_start]) continue;

						if(report[key_start] == true) // replace true with {}
							report[key_start] = {}

						var key_end = key.replace(/.*_([a-z]+)$/, "$1");

						report[key_start][key_end] = val;
		...
		if(Object.keys(report).length > 0) data.actions.report = report;
