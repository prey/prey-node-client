...
var Geo = function(){
	...
	this.get_location = function(){
		...
		Network.get('access_points_list', function(aps_list){
			...
			if(!aps_list){
				self.emit('location', new Error("No access points list, cannot get geolocation."));
				return false;
			}
			...
			client.post("https://www.google.com/loc/json", 
				...
				if(coords.location && coords.location.latitude){
					...
					self.emit('location', null, coords_data);

				} else {

					self.emit('location', new Error("Couldn't get any location data. Try moving around a bit."));
