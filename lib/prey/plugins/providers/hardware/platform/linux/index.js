...
Prey.err._set('DMIDECODE','Linux dmidecode failure');
  ...
  cache.value('macs',function(cachedMacs) {
    ...
    exec(mac_address_cmd(nic_name), function(err, stdout){
      ...
      */
      ...
      console.log("Got mac "+newValue+" for nic:"+nic_name);
...
exports.get_first_mac_address = function(cb) {
  cache.value('macs',function(macs) {
    ...
    console.log('getting first mac for '+keys[0]+ ' = '+macs[keys[0]]);
    cb(null,macs[keys[0]]);
  });
};
