  ...
  this.get_active_access_point = function(callback){
    ...
    this.get('access_points_list', function(err, list){
      ...
      if(err) return callback(err);
        ...
        exec(os_functions.active_access_point_mac_cmd, function(err, stdout, stderr){
          ...
          var padded_mac = stdout.toString().trim().replace(/(^|:)(?=[0-9a-fA-F](?::|$))/g, "$10")

          for(i in list){
            var ap = list[i];
            if(ap.mac_address == padded_mac)
