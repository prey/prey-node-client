 ...
 **/
exp.get_public_ip = helpers.memoize(function(callback){  
  var
...
exp.get_public_ip.arity = 0;

/**
 * Callsback an internal IP address.
 **/
exp.get_private_ip = helpers.memoize(function(callback) {
  hardware.get_network_interfaces_list(function(err, list){
    if(err) return callback(_error(err));

    var ips = list.filter(function(nic) { return exp.is_ip_address(nic.ip_address); });      
    if (ips.length > 0)
 ...
 **/
exp.get_active_network_interface = helpers.memoize(function(callback){
...
exp.get_active_network_interface.arity = 0;
...
exp.get_first_wireless_interface.arity = 0;
...
exp.get_active_access_point.arity = 0;

/**
 * 
 **/
exp.get_access_points_list = helpers.memoize(function(callback){
  exp.get_first_wireless_interface(function(err, wifi_nic_name){
    if(err) return callback(_error(err));
...
exp.get_access_points_list.arity = 0;
...
exp.get_open_access_points_list.arity = 0;