...
var Network = function(){
	...
	this.get_private_ip = function(){
		...
		Hardware.get('network_interfaces_list', function(err, list){
			...
			return self.emit('private_ip', new Error("No private IP found."));
...
*/
	...
	this.get_access_points_list = function(){
		...
		this.get('first_wireless_interface', function(err, wifi_nic_name){
			...
			if(err || !wifi_nic_name)
				return self.emit('access_points_list', new Error("Wifi device required for access points list."));

			os_functions.get_access_points_list(wifi_nic_name, function(result){

				if(!result)
					return self.emit('access_points_list', new Error("Failed to get a list of nearby nearby wifi hotspots."));
				else if(result.length && result.length > 0)
					return self.emit('access_points_list', null, result);

				try {
					var json_list = JSON.parse(result);
					return self.emit('access_points_list', null, json_list);
				} catch(e) {
					return self.emit('access_points_list', new Error("Wifi hotspots list was returned in invalid format."));
				}
