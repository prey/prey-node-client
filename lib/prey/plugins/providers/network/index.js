...
*/
	...
	this.get_access_points_list = function(){
		...
		this.get('first_wireless_interface', function(wifi_nic_name){
	...
	this.get_first_wireless_interface = function(){
		...
		this.get('wireless_interface_names', function(list){

			if(list[0])
				self.emit('first_wireless_interface', list[0]);
			else
				self.emit('first_wireless_interface', false, 'No wifi network interfaces found.');
	...
	this.get_wireless_interface_names = function(){
		...
		exec(os_functions.wireless_devices_list_cmd, function(err, stdout, stderr){
			...
			if(err) return self.emit('wireless_interface_names', false, e.message);
			
			var list = stdout.toString().trim().split('\n');
			self.emit('wireless_interface_names', list);
