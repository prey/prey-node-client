 ...
 **/
self.get_public_ip = helpers.memoize(function(callback){  
  var
 ...
 **/
self.get_private_ip = helpers.memoize(function(callback) {
 ...
 * @param {String} platform specific name, e.g. eth0, Local Area Connection
 **/
self.get_nic_by_name = helpers.memoize(function(name, callback) {
  hardware.get_network_interfaces_list(function(err, list){
    if(err) return callback(_error(err));
 ...
 **/
self.get_active_network_interface = helpers.memoize(function(callback){
