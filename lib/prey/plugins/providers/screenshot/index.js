...
var Screenshot = function(){
	...
	this.get_screenshot = function(options){
		...
		exec(cmd, function(err, stdout, stderr){
			...
			if(err) return self.emit('screenshot', false, err.code);

			path.exists(screenshot_file, function(exists){
				if(exists)
					self.emit('screenshot', {file: screenshot_file, content_type: 'image/jpeg'});
				else
					self.emit('screenshot', false, "Unable to get screenshot.")
			})
