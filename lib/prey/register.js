...
var Register = {
	...
	device: function(options, callback){
		...
		Register.get_device_data(function(data){
			...
			http_client.post(url, {device: data}, options, function(err, response, body){
	...
	get_device_data: function(callback){
		...
		Hardware.get('firmware_info', function(hw_info){
			data.title = hw_info.hardware_product;
			data.device_type = hw_info.formfactor == 'laptop' ? 'Laptop' : 'Desktop';
			data.vendor_name = hw_info.chassis_manufacturer;
			data.model_name  = hw_info.hardware_product;
			callback_if_ready();
		});
