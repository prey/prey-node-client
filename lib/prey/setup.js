...
var run = exports.run = function(){
	if (config.persisted()){
		...
		fs.mkdirSync(path.resolve(program.path));
		config.sync(common.default_config_file, function(err){

      // the err could be a missing /etc/prey dir too 
			if(err) return exit_error(messages.no_perms,err);
...
var end_setup = function(err){
	if (err) return exit_error(messages.error_storing_values + err.toString());
...
var setup_driver = function(driver){
  var setup;
	try {
		setup = require('./plugins/drivers/' + driver.name + '/setup');
	} catch(e) {
...
var exit_error = function(msg,err){
	console.log("\n" + msg.toString());
