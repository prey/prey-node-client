...
var AsyncCache = function() {
  ...
  var newValue = function(id,el,callback) {
    el.generate(function(value) {
      if (!value) {
        console.log("!! Cache generated null for "+id);
      }
  ...
  /*
    In some instances the generation of a cache value can't be completed in
    entirety by the generation function. Then the idiom is for the generation
    function to return a composite value, which is then tweaked. In such an
    instance when one has a new/tweaked value, then one may use 'set' to update
    the cache internals appropriately.
  */
  ...
  this.manage = function(id,generate,freshen) {
    ...
    var entry = {time:null,value:null };
  ...
  this.value = function(id,force,callback) {
    if(!callback) {
      if (typeof force === 'function') {
        callback = force;
        force = false;
      } else throw new Error("AsyncCache.value: No callback supplied");
    }
  ...
  this.entry = function(id,force,callback) {
    if(!callback) {
      if (typeof force === 'function') {
        callback = force;
        force = false;
      } else throw new Error("AsyncCache.entry: No callback supplied");
    }
...
util.inherits(AsyncCache, Emitter);
