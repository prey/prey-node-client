...
var Geo = function(){
	...
	this.start = function(){
		...
		this.get('location', function(coords){
	...
	this.get_location = function(){
		...
		Network.get('access_points_list', function(aps_list){
			...
			http_client.post("https://www.google.com/loc/json", options).on('complete', function(json_response, response){
				...
				if(json_response instanceof String){
					...
					try {
						...
						self.emit('location', false, e.code);
						return false;
				...
				if(coords.location.latitude){
					...
					self.emit('location', false, "Couldn't get any location data. Try moving around a bit.");
