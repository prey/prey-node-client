...
system.get_os_version((err, os_version) => {
  if (!err) {
    ...
    system.user_agent = `Prey/${version} (Node ${
      process.version
    } ${system.os_name[0].toUpperCase()}${system.os_name.slice(
      1
    )} ${os_version})`;
  }
});
...
system.get_os_info = remember((cb) => {
  ...
  async.parallel(
    [system.get_os_name, system.get_os_version],
    (err, results) => {
      if (done) return;
...
system.get_root_user = remember((cb) => {
  cb(null, 'root');
});
...
system.get_running_user = () =>
  // eslint-disable-next-line implicit-arrow-linebreak
  clean_string(
    // eslint-disable-next-line comma-dangle
    process.env.LOGNAME || process.env.USER || process.env.USERNAME || 'System'
  );

const getUser = {
  logged_user: system.get_logged_user,
  admin_user: system.get_admin_user,
  root_user: system.get_root_user,
};

function get(type, cb) {
  getUser[type]((err, user) => cb(err, user));
}
...
system.spawn_as_admin_user = function (command, args, opts, cb) {
  ...
  if (system.os_name == 'windows') {
    system.check_service(options, (err, data) => {
      // An error means the new service isn't available
      if (err) {
        if (
          options.args &&
          Array.isArray(options.args) &&
          options.args[0].includes('wipe')
        ) {
          // For wipe only when service is not available
          return as(
            'logged_user',
            'spawn',
            data.command,
...
system.run_as_root = function (command, args, opts, cb) {
  as('root_user', 'exec', command, args, opts, cb);
};
