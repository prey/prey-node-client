...
system.get_os_version(function(err, os_version){
  if (!err) system.user_agent = 'Prey/' + version + ' (Node ' + process.version + ' ' + system.os_name[0].toUpperCase() + system.os_name.slice(1) + ' ' + os_version + ')';
});
...
system.spawn_as_admin_user = function(command, args, opts, cb) {
  if (os_name == 'windows') {
    system.check_service(function(err) {    // An error means the new service isn't running
      if (err) return as('logged_user', 'spawn', command, args, opts, cb);
