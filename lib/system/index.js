...
const system = require(join(__dirname, osName));
// eslint-disable-next-line import/no-dynamic-require
...
system.get_os_version(function(err, os_version) {
  if (!err) {
    system.osRelease = os_version;
    system.user_agent = 'Prey/' + version + ' (Node ' + process.version + ' ' + system.osName[0].toUpperCase() + system.osName.slice(1) + ' ' + os_version + ')';
  }
});
