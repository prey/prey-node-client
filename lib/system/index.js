...
const { cleanString: clean_string } = require('../utils/string');
...
// Short-lived cache + single-flight to collapse bursty logged-user lookups.
// Each miss spawns a PowerShell process on Windows, so de-duplicating rapid
// callers meaningfully reduces process churn (and Defender scanning). Errors
// are never cached, so lock-screen / no-user states stay live.
const LOGGED_USER_TTL_MS = 4000;
let loggedUserCache = { value: /** @type {string|null} */ (null), expires: 0 };
/** @type {Array<(err: Error|null, user?: string) => void>|null} */
let loggedUserPending = null;

/**
 * @param {(err: Error|null, user?: string) => void} cb
 * @param {boolean} [bypassCache] skip the TTL cache — for callers that need a
 *   fresh value (e.g. the lock action's 5s session poll detecting user switch).
 */
system.get_logged_user = (cb, bypassCache) => {
  if (typeof cb !== 'function') return;

  const now = Date.now();
  if (!bypassCache && loggedUserCache.value && loggedUserCache.expires > now) {
    cb(null, loggedUserCache.value);
    return;
  }

  // Coalesce concurrent (non-bypass) callers onto a single lookup.
  if (!bypassCache) {
    if (loggedUserPending) {
      loggedUserPending.push(cb);
      return;
    }
    loggedUserPending = [cb];
  }
   ...
   * @param {string} [user]
   */
  const deliver = (err, user) => {
    if (bypassCache) {
      cb(err, user);
      return;
    }
    const waiters = loggedUserPending || [];
    loggedUserPending = null;
    waiters.forEach((waiter) => {
      try {
        waiter(err, user);
      } catch (e) { /* ignore individual callback errors */ }
    });
  };

  system.find_logged_user((e, user) => {
    if (e || !user || user.trim() === '') {
      const err = /** @type {NodeJS.ErrnoException} */ (new Error('No logged user detected.'));
    ...
    loggedUserCache = { value: user, expires: Date.now() + LOGGED_USER_TTL_MS };

    // Only log when user changes or first time
    if (lastLoggedUser !== user) {
      // @ts-ignore — common.logger is not reflected in the module type
      const { logger } = require('../agent/common');
      logger.info(`Logged user: ${user}`);
      lastLoggedUser = user;
    }

    return deliver(null, user);
  });
};
...
system.get_admin_user = remember((cb) => {
  system.find_admin_user((e, user) => {
    if (e || !user || user.trim() == '') {
      const err = /** @type {NodeJS.ErrnoException} */ (new Error('No admin user detected.'));
      if (e) err.message += ` ${e.message}`;
...
function as(user_type, type, bin, args, opts, cb, user = null) {
  if (typeof opts === 'function') {
    var cb = opts;
    // @ts-ignore — var re-declaration is pre-existing; migrating to let is a separate chore
    var opts = {};
  } else if (!opts) {
    // @ts-ignore — var re-declaration is pre-existing; migrating to let is a separate chore
    var opts = {};
...
function as_user(options, cb) {
  ...
  if (type == 'exec') {
    ...
    // @ts-ignore — impersonating is a custom property tracking the running user
  ...
  child.on('error', (err) => {
    // @ts-ignore — system errors carry a .code property at runtime
