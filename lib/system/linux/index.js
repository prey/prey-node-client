...
exports.get_session_type = (callback) => {
  exec(`loginctl show-session "$(loginctl | awk '/tty|seat/ {print $1; exit}')" -p Type --value`, (err, stdout) => {
    if (err) {
      return callback(new Error('Unable to detect session type: ' + err.message));
    }

    let sessionType = stdout.toString().trim();

    if (!sessionType) {
      return callback(new Error('XDG_SESSION_TYPE not set'));
    }
