...
exports.get_os_version = function(cb) {
  var release;
  try {
    release = execSync('lsb_release -rs', { timeout: 5000 });
  } catch (e) {
...
exports.find_logged_user = function(callback) {
  ...
  exec(command, { timeout: 8000 }, function(err, out) {
...
exports.find_admin_user = function(cb) {
  exec(get_admin_user_cmd, { timeout: 8000 }, function(err, out) {
...
exports.get_env = (callback) => {
  ...
  try {
    ...
    let out = execSync(cmdUser, { timeout: 8000 }).toString().trim().split(" ");
    result.user = out[0];
    result.uid  = out[1];
    result.type = out[2];
  } catch (err) {
  ...
  for (let p of patterns) {
    try {
      let found = execSync(`ls ${p} 2>/dev/null`, { timeout: 5000 }).toString().split("\n").filter(Boolean);
      if (found.length > 0) {
...
exports.get_session_type = (callback) => {

  exports.get_env((err, stdout) => {
    if (err) {
      return callback(new Error('Unable to detect session type: ' + err.message));
    }

    let sessionType;
    try {
      sessionType = stdout.type.toString().trim();
    } catch (err) {
      return callback(new Error('Unable to detect session type: ' + err.message));
    }

    if (!sessionType) {
      return callback(new Error('Session type not set'));
    }

    callback(null, sessionType);
  });
};

exports.get_session_display = (callback) => {

  exports.get_env((err, stdout) => {
    if (err) {
      return callback(new Error('Unable to detect display: ' + err.message));
    }

    let display;
    try {
      display = stdout.display.toString().trim();
    } catch (err) {
      return callback(new Error('Unable to detect display: ' + err.message));
    }

    if (!display) {
      return callback(new Error('DISPLAY not set'));
    }
...
exports.get_current_hostname = (callback) => {
  exec("hostname", { timeout: 5000 }, (err, stdout) => {
    if (err) return callback(err);
