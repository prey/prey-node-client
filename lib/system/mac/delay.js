...
exports.get = function(callback) {
  ...
  fs.exists(bin_path, function(exists){
    if (!exists) return callback(new Error('Bin path not found: ' + bin_path));
	  ...
	  exec('crontab -l', function(err, stdout){
		  ...
		  if (!delay_value) return callback(new Error('No value found.'));
...
exports.set = function(new_delay, callback){
	...
	echo "' + delay_string + " * * * * " + bin_path + ' &> ' + log_path + '" | crontab -'
