...
var get_logged_user_pid = function() {
  ...
  cp.exec(cmd, { timeout: 8000 }, function(err, stdout, stderr){
...
exports.find_logged_user = function(cb) {
  cp.exec(get_logged_users_cmd, { timeout: 8000 }, function(err, out) {
...
exports.find_admin_user = function(cb) {
  cp.exec(get_admin_user_cmd, { timeout: 8000 }, function(err, out) {
    if (err) return cb(err);
...
exports.get_os_version = function(cb) {
  var release;
  try {
    release = cp.execSync('sw_vers -productVersion', { timeout: 5000 });
  } catch (e) {
...
exports.get_current_hostname = (callback) => {
  cp.exec("scutil --get LocalHostName", { timeout: 5000 }, (err, stdout) => {
...
/**
 * os.cpus() can come back empty (and is typed as possibly absent), so every
 * field falls back rather than dereferencing cpus[0] blind.
 *
 * @param {(err: Error|null, info: {model: string, speed: number, cores: number}) => void} callback
 * @returns {void}
 */
exports.get_processor_info = function(callback) {
  var cpus = os.cpus() || [];
  var first = cpus[0] || { model: '', speed: 0 };
  var cpu_info = {
  ...
  if (exports.is_m1_or_m2())
      cpu_info.speed = cpu_info.speed * 100; ///////////////////////when it is M1, it returns the value in Ghz, it is the reason because it is multiplied by 100
...
exports.is_m1_or_m2 = function(){
  var cpus = os.cpus() || [];
  var  model = ((cpus[0] || { model: '' }).model || '').trim();
...
exports.get_prey_user_version = function(cb) {
  try {
    ...
    prey_user_version = cp.execSync(cmd, { timeout: 5000 });
