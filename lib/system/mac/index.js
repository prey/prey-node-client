...
exports.get_prey_user_version = function(cb) {
  ...
  let cmd = '';
  if (paths.current) {
    cmd = join(paths.current,'bin',file_prey_user);
    if (!fs.existsSync(cmd)) {
      return cb(new Error("Unable to get prey-user-version,don't exist  prey-user-version"))
    }
    cmd = cmd + ' -v';
  }
  else{
    return cb(new Error("Unable to get prey-user-version,don't exist current")) 
  }
