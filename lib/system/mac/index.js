...
var cp = require('child_process'),
  osName = process.platform.replace('darwin', 'mac'),
  os = require('os'),
  paths = require('./../paths'),
...
exports.find_logged_user = function (cb) {
  cp.exec(get_logged_users_cmd, function (err, out) {
    ...
    var items = out.split('\n');
    var non_root = items.filter(function (x) {
      return x != '' && x != 'root';
    });
...
exports.find_admin_user = function (cb) {
  cp.exec(get_admin_user_cmd, function (err, out) {
    ...
    var non_root = admins.filter(function (x) {
      return x != '' && x != 'root' && x != 'GroupMembership:';
    });
    ...
    exports.find_logged_user(function (err, logged_user) {
      ...
      if (non_root.indexOf(logged_user) != -1) return cb(null, logged_user);
      else return cb(null, non_root[0]);
    });
  });
};
...
exports.get_python_version = (callback) => {
  ...
  cp.exec(
    'python' + python_version + ' || python3' + python_version,
    (err, stdout) => {
      if (err) return callback(err);
...
exports.get_processor_info = function (callback) {
  ...
  if (exports.is_m1_or_m2()) cpu_info.speed = cpus[0].speed * 100; ///////////////////////when it is M1, it returns the value in Ghz, it is the reason because it is multiplied by 100
...
exports.is_m1_or_m2 = function () {
  ...
  if (model.includes('M1') || model.includes('M2')) return true;
  return false;
};
  ...
  try {
    ...
    if (paths.versions) {
      ...
      if (!fs.existsSync(cmd)) {
        return cb(new Error("Doesn't exist file prey-user."));
      }
      cmd = cmd + ' -v';
    } else {
      return cb(new Error("Doesn't directory versions"));
    }
