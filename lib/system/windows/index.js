...
//add johao
/* si.battery((memory) => {
  logger.info("batterybattery:;" + JSON.stringify(memory ))
 
}) 
si.processes((memory) => {
  logger.info("currentLoad:;" + JSON.stringify(memory ))
  ...
  /*  si.battery((battery) => {
      logger.info("battery:;" + JSON.stringify(battery ))
    ...
    wmic.get_value('computersystem', 'username', null, function(err, wmiout) {
      ...
      logger.info("get_value username;" +wmiout )
      logger.info("get_value err;" +err )
      wmiout = '';*/
      ...
      /* {
        if (gte(common.os_release,'10.0.0')) {  // retry only for windows 10
          exec("powershell (Get-WmiObject -Class win32_computersystem).UserName", (err, psout) => {//errro aqui
  
            logger.info("get_value powershell username;" +psout )
            logger.info("get_value err;" +err )
