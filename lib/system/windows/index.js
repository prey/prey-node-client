...
var path = require('path'),
  ...
  exec = cp.exec,
  spawn = cp.spawn,
  osName = os.platform().replace('win32', 'windows');
...
exports.find_logged_user = function (callback) {
  ...
  wmic.get_value('computersystem', 'username', null, function (err, wmiout) {
    if (err || wmiout.toString().trim() == '') {
      ...
      si.users((user) => {
        if (!computerName || !user || user.length === 0 || !user[0].user)
...
exports.get_os_edition = (callback) => {
  ...
  var common = require('../../common'),
    gte = common.helpers.isGreaterOrEqual;

  if (gte(common.osRelease, '10.0.0')) {
    //10.0.20000 is w11 //10.0.0 is w10
    si.osInfo((stdoutsi) => {
      if (
        !stdoutsi ||
        !stdoutsi.distro ||
        stdoutsi.distro.toString().trim() == ''
      ) {
        if (typeof callback !== 'function') return;
...
exports.get_winsvc_version = function (callback) {
  var common = require('../../common'),
    gte = common.helpers.isGreaterOrEqual;
  if (osName != 'windows' || !gte(common.osRelease, '10.0.0')) {
    if (typeof callback !== 'function') return;
    return callback(null, null);
  }
  exec(
    path.join(paths.install, 'wpxsvc.exe') + ' -winsvc=version',
    (err, stdout) => {
      if (err) {
        if (typeof callback !== 'function') return;
        return callback(null, null);
...
exports.compatible_with_module_tpm = function (data) {
  ...
  if (
    data.osName == 'windows' &&
    gte(os.release().trim(), '10.0.0') &&
    data.os_edition &&
    editions.includes(data.os_edition) &&
    data.winsvc_version &&
    gte(data.winsvc_version, '2.0.0')
  )
    return true;
  return false;
};
