...
exports.find_logged_user = function(callback) { 
  ...
  var logger     = require('./../../common').logger.prefix('system');

    wmic.get_value('computersystem', 'username', null, function(err, wmiout) {
      logger.info("computersystem err;" + err );
      logger.info("computersystem wmiout;" + JSON.stringify(wmiout) );
      if (err || wmiout.toString().trim() == ''){
        var computerName = (process.env['COMPUTERNAME'] || process.env['COMPUTERNAME'] == '') ? 'Unknown Computer':process.env['COMPUTERNAME'] ; 
        si.users((user) => {
       //  user = [];

      logger.info("user user;" + JSON.stringify(user) );
 
          let out = computerName + "\\" + ((user.length > 0) ? user[0].user : 'Unknown User');

          logger.info("computersystem users;" + JSON.stringify(user) ); 
          logger.info("computersystem users;" + JSON.stringify(out) ); 
...
exports.get_os_edition = (callback) => {
  ...
  var logger     = common.logger.prefix('system');

  logger.info("pasa por get_os_edition:" + os_name);
  logger.info("pasa por get_os_edition:" + gte(common.os_release,"10.0.0"));
  ...
  if (gte(common.os_release,"10.0.0")) { //10.0.20000 is w11 //10.0.0 is w10
    si.osInfo((stdoutsi) => {
      ...
      logger.info("get_os_edition osInfo;" + JSON.stringify(stdoutsi) );

      if (typeof callback !== 'function')
      logger.info("get_os_edition callback; no es callback function" );
        if (typeof callback !== 'function')
          return;

          var edition = "Home";
          if (stdoutsi && stdoutsi.distro) {
            edition = stdoutsi.distro.split(" ").splice(3)[0];     
          }
       

      logger.info("get_os_edition edition;" + edition );
...
exports.get_winsvc_version = function(callback) {
  ...
  exec(path.join(paths.install ,'wpxsvc.exe') + ' -winsvc=version', (err, stdout) => {
    ...
    logger.info("get_winsvc_version data: " + JSON.stringify(stdout));
    if (err){
      if (typeof callback !== 'function')
        return;
      return callback(null, null);
