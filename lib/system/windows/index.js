...
exports.find_logged_user = (callback) => {
  ...
  cp.exec('powershell -Command "(Get-WmiObject -Class win32_computersystem).UserName"', (err, psout) => {
    ...
    //if (!gte(common.os_release, '10.0.22000')) return done(null, psout);

    // Get the session ID from the current logged user 
    cp.exec(`powershell -Command "Get-WmiObject Win32_Process | Where-Object { $_.Name -eq 'explorer.exe' -and $_.GetOwner().User -eq '${username}' } | Select-Object -ExpandProperty SessionId"`, (err, stdout) => {
      if (err) logger.info("ERROR2 CMD! " + err);
      targetSessionID = parseInt(stdout.trim());
      logger.info("TGID: " + targetSessionID)

      // Get locked session id's and compare with the current one
      cp.exec('powershell -Command "(Get-Process -Name LogonUI -ErrorAction SilentlyContinue).SessionId"', (err2, stdout2) => {
        if (err2) logger.info("ERROR3 CMD! " + err2);
        const sessionIds = stdout2.split('\n').filter(sessionId => sessionId.trim() !== '').map(id => parseInt(id.trim(), 10));
