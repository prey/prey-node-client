...
exports.find_logged_user = (callback) => {
  ...
  exec('powershell -Command "(Get-WmiObject -Class win32_computersystem).UserName"', (err, psout) => {
    ...
    exec(`powershell -Command "Get-WmiObject Win32_Process | Where-Object { $_.Name -eq 'explorer.exe' -and $_.GetOwner().User -eq '${username}' } | Select-Object -ExpandProperty SessionId"`, (errPower, sidout) => {
      ...
      exec('powershell -Command "(Get-Process -Name LogonUI -ErrorAction SilentlyContinue).SessionId"', (errPowerGet, lockedSid) => {
