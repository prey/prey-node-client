...
// process may inherit an invalid/read-only cwd, making spawn throw synchronously
// (e.g. `spawn EROFS`). Forcing a known-good cwd avoids that failure mode.
...
/**
 * @param {(err: (Error|null), version: (string|null)) => void} cb
 */
const get_version = (cb) => {
  if (version_cache) return cb(null, version_cache);
  try {
    return exec(`"${get_bin()}" -winsvc=version`, { cwd: SAFE_CWD }, (err, stdout) => {
      if (!err && stdout) version_cache = stdout.split('\n')[0].trim();
      return cb(null, version_cache || null);
    });
  } catch (e) {
    // synchronous spawn throw — report "unknown version" so callers fall back
    return cb(null, null);
  }
};
