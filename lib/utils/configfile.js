...
class ConfigFile {
  ...
  all = (callback) => {
    this.load(() => {
      if (callback && typeof callback === 'function') return callback(this.preyConfiguration);
      return this.preyConfiguration;
    });
  };

  getData = (key) => this.preyConfiguration[key];

  getDataFromDb = (cb) => {
    this.load(() => {
      if (cb && typeof cb === 'function') cb(this.preyConfiguration);
    });
  };

  getDataFromDbKey = (key, cb) => {
    this.load(() => {
      if (cb && typeof cb === 'function') cb(this.preyConfiguration[key]);
    });
  };
