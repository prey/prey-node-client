...
const fetchEnvVar = require('./fetch-env-var');
...
const dictionary = {
  ...
  'control-panel.send_location_on_connect': 'prey_control_send_location',
  'control-panel.force_wifi_on': 'prey_control_force_wifi_on',
  'control-panel.permissions.wifi_location': 'prey_control_wifi_location',
  'control-panel.permissions.native_location': 'prey_control_native_location',
};
...
class ConfigFile {
  ...
  preyConfiguration = {
    ...
    'control-panel.force_wifi_on': false,
    ...
    'control-panel.tracking_schedule': null,
    exceptions_daily_limit: 50,
    exceptions_per_error_limit: 3,
  };
  ...
  load = (cb) => {
    ...
    getDataDb('preyconf', (err, dataDb) => {
          ...
          if (!err && !dataDb) {
            getDataDb('shouldPreyCFile', (errorGetData, dataFromDb) => {
              ...
              if (dataFromDb && dataFromDb.length > 0) {
                ...
                storage.do('set', { type: 'keys', id: 'shouldPreyCFile', data: { value: gotApiDeviceKey } }, (errSetting) => {
      ...
      let data;
      try {
        data = JSON.parse(dataDb[0].value);
      } catch (e) {
        logger.error(`Error parsing preyconf data: ${e}`);
      }
      if (!data) return this.setFull(cb);
      Object.keys(this.preyConfiguration).forEach((key) => {
        if (Object.hasOwn(data, key)) this.preyConfiguration[key] = data[key];
      });
  ...
  getData = (key) => {
    try {
      if (fetchEnvVar('debug') && fetchEnvVar(dictionary[`${key}`])) {
        return fetchEnvVar(dictionary[`${key}`]);
      }
  ...
  onDataChange = (key, fn) => {
    if (!this.changeListeners[key]) this.changeListeners[key] = [];
    this.changeListeners[key].push(fn);
  };
   ...
   * @param {function} fn
   */
  offDataChange = (key, fn) => {
    if (this.changeListeners[key]) {
      this.changeListeners[key] = this.changeListeners[key].filter((f) => f !== fn);
    }
  };

  setData = (key, value, cb) => {
    const oldValue = this.preyConfiguration[key];
    ...
    if (oldValue !== value && this.changeListeners[key]) {
      this.changeListeners[key].forEach((fn) => fn(value, oldValue));
    }
  };
