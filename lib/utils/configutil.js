...
const setKey = (whatToGet, valuetoSet, cb) => {
  storage.do(
    'set',
    { type: 'keys', id: whatToGet, data: { value: typeof valuetoSet === 'string' ? valuetoSet : JSON.stringify(valuetoSet) } },
    () => { cb(); },
  );
};
/**
...
const updateKey = (whatToGet, valuetoSet, cb) => {
  storage.do('update', {
    type: 'keys', id: whatToGet, columns: 'value', values: typeof valuetoSet === 'string' ? valuetoSet : JSON.stringify(valuetoSet),
  }, () => { cb(); });
};
/**
